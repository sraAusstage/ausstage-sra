...
"ts.style_element_id = style_file_element_link.style_element_id AND style_file_el" +
"ement_link.style_file_id = "
 + m_style_file_id + " " + "ORDER BY style_file_element_link.style_target_id";
    ...
    private String getMetadata()
    {
        try
        {
            ...
            if(m_page_id != null && !m_page_id.equals("null") && !m_page_id.equals(""))
            {
                ...
                for(l_buffer = l_buffer + ("<META name=\"TITLE\" content=\"" + m_page_title + "\">\n"); rset.next(); l_buffer = l_buffer + ("<META name=\"" + rset.getString("name") + "\" content=\"" + rset.getString("value") + "\">\n")) { }
                rset.close();
            }
            stmt.close();
            String s1 = l_buffer;
...
;
            ...
            {
                sqlString = "SELECT item_repository.* FROM item_repository, item_layout_text_rel, layout_text" +
"_item WHERE layout_text_item.layout_text_item_title = '"
 + p_item_title + "' " + "AND layout_text_item.layout_text_item_id = item_layout_text_rel.layout_text_item" +
"_id "
 + "AND item_layout_text_rel.page_id = " + m_page_id + " " + "AND item_layout_text_rel.item_id = item_repository.item_id ";
    ...
    public String breadCrumbControl(String p_alt_current, boolean p_preview)
    {
        ...
        try
        {
            ...
            do
            {
                CachedRowSet l_rs = m_db.runSQL("SELECT parent.page_id, parent.title, parent.url FROM pages, pages parent WHERE p" +
"ages.page_id = "
 + l_next_page + " " + "AND pages.parent_id = parent.page_id", l_stmt);
                if(l_rs.next())
                {
    ...
    public String discussionBoardControl(boolean p_preview)
    {
        ...
        try
        {
            ...
            if(l_action.equals("savePost"))
            {
                addNewPost("0", l_chosen_topic);
                l_res = discussionByTopic(l_url, l_chosen_topic);
            } else
            if(l_action.equals("viewPost"))
            {
                l_res = viewPost(l_url, l_chosen_topic, l_post_id);
            } else
            if(l_action.equals("replyPost"))
            {
                addNewPost(l_post_id, l_chosen_topic);
                l_res = discussionByTopic(l_url, l_chosen_topic);
            } else
            if(!l_chosen_topic.equals(""))
            {
                l_res = discussionByTopic(l_url, l_chosen_topic);
            } else
            {
                l_res = "<a" + applyStyle(4) + ">Welcome to the Discussion Board. Please choose from the topics below:<BR><BR></" +
"a>"
;
    ...
    private void addNewPost(String p_parent_id, String p_topic_id)
    {
        ...
        try
        {
            ...
            {
                l_sql = "INSERT INTO discussion_posts (parent_id, topic_id, post_creator, post_date, post" +
"_subject, post_content, post_archived) values ("
 + p_parent_id + ", " + p_topic_id + ", '" + m_db.plSqlSafeString(l_creator) + "', " + m_db.safeDateFormat(l_date, true) + ", '" + m_db.plSqlSafeString(l_subject) + "', '" + m_db.plSqlSafeString(l_content) + "', 'f')";
...
"back();' style='width=70px'>&nbsp;&nbsp;&nbsp;<input name='newpostSubmit' type='" +
"Submit' value='Post' style='width=70px'>"
;
        l_buffer = l_buffer + "</td>";
        l_buffer = l_buffer + "</tr>";
        l_buffer = l_buffer + "<tr>";
    ...
    private String viewPost(String p_url, String p_chosen_topic, String p_post_id)
    {
        ...
        l_sql = "SELECT post_date, topic_name, post_creator, post_subject FROM discussion_posts, " +
"discussion_topics WHERE post_id = "
 + p_post_id + " " + "AND discussion_posts.topic_id = discussion_topics.topic_id " + "AND post_archived = 'f' ";
        ...
        try
        {
            ...
            if(l_rs.next())
            {
                ...
                l_buffer = l_buffer + "<td valign='top' " + applyStyle(4) + " width='50'>Creator:";
                l_buffer = l_buffer + "</td>";
                l_buffer = l_buffer + "<td valign='top' " + applyStyle(4) + " width='400'><input name='f_creator' " + applyStyle(4) + " type='text' style='width=300px'  maxlength='100'>";
                l_buffer = l_buffer + "</td>";
                l_buffer = l_buffer + "</tr>";
                l_buffer = l_buffer + "<tr>";
                l_buffer = l_buffer + "<td valign='top' colspan='2' " + applyStyle(4) + " width='450'><img src='" + "/images" + "/spacer.gif' width='10' height='10'>";
                ...
                l_buffer = l_buffer + "<td valign='top' " + applyStyle(4) + " width='50'>Subject:";
                l_buffer = l_buffer + "</td>";
                l_buffer = l_buffer + "<td valign='top' " + applyStyle(4) + " width='400'><input name='f_subject' " + applyStyle(4) + " type='text' style='width=300px'  maxlength='100'>";
                l_buffer = l_buffer + "</td>";
                l_buffer = l_buffer + "</tr>";
                l_buffer = l_buffer + "<tr>";
                l_buffer = l_buffer + "<td valign='top' colspan='2' " + applyStyle(4) + " width='450'><img src='" + "/images" + "/spacer.gif' width='10' height='10'>";
    ...
    private String displayDiscussionTopics(String p_url)
    {
        ...
        ResultSet l_rs = null;
        String l_buffer = "";
        l_sql = "SELECT topic_id, topic_name FROM discussion_topics WHERE topic_archived = 'f' OR" +
"DER by topic_name"
;
        try
        {
    ...
    private String discussionThreads(String p_url, String p_parent_id, String p_topic_id, String p_level)
    {
        ...
        String p_buffer = "";
        SimpleDateFormat formattedDateTime = new SimpleDateFormat("dd-MMM-yyyy HH:mm");
        l_sql = String.valueOf("SELECT post_id, post_date, post_subject, post_creator FROM discussion_posts WHER" +
"E parent_id     = "
 + p_parent_id + " " + "AND   topic_id      = " + p_topic_id + " " + "AND   post_archived = 'f' " + "ORDER by post_id ").concat(String.valueOf(AppConstants.POSTS_ORDER));
        try
        {
            ...
            for(l_rs = m_db.runSQL(l_sql, stmt); l_rs.next();)
            {
                ...
                post_subject = "<a href='" + post_expand + "'>" + l_rs.getString("post_subject") + "<BR>(by " + l_rs.getString("post_creator") + " - " + post_date_formatted + ")</a>";
    ...
    public String dropDownControl(String p_width, String p_left_pos, String p_top_pos, String p_font, String p_font_size, String p_font_lowercase, String p_font_color, 
            String p_font_hover_color, String p_back_color, String p_back_hover_color, String p_border_color, String p_seperator_color, String p_display_on_click, String p_override_image_keys, 
            boolean p_preview)
    {
        String l_res = "";
        ...
        try
        {
            ...
            l_res = "<script LANGUAGE='JavaScript1.2' SRC='/js/hm_loader.js' TYPE='text/javascript'><" +
"/script>\n"
;
    ...
    public String familyLinksControl(String p_width, String p_indents, String p_bgcolours, String p_rollover_bgcolours, String p_current_bgcolours, String p_current_rollover_bgcolours, String p_divider_colour, 
            String p_padding, String p_lineheight, String p_align, String p_valign, String p_selected_image, String p_left_image_key, String p_right_image_key, 
            String p_dividerheight, boolean p_preview)
    {
        ...
        try
        {
            ...
            if(p_rollover_bgcolours.length() > 0)
            {
                ...
                {
                    l_rollover_bgcolours.setElementAt(" onMouseOver=\"this.style.backgroundColor='" + l_rollover_bgcolours.elementAt(l_i) + "';\" onMouseOut=\"this.style.backgroundColor='" + l_bgcol_strs.elementAt(l_i) + "';\"", l_i);
                }
            ...
            if(p_current_rollover_bgcolours.length() > 0)
            {
                ...
                {
                    l_current_rollover_bgcolours.setElementAt(" onMouseOver=\"this.style.backgroundColor='" + l_current_rollover_bgcolours.elementAt(l_i) + "';\" onMouseOut=\"this.style.backgroundColor='" + l_current_bgcol_strs.elementAt(l_i) + "';\"", l_i);
                }
...
".sect_id=sections.sect_id ORDER BY sections.order_id, pages.display_in_menu, pag" +
"es.title "
;
            for(rset = m_db.runSQL(l_sql, l_stmt); rset.next();)
            {
                ...
                if(l_curr_page_id.equals(m_page_id))
                {
                    ...
                    l_buffer = l_buffer + getFamilyLinksPageHTML(l_curr_page_id, l_curr_level, p_width, l_indents.elementAt(l_curr_level).toString(), l_bgcolours.elementAt(l_curr_level).toString(), l_rollover_bgcolours.elementAt(l_curr_level).toString(), l_current_bgcolours.elementAt(l_curr_level).toString(), l_current_rollover_bgcolours.elementAt(l_curr_level).toString(), l_divider_colour, l_padding, l_lineheight, l_align, l_valign, 11, l_selected_image_path, l_left_image_path.elementAt(l_curr_level).toString(), l_right_image_path.elementAt(l_curr_level).toString(), p_dividerheight, l_page_url, l_page_title);
                } else
                {
                    l_buffer = l_buffer + getFamilyLinksPageHTML(l_curr_page_id, l_curr_level, p_width, l_indents.elementAt(l_curr_level).toString(), l_bgcolours.elementAt(l_curr_level).toString(), l_rollover_bgcolours.elementAt(l_curr_level).toString(), l_current_bgcolours.elementAt(l_curr_level).toString(), l_current_rollover_bgcolours.elementAt(l_curr_level).toString(), l_divider_colour, l_padding, l_lineheight, l_align, l_valign, 9, l_selected_image_path, l_left_image_path.elementAt(l_curr_level).toString(), l_right_image_path.elementAt(l_curr_level).toString(), p_dividerheight, l_page_url, l_page_title);
                }
                l_buffer = l_buffer + generateChildren(l_curr_page_id, l_curr_level + 1, l_width, l_indents, l_bgcolours, l_rollover_bgcolours, l_current_bgcolours, l_current_rollover_bgcolours, l_divider_colour, l_padding, l_lineheight, l_align, l_valign, l_selected_image_path, l_left_image_path, l_right_image_path, p_dividerheight, l_pages_to_display, l_displayed_current_page);
            }
    ...
    private String generateChildren(String p_page_id, int p_curr_level, String p_width, Vector p_indents, Vector p_bgcolours, Vector p_rollover_bgcolours, Vector p_current_bgcolours, 
            Vector p_current_rollover_bgcolours, String p_divider_colour, String p_padding, String p_lineheight, String p_align, String p_valign, String p_selected_image_path, 
            Vector p_left_image_path, Vector p_right_image_path, String p_dividerheight, Vector p_pages_to_display, boolean p_displayed_current_page)
    {
        String l_buffer = "";
        ...
        try
        {
            ...
            {
                String sqlString = "SELECT page_id, title, url FROM pages WHERE parent_id='" + p_page_id + "' and " + "is_approved = 't' and " + "display_in_menu > 0 and " + "hidden_page = 'f' " + "ORDER BY display_in_menu, title ";
                ...
                do
                {
                    ...
                    if(p_pages_to_display.contains(l_curr_page_id))
                    {
                        ...
                        {
                            l_buffer = l_buffer + getFamilyLinksPageHTML(l_curr_page_id, p_curr_level, p_width, p_indents.elementAt(p_curr_level).toString(), p_bgcolours.elementAt(p_curr_level).toString(), p_rollover_bgcolours.elementAt(p_curr_level).toString(), p_current_bgcolours.elementAt(p_curr_level).toString(), p_current_rollover_bgcolours.elementAt(p_curr_level).toString(), p_divider_colour, p_padding, p_lineheight, p_align, p_valign, 10, p_selected_image_path, p_left_image_path.elementAt(p_curr_level).toString(), p_right_image_path.elementAt(p_curr_level).toString(), p_dividerheight, l_page_url, l_page_title);
                        } else
                        if(l_curr_page_id.equals(m_page_id))
                        {
                            l_displayed_current_page = true;
                            l_buffer = l_buffer + getFamilyLinksPageHTML(l_curr_page_id, p_curr_level, p_width, p_indents.elementAt(p_curr_level).toString(), p_bgcolours.elementAt(p_curr_level).toString(), p_rollover_bgcolours.elementAt(p_curr_level).toString(), p_current_bgcolours.elementAt(p_curr_level).toString(), p_current_rollover_bgcolours.elementAt(p_curr_level).toString(), p_divider_colour, p_padding, p_lineheight, p_align, p_valign, 11, p_selected_image_path, p_left_image_path.elementAt(p_curr_level).toString(), p_right_image_path.elementAt(p_curr_level).toString(), p_dividerheight, l_page_url, l_page_title);
                        } else
                        {
                            l_buffer = l_buffer + getFamilyLinksPageHTML(l_curr_page_id, p_curr_level, p_width, p_indents.elementAt(p_curr_level).toString(), p_bgcolours.elementAt(p_curr_level).toString(), p_rollover_bgcolours.elementAt(p_curr_level).toString(), p_current_bgcolours.elementAt(p_curr_level).toString(), p_current_rollover_bgcolours.elementAt(p_curr_level).toString(), p_divider_colour, p_padding, p_lineheight, p_align, p_valign, 9, p_selected_image_path, p_left_image_path.elementAt(p_curr_level).toString(), p_right_image_path.elementAt(p_curr_level).toString(), p_dividerheight, l_page_url, l_page_title);
                        }
                        l_buffer = l_buffer + generateChildren(l_curr_page_id, p_curr_level + 1, p_width, p_indents, p_bgcolours, p_rollover_bgcolours, p_current_bgcolours, p_current_rollover_bgcolours, p_divider_colour, p_padding, p_lineheight, p_align, p_valign, p_selected_image_path, p_left_image_path, p_right_image_path, p_dividerheight, p_pages_to_display, l_displayed_current_page);
                    }
                } while(true);
                rset.close();
            }
    ...
    public String getFamilyLinksPageHTML(String p_curr_page_id, int p_curr_level, String p_width, String p_indent, String p_bgcolour, String p_rollover_bgcolour, String p_current_bgcolour, 
            String p_current_rollover_bgcolour, String p_divider_colour, String p_padding, String p_lineheight, String p_align, String p_valign, int p_style, 
            String p_selected_image_path, String p_left_image_path, String p_right_image_path, String p_dividerheight, String p_url, String page_title)
    {
        String l_buffer = "";
        try
        {
            ...
            l_buffer = l_buffer + ("<td width=\"" + p_indent + "\"" + "><img src=\"" + "/images" + "/spacer.gif\" width=\"" + p_indent + "\" height=\"" + p_lineheight + "\"></td>");
            l_buffer = l_buffer + ("<td valign=\"bottom\" width=\"" + (Integer.parseInt(p_width) - Integer.parseInt(p_indent)) + "\"" + p_bgcolour + " " + p_rollover_bgcolour + " " + p_align + ">");
            ...
            l_buffer = l_buffer + ("<table width='100%' cellpadding='0' cellspacing='0'><tr><td width=\"" + (Integer.parseInt(p_width) - Integer.parseInt(p_indent)) + "\"" + p_divider_colour + " height='" + p_dividerheight + "'><img border=\"0\" src=\"" + "/images" + "/spacer.gif\" WIDTH=\"" + (Integer.parseInt(p_width) - Integer.parseInt(p_indent)) + "\" HEIGHT=\"1\"></td></tr></table>");
...
"types WHERE file_repository.filt_id = file_types.filt_id ORDER BY file_repositor" +
"y.file_title ASC) WHERE ROWNUM <= 5"
;
                } else
                {
...
"RE file_repository.filt_id = file_types.filt_id ORDER by file_repository.file_ti" +
"tle ASC"
;
                }
            } else
...
"tory.file_repos_id = files_pages_rel.file_repos_id AND files_pages_rel.page_id =" +
" "
 + m_page_id + " " + "AND file_repository.filt_id = file_types.filt_id " + "ORDER by file_repository.file_title ASC";
            }
            CachedRowSet rset1;
            for(rset1 = m_db.runSQL(sqlString, stmt1); rset1.next();)
            {
                ...
                {
                    l_control = l_control + ("<td valign='top' align='left'><a" + applyStyle(8) + " href='Javascript://' onclick='Javascript:openVersionHistory();'>" + l_file_title + " (" + l_file_size_conv + " KB)</a>\n");
                    l_control = l_control + "<script language='Javascript'>\n";
                    l_control = l_control + "<!--\n";
                    l_control = l_control + "function openVersionHistory() { \n";
                    l_control = l_control + ("window.open('content_repos_history_1.jsp?index=" + l_file_id + "&public=1','view_window','toolbar=no,width=550,height=370,directories=0,status=0" +
",scrollbars=1,menubar=0,location=0,resizable=1'); "
);
                    l_control = l_control + "}\n";
                    l_control = l_control + " //-->\n";
                ...
                {
                    l_control = l_control + ("<td valign='top' align='left'><a" + applyStyle(8) + " href='" + "/custom/files" + l_file_path + "' target='_blank'>" + l_file_title + " (" + l_file_size_conv + " KB)</a>");
                }
                l_control = l_control + "</td>";
                l_control = l_control + l_padding;
                l_control = l_control + "</tr>";
                l_control = l_control + "<tr>";
                l_control = l_control + l_padding;
                l_control = l_control + "<td colspan=2 valign='top' align='left'><img src='/custom/images/spacer.gif' widt" +
"h=100% height=5>"
;
                l_control = l_control + "</td>";
                l_control = l_control + l_padding;
...
"_id = layout_template_links.page_id AND layout_template_links.link_section = 'fo" +
"oter' AND layout_template_links.layout_template_id = "
    ...
    public String formControl(String p_name, boolean p_preview)
    {
        try
        {
            ...
            if(p_preview)
            {
                ...
                l_form_body = l_form_body + "<tr><td width='20' " + applyStyle(15) + ">name:</td><td width='10'>&nbsp;</td><td width='40'><input " + applyStyle(12) + " type=text value='Test Name'></td></tr>";
                l_form_body = l_form_body + "<tr><td width='20' " + applyStyle(15) + ">Age:</td><td width='10'>&nbsp;</td><td width='40'><input " + applyStyle(12) + " type=text value='Test Age'></td></tr>";
                l_form_body = l_form_body + "<tr><td width='20' " + applyStyle(15) + ">Address:</td><td width='10'>&nbsp;</td><td width='40'><input " + applyStyle(12) + " type=text value='Test Address'></td></tr>";
                l_form_body = l_form_body + "<tr><td width='20' " + applyStyle(15) + ">Phone Number:</td><td width='10'>&nbsp;</td><td width='40'><input " + applyStyle(12) + " type=text value='Test Phone Number'></td></tr>";
                l_form_body = l_form_body + "<tr><td width='20' " + applyStyle(15) + ">Email:</td><td width='10'>&nbsp;</td><td width='40'><input " + applyStyle(12) + " type=text value='Test Email'></td></tr>";
                l_form_body = l_form_body + "<tr><td colspan='3'>&nbsp;</td></tr>";
                l_form_body = l_form_body + "<tr><td align='left'><input " + applyStyle(13) + " type='button' value='Cancel'></td><td align='center'><input " + applyStyle(13) + " type='button' value='Clear'></td><td align='right'><input " + applyStyle(13) + " type='button' value='Submit'></td></tr>";
                l_form_body = l_form_body + "<tr><td colspan='3'>&nbsp;</td></tr>";
                l_form_body = l_form_body + "</table></form>";
                l_form_body = "<span" + applyStyle(4) + ">" + l_form_body + "</span>";
            } else
            {
                sqlString = "SELECT form_layout.form_layout_id FROM form_layout, form_repository, form_layout" +
"_form_rel, layout_form_item WHERE form_layout_form_rel.page_id="
 + m_page_id + " AND " + "form_layout_form_rel.form_id=form_repository.form_id AND " + "form_repository.form_id=form_layout.form_id AND " + "layout_form_item.layout_form_item_id=form_layout_form_rel.layout_form_item_id AN" +
"D "
 + "layout_form_item.layout_form_item_title='" + m_db.plSqlSafeString(p_name) + "'";
...
"t_form_item where form_layout_form_rel.layout_form_item_id=layout_form_item.layo" +
"ut_form_item_id and form_layout_form_rel.page_id="
 + m_page_id + " and form_layout_form_rel.flfr_id=" + "form_process.flfr_id";
...
"sitory.form_id=form_layout.form_id and form_layout.form_layout_id="
 + form_layout_id;
    ...
    public String headerLinksControl(boolean p_preview)
    {
        ...
        try
        {
            ...
            {
                String l_sql = "SELECT pages.title, pages.url FROM pages, layout_template_links WHERE pages.page" +
"_id = layout_template_links.page_id AND layout_template_links.link_section = 'he" +
"ader' AND layout_template_links.layout_template_id = "
 + m_layout_template_id + " " + "ORDER BY layout_template_links.link_order ASC, pages.title ASC";
                CachedRowSet l_rs;
...
"<input name='loginpageid' id='loginpageid' type='hidden' value='"
 + m_page_id + "'>" + "<input name='redirectpageid' id='redirectpageid' type='hidden' value='" + l_redirectPageId + "'>";
    ...
    public String searchControl(HttpServletRequest p_request, String p_button_repos_key, String p_table_width, String p_title_text, String p_helper_text, String p_bg_color, String p_page_id, 
            String p_show_search_types, String p_show_results_per_page, String p_show_order_by, String p_show_scope, String p_default_scope, String p_show_external_search_checkbox, String p_external_search_checkbox_default, 
            String p_show_natural_lang_button, String p_show_precision_search_button, String p_show_metadata_select, boolean p_preview)
    {
        String ret = "";
        ...
        try
        {
            ...
            Statement l_stmt = m_db.m_conn.createStatement();
            String sqlString = "select layout_display_file from layout_templates, pages where pages.layout_templ" +
"ate_id=layout_templates.layout_template_id and pages.page_id="
 + p_page_id;
            CachedRowSet rset = m_db.runSQL(sqlString, l_stmt);
            if(rset.next())
            {
...
;
            ...
            if(AppConstants.DATABASE_TYPE == 1)
            {
                if(p_show_external_search_checkbox != null && p_show_external_search_checkbox.equals("true"))
                {
                    ...
                    if(l_external_search != null && l_external_search.equals("true"))
                    {
                        ret = ret + " checked";
                    }
    ...
    public String searchResultsControl(HttpServletRequest p_request, String p_altColour, boolean p_preview)
    {
        ...
        try
        {
            Search basicSearch = new Search(m_db);
            String searchString = p_request.getParameter("str");
            String searchType = p_request.getParameter("type");
            DOMUtil l_domUtil = new DOMUtil();
            String l_showXml = p_request.getParameter("showxml");
            String l_searchresultsURL = p_request.getRequestURI();
            String l_order = p_request.getParameter("orderby");
            String l_page_id = p_request.getParameter("xcid");
            String l_doco_repos_search_type = p_request.getParameter("rst");
            String l_external_search = p_request.getParameter("external");
            String l_natural_lang = p_request.getParameter("nat_lang");
            String l_metadata_type[] = p_request.getParameterValues("mdt");
            Vector l_metadata_type_vec = new Vector();
            HttpSession l_session = p_request.getSession(true);
            if(searchString != null && searchType != null && !searchString.equals("null") && !searchString.equals(""))
            {
                int resultsPerPage = Integer.parseInt(p_request.getParameter("rpp"));
                int currentPage = Integer.parseInt(p_request.getParameter("page"));
                basicSearch.setSearchType(searchType);
                basicSearch.setSearchString(searchString);
                basicSearch.setResultsPerPage(resultsPerPage);
                basicSearch.setSearchResultsPageId(l_page_id);
                if(l_external_search == null || !l_external_search.equals("true"))
                {
                    basicSearch.setExternalSearch(false);
                } else
                {
                ...
                if(l_session.getAttribute("publicAuthId") != null && !l_session.getAttribute("publicAuthId").equals(""))
                {
                    basicSearch.setPublicLoggedInAuthor(l_session.getAttribute("publicAuthId").toString());
                }
                basicSearch.performSearch();
                String l_xml = basicSearch.getXMLResults();
                if(l_showXml == null)
                {
                    boolean l_orderBy;
                    if(l_order == null || l_order.equals("title"))
                    {
                        l_orderBy = false;
                    } else
                    {
                    ...
                    ret = ret + "<form method='POST' name='searchResultsForm' id='searchResultsForm'>\n\r";
                    String l_xsl = basicSearch.generateXSL(currentPage, resultsPerPage, l_searchresultsURL, l_orderBy, p_altColour, p_request);
                    ret = DOMUtil.transformXmlWithXsl(l_xml, l_xsl);
                    ret = ret + ("<input type='hidden' name='xcid' value='" + l_page_id + "'>");
                    ret = ret + "</form>\n\r";
                } else
                {
                    ret = l_xml;
    ...
    public String netMeetingControl(String p_mode, boolean p_preview)
    {
        ...
        try
        {
            ...
            if(p_mode.equals("DataOnly"))
            {
                ...
                l_buffer = l_buffer + "  <td>&nbsp;</td>";
                l_buffer = l_buffer + "  <td>";
                l_buffer = l_buffer + ("    <input type=button " + applyStyle(13) + " value='Call' id=CallToBtn onclick=NetMeeting.CallTo(CallToAddress.value) style=" +
"'width:60'>&nbsp;"
);
                l_buffer = l_buffer + "  </td>";
                l_buffer = l_buffer + "</tr>";
                ...
                l_buffer = l_buffer + "  <td>";
                l_buffer = l_buffer + ("    <input type=button " + applyStyle(13) + " value='Hang Up' id=HangUpBtn onclick=NetMeeting.LeaveConference() style='width:" +
"60'>"
);
                l_buffer = l_buffer + "  </td>";
                l_buffer = l_buffer + "</tr>";
