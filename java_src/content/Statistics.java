...
public class Statistics
{
    ...
    public void logPublicEvent(String p_browser_ip, String p_page_title)
    {
        try
        {
            ...
            {
                sqlString = "INSERT INTO stats_public (stats_ip, stats_date, page_title) VALUES ('" + p_browser_ip + "', to_date(" + sqlDate + "), '" + m_db.plSqlSafeString(p_page_title) + "')";
...
"><Properties BorderType='None' AutoWidth='False' AutoHeight='False'/><Text></Tex" +
"t></Textbox></Chart>"
;
        ...
        try
        {
            ...
            if(p_chart.equals("hits_homepage"))
            {
                ...
                if(m_rs.next())
                {
                    ...
                    m_sql = "SELECT count(stats_public_id) as totalValue, stats_date as actualValue FROM stat" +
"s_public WHERE stats_date between "
 + m_startDate + " AND " + m_endDate + " " + "AND page_title = '" + m_rs.getString("title") + "' " + "GROUP BY stats_date " + "ORDER BY stats_date ASC";
                }
                m_rs.close();
            } else
            if(p_chart.equals("hits_all"))
            {
                p_title = "Total Site Hits (" + m_startDateTitle + " to " + m_endDateTitle + ")";
                m_sql = "SELECT count(stats_public_id) as totalValue, stats_date as actualValue FROM stat" +
"s_public WHERE stats_date between "
 + m_startDate + " AND " + m_endDate + " " + "GROUP BY stats_date " + "ORDER BY stats_date ASC";
            } else
            if(p_chart.equals("hits_top10"))
            {
                p_title = "Top 10 Page Hits (" + m_startDateTitle + " to " + m_endDateTitle + ")";
                m_sql = "SELECT count(stats_public_id) as totalValue, page_title as actualValue FROM stat" +
"s_public WHERE stats_date between "
 + m_startDate + " AND " + m_endDate + " " + "GROUP BY page_title ORDER BY totalValue DESC";
            } else
            if(p_chart.equals("vistors_top10"))
            {
                p_title = "Top 10 Visitors (" + m_startDateTitle + " to " + m_endDateTitle + ")";
                m_sql = "SELECT count(stats_public_id) as totalValue, stats_ip as actualValue FROM stats_" +
"public WHERE stats_date between "
 + m_startDate + " AND " + m_endDate + " " + "GROUP BY stats_ip ORDER BY totalValue DESC";
            } else
            if(p_chart.equals("admin_logins_daily"))
            {
                p_title = "Daily Administration Logins (" + m_startDateTitle + " to " + m_endDateTitle + ")";
                m_sql = "SELECT count(stats_admin_id) as totalValue, stats_date as actualValue FROM stats" +
"_admin WHERE stats_date between "
 + m_startDate + " AND " + m_endDate + " " + "AND fun_id = " + "-1" + " " + "GROUP BY stats_date " + "ORDER BY stats_date ASC";
            } else
            if(p_chart.equals("admin_content"))
            {
                p_title = "Daily Content Authoring (" + m_startDateTitle + " to " + m_endDateTitle + ")";
                m_sql = "SELECT count(stats_admin_id) as totalValue, stats_date as actualValue FROM stats" +
"_admin WHERE stats_date between "
 + m_startDate + " AND " + m_endDate + " " + "AND (fun_id = " + "1" + " " + "OR fun_id = " + "2" + " " + "OR fun_id = " + "3" + " " + "OR fun_id = " + "4" + " " + "OR fun_id = " + "5" + ") " + "GROUP BY stats_date " + "ORDER BY stats_date ASC";
            }
