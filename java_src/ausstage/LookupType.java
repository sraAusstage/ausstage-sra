...
public class LookupType {
	 ...
	 * 
	 * Parameters: code_type : code_type of the Lookup_type record
	 * 
	 * Returns: None
	 */
	public void load(int p_id) {
		...
		try {
			...
			sqlString = "SELECT * FROM lookup_types WHERE " + "id=" + p_id;
	...
	public boolean add() {
		...
		try {
			...
			if (validateObjectForDB()) {
				...
				sqlString = "INSERT INTO lookup_types (code_type, description, system_flag, id) VALUES (" + "'" + m_db.plSqlSafeString(m_code_type) + "', '"
						+ m_db.plSqlSafeString(m_description) + "', '" + m_db.plSqlSafeString(strSystemFlag) + "', " + (rs.getInt("id") + 1) + ")";
	...
	public boolean update() {
		...
		try {
			...
			if (validateObjectForDB()) {
				...
				sqlString = "UPDATE lookup_types set code_type ='" + m_db.plSqlSafeString(m_code_type) + "',  description ='" + m_db.plSqlSafeString(m_description) + "' "
						+ ",  system_flag ='" + m_db.plSqlSafeString(strSystemFlag) + "' " + "where id=" + m_id;
				m_db.runSQL(sqlString, stmt);
				l_ret = true;
			}
			stmt.close();
	 ...
	 * Name: checkInUse ()
	 * 
	 * Purpose: Checks to see if the specified Country is in use in the
	 * database.
	 * 
