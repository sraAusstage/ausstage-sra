...
public class SearchTables {
	...
	public void setTableList1(String p_Table_List1, String p_keyword_textbox1) {
		...
		if (m_Table_List1.equals("") || m_Table_List1.equals(null)) {
			m_keyword_textbox1 = "";
		} else {// make the string safe with no white space either side of the
				// string
	...
	public void setFirstDate(String p_FirstDate_dd, String p_FirstDate_mm, String p_FirstDate_yyyy) {
		if (p_FirstDate_yyyy != null) {
			m_FirstDate_yyyy = p_FirstDate_yyyy;
		}
		;
		if (p_FirstDate_mm != null) {
			m_FirstDate_mm = p_FirstDate_mm;
		}
		;
		if (p_FirstDate_dd != null) {
			m_FirstDate_dd = p_FirstDate_dd;
		}

	}

	public void setBetweenFromDate(String p_Between1_dd, String p_Between1_mm, String p_Between1_yyyy) {
		if (p_Between1_yyyy != null) {
			m_Between1_yyyy = p_Between1_yyyy;
		}
		;
		if (p_Between1_mm != null) {
			m_Between1_mm = p_Between1_mm;
		}
		;
		if (p_Between1_dd != null) {
			m_Between1_dd = p_Between1_dd;
		}

	}

	public void setBetweenToDate(String p_Between2_dd, String p_Between2_mm, String p_Between2_yyyy) {
		if (p_Between2_yyyy != null) {
			m_Between2_yyyy = p_Between2_yyyy;
		}
		;
		if (p_Between2_mm != null) {
			m_Between2_mm = p_Between2_mm;
		}
		;
		if (p_Between2_dd != null) {
			m_Between2_dd = p_Between2_dd;
		}
	...
	// the uppermost table selected will determine which results are returned to
	// the user
	private String getSelectString() {
		...
		if (m_result_type.equals("Event")) {
			...
			setDateClause();// if event is result type we need to include the
							// dates entered as part of the select clause.
	...
	public String getTables() {
		...
		/***************************************************************************************************
		 * It should be noted that in the setClause method when there are 2 or 3
		 * table lists selectd i.e. contributor, event and organisation that the
		 * contains database text search function is not used. It was found by
		 * Luke, Jim, and Justin that the contains search function works very
		 * inefficientlt with multiple conditions in the where clause. I.e.
		 * contributor.contribid = conevlink.contribid etc.. For this reason
		 * when we link over miltiple tables including using link tables we have
		 * to use LIKE pattern matching instead of the contains funciton.
		 * Contains is best and very fast if you use it with minimal or no
		 * conditions in the where clause. What is the fastest type of text
		 * query? For more information go to this link:
		 * http://otn.oracle.com/products/text/x/faqs/imt_perf_faq.html#q07 The
		 * fastest type of query will meet the following conditions:
		 * 
		 * Single CONTAINS clause No other conditions in the WHERE clause
		 * EITHER: No "ORDER BY" clause at all, OR just
		 * "ORDER BY SCORE(n) DESCEND" together with the "FIRST_ROWS" hint Only
		 * the first page of results is returned (eg. the first 10 or 20 hits).
		 ****************************************************************************************************/
		...
		if (m_Table_List1.equals("Event") && m_Table_List2.equals("") && m_Table_List3.equals("")) {
			...
			setClause("upper(search_event.combined_all) like '%" + m_keyword_textbox1 + "%' and upper(search_contributor.combined_all) like '%" + m_keyword_textbox2
			...
			setClause("upper(search_event.combined_all) like '%" + m_keyword_textbox1 + "%' and upper(search_organisation.combined_all) like '%" + m_keyword_textbox2
					+ "%' and search_event.eventid = orgevlink.eventid and orgevlink.organisationid = search_organisation.organisationid");
			...
			setClause("upper(search_event.combined_all) like '%" + m_keyword_textbox1 + "%' and upper(search_venue.combined_all) like '%" + m_keyword_textbox2
					+ "%' and search_event.event_venueid = search_venue.venueid");
			l_tables_used = "search_event, search_venue";
		}
		/*
		 * else if(m_Table_List1.equals("Event") &&
		 * m_Table_List2.equals("Publication") && m_Table_List3.equals("")){
		 * setClause("upper(search_event.combined_all) like '%" +
		 * m_keyword_textbox1 +
		 * "%' and upper(search_publication.combined_all) like '%" +
		 * m_keyword_textbox2 +
		 * "%' and search_event.eventid = artevlink.eventid and artevlink.articleid = articles.articleid and articles.public_id = search_publication.publicationid"
		 * ); l_tables_used =
		 * "search_event, search_publication, artevlink, articles"; }
		 */
		else if (m_Table_List1.equals("Event") && m_Table_List2.equals("") && m_Table_List3.equals("Contributor")) {
			setClause("upper(search_event.combined_all) like '%" + m_keyword_textbox1 + "%' and upper(search_contributor.combined_all) like '%" + m_keyword_textbox3
					+ "%' and search_event.eventid = conevlink.eventid and conevlink.contributorid = search_contributor.contributorid");
			l_tables_used = "search_event, conevlink, search_contributor";
		} else if (m_Table_List1.equals("Event") && m_Table_List2.equals("") && m_Table_List3.equals("Organisation")) {
			setClause("upper(search_event.combined_all) like '%" + m_keyword_textbox1 + "%' and upper(search_organisation.combined_all) like '%" + m_keyword_textbox3
					+ "%' and search_event.eventid = orgevlink.eventid and orgevlink.organisationid = search_organisation.organisationid");
			l_tables_used = "search_event, orgevlink, search_organisation";
		} else if (m_Table_List1.equals("Event") && m_Table_List2.equals("") && m_Table_List3.equals("Venue")) {
			setClause("upper(search_event.combined_all) like '%" + m_keyword_textbox1 + "%' and upper(search_venue.combined_all) like '%" + m_keyword_textbox3
					+ "%' and search_event.event_venueid = search_venue.venueid");
			l_tables_used = "search_event, search_venue";
		}
		/*
		 * else if(m_Table_List1.equals("Event") && m_Table_List2.equals("") &&
		 * m_Table_List3.equals("Publication")){
		 * setClause("upper(search_event.combined_all) like '%" +
		 * m_keyword_textbox1 +
		 * "%' and upper(search_publication.combined_all) like '%" +
		 * m_keyword_textbox3 +
		 ...
		 */
			...
			setClause("upper(search_event.combined_all) like '%" + m_keyword_textbox2 + "%' and upper(search_contributor.combined_all) like '%" + m_keyword_textbox3
					+ "%' and search_event.eventid = conevlink.eventid and conevlink.contributorid = search_contributor.contributorid");
			l_tables_used = "search_event, conevlink, search_contributor";
		} else if (m_Table_List1.equals("") && m_Table_List2.equals("Event") && m_Table_List3.equals("Organisation")) {
			setClause("upper(search_event.combined_all) like '%" + m_keyword_textbox2 + "%' and upper(search_organisation.combined_all) like '%" + m_keyword_textbox3
					+ "%' and search_event.eventid = orgevlink.eventid and orgevlink.organisationid = search_organisation.organisationid");
			l_tables_used = "search_event, orgevlink, search_organisation";
		} else if (m_Table_List1.equals("") && m_Table_List2.equals("Event") && m_Table_List3.equals("Venue")) {
			setClause("upper(search_event.combined_all) like '%" + m_keyword_textbox2 + "%' and upper(search_venue.combined_all) like '%" + m_keyword_textbox3
					+ "% and search_event.event_venueid = search_venue.venueid");
			l_tables_used = "search_event, search_venue";
		}/*
		 * else if(m_Table_List1.equals("") && m_Table_List2.equals("Event") &&
		 * m_Table_List3.equals("Publication")){
		 * setClause("upper(search_event.combined_all) like '%" +
		 * m_keyword_textbox2 +
		 * "%' and upper(search_publication.combined_all) like '%" +
		 ...
		 */
			...
			setClause("upper(search_event.combined_all) like '%"
					+ m_keyword_textbox1
					+ "%' and upper(search_contributor.combined_all) like '%"
					+ m_keyword_textbox2
					+ "%' and upper(search_organisation.combined_all) like '%"
					+ m_keyword_textbox3
					+ "%' and search_event.eventid = conevlink.eventid and conevlink.contributorid = search_contributor.contributorid and search_event.eventid = orgevlink.eventid and orgevlink.organisationid = search_organisation.organisationid");
			...
			setClause("upper(search_event.combined_all) like '%"
					...
					+ "%' and upper(search_contributor.combined_all) like '%"
					+ m_keyword_textbox2
					+ "%' and upper(search_venue.combined_all) like '%"
					+ m_keyword_textbox3
					+ "%' and search_event.event_venueid = search_venue.venueid and search_event.eventid = conevlink.eventid and conevlink.contributorid = search_contributor.contributorid");
		 ...
		 * else if(m_Table_List1.equals("Event") &&
		 * m_Table_List2.equals("Contributor") &&
		 * m_Table_List3.equals("Publication")){
		 * setClause("upper(search_event.combined_all) like '%" +
		 * m_keyword_textbox1 +
		 * "%' and upper(search_contributor.combined_all) like '%" +
		 ...
		 * "%' and upper(search_publication.combined_all) like '%" +
		 * m_keyword_textbox3 +
		 * "%' and search_event.eventid = conevlink.eventid and conevlink.contributorid = search_contributor.contributorid and search_event.eventid = artevlink.eventid and"
		 * +
		 * " artevlink.articleid = articles.articleid and articles.public_id = search_publication.publicationid"
		 * ); l_tables_used =
		 * "search_event, search_contributor, search_publication, conevlink, artevlink, articles"
		 * ; } else if(m_Table_List1.equals("Event") &&
		 * m_Table_List2.equals("Publication") &&
		 * m_Table_List3.equals("Contributor")){
		 * setClause("upper(search_event.combined_all) like '%" +
		 * m_keyword_textbox1 +
		 * "%' and upper(search_publication.combined_all) like '%" +
		 * m_keyword_textbox2 +
		 ...
		 */
			...
			setClause("upper(search_event.combined_all) like '%"
					...
					+ "%' and upper(search_venue.combined_all) like '%"
					+ m_keyword_textbox3
					+ "%' and search_event.eventid = orgevlink.eventid and orgevlink.organisationid = search_organisation.organisationid and search_event.event_venueid = search_venue.venueid");
		 ...
		 * else if(m_Table_List1.equals("Event") &&
		 * m_Table_List2.equals("Organisation") &&
		 * m_Table_List3.equals("Publication")){
		 * setClause("upper(search_event.combined_all) like '%" +
		 * m_keyword_textbox1 +
		 * "%' and upper(search_organisation.combined_all) like '%" +
		 ...
		 * "%' and upper(search_publication.combined_all) like '%" +
		 * m_keyword_textbox3 +
		 * "%' and search_event.eventid = orgevlink.eventid and orgevlink.organisationid = search_organisation.organisationid and search_event.eventid = artevlink.eventid and artevlink.articleid = articles.articleid and articles.public_id = search_publication.publicationid"
		 * ); l_tables_used =
		 * "search_event, search_organisation, orgevlink, artevlink, articles, search_publication"
		 * ; } else if(m_Table_List1.equals("Event") &&
		 * m_Table_List2.equals("Publication") &&
		 * m_Table_List3.equals("Organisation")){
		 * setClause("upper(search_event.combined_all) like '%" +
		 * m_keyword_textbox1 +
		 * "%' and upper(search_publication.combined_all) like '%" +
		 * m_keyword_textbox2 +
		 ...
		 * "%' and upper(search_publication.combined_all) like '%" +
		 * m_keyword_textbox3 +
		 * "%' and search_event.event_venueid = search_venue.venueid and search_event.eventid = artevlink.eventid and artevlink.articleid = articles.articleid and articles.public_id = search_publication.publicationid"
		 * ); l_tables_used =
		 * "search_event, artevlink, articles, search_publication, search_venue"
		 * ; } else if(m_Table_List1.equals("Event") &&
		 * m_Table_List2.equals("Publication") &&
		 * m_Table_List3.equals("Venue")){
		 * setClause("upper(search_event.combined_all) like '%" +
		 * m_keyword_textbox1 +
		 * "%' and upper(search_publication.combined_all) like '%" +
		 * m_keyword_textbox2 +
		 ...
		 ********************************************************/
			...
			setClause("upper(search_contributor.combined_all) like '%" + m_keyword_textbox1 + "%' and upper(search_event.combined_all) like '%" + m_keyword_textbox2
					+ "%' and search_contributor.contributorid  = conevlink.contributorid  and conevlink.eventid = search_event.eventid order by last_name, first_name asc");
			...
			setClause("upper(search_contributor.combined_all) like '%"
					...
					+ "%' and upper(search_venue.combined_all) like '%"
					+ m_keyword_textbox2
					+ "%' and search_contributor.contributorid  = conevlink.contributorid  and conevlink.eventid = search_event.eventid and search_event.event_venueid = search_venue.venueid order by last_name, first_name asc");
			l_tables_used = "search_contributor, search_event, conevlink, search_venue";
		}
		/*
		 * else if(m_Table_List1.equals("Contributor") &&
		 * m_Table_List2.equals("Publication") && m_Table_List3.equals("")){
		 * setClause("upper(search_contributor.combined_all) like '%" +
		 * m_keyword_textbox1 +
		 * "%' and upper(search_publication.combined_all) like '%" +
		 * m_keyword_textbox2 +
		 * "%' and search_contributor.contributorid  = conevlink.contributorid  and conevlink.eventid = search_event.eventid and search_event.eventid = artevlink.eventid and artevlink.articleid = articles.articleid and articles.public_id = search_publication.publicationid order by last_name, first_name asc"
		 * ); l_tables_used =
		 * "search_contributor, search_event, conevlink, artevlink, articles, search_publication "
		 * ; }
		 */
		else if (m_Table_List1.equals("Contributor") && m_Table_List2.equals("") && m_Table_List3.equals("Event")) {
			setClause("upper(search_contributor.combined_all) like '%" + m_keyword_textbox1 + "%' and upper(search_event.combined_all) like '%" + m_keyword_textbox3
					+ "%' and search_contributor.contributorid  = conevlink.contributorid  and conevlink.eventid = search_event.eventid order by last_name, first_name asc");
			l_tables_used = "search_contributor, search_event, conevlink";
		 ...
		 * else if(m_Table_List1.equals("Contributor") &&
		 * m_Table_List2.equals("") && m_Table_List3.equals("Publication")){
		 * setClause("upper(search_contributor.combined_all) like '%" +
		 * m_keyword_textbox1 +
		 * "%' and upper(search_publication.combined_all) like '%" +
		 * m_keyword_textbox3 +
		 ...
		 */
			...
			setClause("upper(search_contributor.combined_all) like '%" + m_keyword_textbox2 + "%' and upper(search_event.combined_all) like '%" + m_keyword_textbox3
					+ "%' and search_contributor.contributorid  = conevlink.contributorid  and conevlink.eventid = search_event.eventid order by last_name, first_name asc");
			l_tables_used = "search_contributor, search_event, conevlink";
		 ...
		 */
			...
			setClause("upper(search_contributor.combined_all) like '%"
					+ m_keyword_textbox1
					+ "%' and upper(search_event.combined_all) like '%"
					+ m_keyword_textbox2
					+ "%' and upper(search_organisation.combined_all) like '%"
					+ m_keyword_textbox3
					+ "%' and search_contributor.contributorid  = conevlink.contributorid  and conevlink.eventid = search_event.eventid and search_event.eventid = orgevlink.eventid and orgevlink.organisationid = search_organisation.organisationid order by last_name, first_name asc");
		 ...
		 * else if(m_Table_List1.equals("Contributor") &&
		 * m_Table_List2.equals("Event") &&
		 * m_Table_List3.equals("Publication")){ String clause =
		 * " artevlink.articleid = articles.articleid and articles.public_id = search_publication.publicationid order by last_name, first_name asc"
		 * ; setClause("upper(search_contributor.combined_all) like '%" +
		 * m_keyword_textbox1 +
		 * "%' and upper(search_event.combined_all) like '%" +
		 * m_keyword_textbox2 +
		 * "%' and upper(search_publication.combined_all) like '%" +
		 * m_keyword_textbox3 +
		 * "%' and search_contributor.contributorid  = conevlink.contributorid  and conevlink.eventid = search_event.eventid and search_event.eventid = artevlink.eventid and"
		 * + clause); l_tables_used =
		 * "search_contributor, search_event, conevlink, artevlink, articles, search_publication"
		 * ; } else if(m_Table_List1.equals("Contributor") &&
		 * m_Table_List2.equals("Publication") &&
		 * m_Table_List3.equals("Event")){ String clause =
		 * " artevlink.articleid = articles.articleid and articles.public_id = search_publication.publicationid order by last_name, first_name asc"
		 * ; setClause("upper(search_contributor.combined_all) like '%" +
		 * m_keyword_textbox1 +
		 * "%' and upper(search_publication.combined_all) like '%" +
		 ...
		 */
			...
			String clause = "upper(search_contributor.combined_all) like '%" + m_keyword_textbox1 + "%' and upper(search_organisation.combined_all) like '%" + m_keyword_textbox2
					+ "%' and upper(search_venue.combined_all) like '%" + m_keyword_textbox3 + "%'";
			...
			String clause = "upper(search_contributor.combined_all) like '%" + m_keyword_textbox1 + "%' and upper(search_venue.combined_all) like '%" + m_keyword_textbox2
					+ "%' and upper(search_organisation.combined_all) like '%" + m_keyword_textbox3 + "%'";
			String clause1 = " order by last_name, first_name asc";
			setClause(clause
					+ "  and search_contributor.contributorid  = contfunctlink.contributorid and contfunctlink.contributorfunctpreferredid = contfunct.contributorfunctpreferredid and contfunct.contributorfunctpreferredid = orgevlink.artistic_function and orgevlink.organisationid = search_organisation.organisationid and search_contributor.contributorid  = conevlink.contributorid  and conevlink.eventid = search_event.eventid and search_event.event_venueid = search_venue.venueid"
					+ clause1);
		 ...
		 * m_Table_List2.equals("Organisation") &&
		 * m_Table_List3.equals("Publication")){ String clause =
		 * "upper(search_contributor.combined_all) like '%" + m_keyword_textbox1
		 * + "%' and upper(search_organisation.combined_all) like '%" +
		 * m_keyword_textbox2 +
		 * "%' and upper(search_publication.combined_all) like '%" +
		 * m_keyword_textbox3 + "%'"; String clause2 =
		 * " and search_contributor.contributorid  = conevlink.contributorid  and conevlink.eventid = search_event.eventid and search_event.eventid = artevlink.eventid and artevlink.articleid = articles.articleid and articles.public_id = search_publication.publicationid order by last_name, first_name asc"
		 * ; setClause(clause +
		 * " and search_contributor.contributorid  = contfunctlink.contributorid  and contfunctlink.contributorfunctpreferredid = contfunct.contributorfunctpreferredid and contfunct.contributorfunctpreferredid = orgevlink.artistic_function and orgevlink.organisationid = search_organisation.organisationid"
		 * + clause2 ); l_tables_used =
		 * "search_contributor, orgevlink, search_organisation, search_venue, artevlink, articles, search_publication, search_event, conevlink, contfunct, contfunctlink"
		 * ; } else if(m_Table_List1.equals("Contributor") &&
		 * m_Table_List2.equals("Publication") &&
		 * m_Table_List3.equals("Organisation")){ String clause =
		 * "upper(search_contributor.combined_all) like '%" + m_keyword_textbox1
		 * + "%' and upper(search_publication.combined_all) like '%" +
		 * m_keyword_textbox2 +
		 * "%' and upper(search_organisation.combined_all) like '%" +
		 ...
		 * m_keyword_textbox2 +
		 * "%' and upper(search_publication.combined_all) like '%" +
		 * m_keyword_textbox3 + "%'"; setClause(clause +
		 ...
		 * ); l_tables_used =
		 * "search_contributor, conevlink, search_event, search_venue, artevlink, articles, search_publication"
		 * ; } else if(m_Table_List1.equals("Contributor") &&
		 * m_Table_List2.equals("Publication") &&
		 * m_Table_List3.equals("Venue")){ String clause =
		 * "upper(search_contributor.combined_all) like '%" + m_keyword_textbox1
		 * + "%' and upper(search_publication.combined_all) like '%" +
		 * m_keyword_textbox2 +
		 * "%' and upper(search_venue.combined_all) like '%" +
		 ...
		 ********************************************************/
			...
			setClause("upper(search_organisation.combined_all) like '%" + m_keyword_textbox1 + "%' and upper(search_event.combined_all) like '%" + m_keyword_textbox2
					+ "%' and search_event.eventid = orgevlink.eventid and orgevlink.organisationid = search_organisation.organisationid order by name asc");
			...
			setClause("upper(search_organisation.combined_all) like '%"
					+ m_keyword_textbox1
					+ "%' and upper(search_organisation.combined_all) like '%"
					+ m_keyword_textbox2
					+ "%' and search_contributor.contributorid  = contfunctlink.contributorid  and contfunctlink.CONTRIBUTORFUNCTPREFERREDID = contfunct.CONTRIBUTORFUNCTPREFERREDID and contfunct.CONTRIBUTORFUNCTPREFERREDID = orgevlink.artistic_function and orgevlink.organisationid = search_organisation.organisationid order by name asc");
			...
			setClause("upper(search_organisation.combined_all) like '%"
					...
					+ "%' and upper(search_venue.combined_all) like '%"
					+ m_keyword_textbox2
					+ "%' and search_venue.venueid = search_event.event_venueid and search_event.eventid = orgevlink.eventid and orgevlink.organisationid = search_organisation.organisationid order by name asc");
			l_tables_used = "search_organisation, search_event, orgevlink, search_venue";
		}/*
		 * else if(m_Table_List1.equals("Organisation") &&
		 * m_Table_List2.equals("Publication") && m_Table_List3.equals("")){
		 * setClause("upper(search_organisation.combined_all) like '%" +
		 * m_keyword_textbox1 +
		 * "%' and upper(search_publication.combined_all) like '%" +
		 * m_keyword_textbox2 +
		 * "%' and search_organisation.organisationid = orgevlink.organisationid and orgevlink.eventid = search_event.eventid order by name asc"
		 * +
		 ...
		 * ); l_tables_used =
		 * "search_organisation, search_event, search_publication, orgevlink, articles, artevlink"
		 * ; }
		 */
		else if (m_Table_List1.equals("Organisation") && m_Table_List2.equals("") && m_Table_List3.equals("Event")) {
			setClause("upper(search_organisation.combined_all) like '%" + m_keyword_textbox1 + "%' and upper(search_event.combined_all) like '%" + m_keyword_textbox3
					+ "%' and search_event.eventid = orgevlink.eventid and orgevlink.organisationid = search_organisation.organisationid order by name asc");
			l_tables_used = "search_organisation, search_event, orgevlink";
		 ...
		 */
			...
			setClause("upper(search_organisation.combined_all) like '%" + m_keyword_textbox2 + "%' and upper(search_event.combined_all) like '%" + m_keyword_textbox3
					+ "%' and search_event.eventid = orgevlink.eventid and orgevlink.organisationid = search_organisation.organisationid order by name asc");
			l_tables_used = "search_organisation, search_event, orgevlink";
		 ...
		 * "%' and upper(search_publication.combined_all) like '%" +
		 * m_keyword_textbox3 +
		 * "%' and search_organisation.organisationid = orgevlink.organisationid and orgevlink.eventid = search_event.eventid"
		 * +
		 ...
		 */
			...
			setClause("upper(search_organisation.combined_all) like '%" + m_keyword_textbox1 + "%' and upper(search_event.combined_all) like '%" + m_keyword_textbox2
					+ "%' and upper(search_contributor.combined_all) like '%" + m_keyword_textbox3
					+ "%' and search_organisation.organisationid = orgevlink.organisationid and orgevlink.eventid = search_event.eventid"
			...
			setClause("upper(search_organisation.combined_all) like '%" + m_keyword_textbox1 + "%' and upper(search_contributor.combined_all) like '%" + m_keyword_textbox2
					+ "%' and upper(search_event.combined_all) like '%" + m_keyword_textbox3
					+ "%' and search_organisation.organisationid = orgevlink.organisationid and orgevlink.eventid = search_event.eventid"
					+ " and search_event.eventid = conevlink.eventid and conevlink.contributorid = search_contributor.contributorid order by name asc");
			l_tables_used = "search_organisation, search_event, orgevlink, conevlink, search_contributor";
			...
			setClause("upper(search_organisation.combined_all) like '%" + m_keyword_textbox1 + "%' and upper(search_venue.combined_all) like '%" + m_keyword_textbox2
					+ "%' and upper(search_event.combined_all) like '%" + m_keyword_textbox3
					+ "%' and search_organisation.organisationid = orgevlink.organisationid and orgevlink.eventid = search_event.eventid"
					+ " and search_event.event_venueid = search_venue.venueid order by name asc");
			l_tables_used = "search_organisation, search_event, orgevlink, search_venue";
		 ...
		 * " and search_event.eventid = artevlink.eventid and artevlink.articleid = articles.articleid and articles.public_id = search_publication.publicationid order by name asc"
		 * ); l_tables_used =
		 * "search_organisation, search_event, orgevlink, search_publication, articles, artevlink"
		 * ; } else if(m_Table_List1.equals("Organisation") &&
		 * m_Table_List2.equals("Publication") &&
		 * m_Table_List3.equals("Event")){
		 * setClause("upper(search_organisation.combined_all) like '%" +
		 * m_keyword_textbox1 +
		 * "%' and upper(search_publication.combined_all) like '%" +
		 * m_keyword_textbox2 +
		 ...
		 */
			...
			setClause("upper(search_organisation.combined_all) like '%"
					...
					+ "%' and upper(search_venue.combined_all) like '%"
					+ m_keyword_textbox3
					+ "%' and search_contributor.contributorid  = contfunctlink.contributorid  and contfunctlink.CONTRIBUTORFUNCTPREFERREDID = contfunct.CONTRIBUTORFUNCTPREFERREDID"
		 ...
		 * " and search_event.eventid = artevlink.eventid and artevlink.articleid = articles.articleid and articles.public_id = search_publication.publicationid order by name asc"
		 * ); l_tables_used =
		 * "search_organisation, search_event, orgevlink, conevlink, contfunctlink, contfunct, search_contributor, articles, artevlink, search_publication"
		 * ; } else if(m_Table_List1.equals("Organisation") &&
		 * m_Table_List2.equals("Publication") &&
		 * m_Table_List3.equals("Contributor")){
		 * setClause("upper(search_organisation.combined_all) like '%" +
		 ...
		 * ); l_tables_used =
		 * "search_organisation, search_event, orgevlink, search_publication, articles, artevlink, search_venue"
		 * ; } else if(m_Table_List1.equals("Organisation") &&
		 * m_Table_List2.equals("Publication") &&
		 * m_Table_List3.equals("Venue")){
		 * setClause("upper(search_organisation.combined_all) like '%" +
		 ...
		 ********************************************************/
			...
			setClause("upper(search_venue.combined_all) like '%" + m_keyword_textbox1 + "%' and upper(search_event.combined_all) like '%" + m_keyword_textbox2
					+ "%' and search_event.event_venueid = search_venue.venueid order by venue_name asc");
			l_tables_used = "search_event, search_venue";
		} else if (m_Table_List1.equals("Venue") && m_Table_List2.equals("Contributor") && m_Table_List3.equals("")) {
			setClause("upper(search_venue.combined_all) like '%"
					+ m_keyword_textbox1
					+ "%' and upper(search_contributor.combined_all) like '%"
					+ m_keyword_textbox2
					+ "%' and search_venue.venueid = search_event.event_venueid and search_event.eventid = conevlink.eventid and conevlink.contributorid = search_contributor.contributorid order by venue_name asc");
			...
			setClause("upper(search_venue.combined_all) like '%"
					...
					+ "%' and upper(search_organisation.combined_all) like '%"
					+ m_keyword_textbox2
					+ "%' and search_venue.venueid = search_event.event_venueid and search_event.eventid = orgevlink.eventid and orgevlink.organisationid = search_organisation.organisationid order by venue_name asc");
			l_tables_used = "search_event, search_venue, search_organisation, orgevlink";
		}/*
		 * else if(m_Table_List1.equals("Venue") &&
		 * m_Table_List2.equals("Publication") && m_Table_List3.equals("")){
		 * setClause("upper(search_venue.combined_all) like '%" +
		 * m_keyword_textbox1 +
		 * "%' and upper(search_publication.combined_all) like '%" +
		 * m_keyword_textbox2 +
		 * "%' and search_venue.venueid = search_event.event_venueid and search_event.eventid = artevlink.eventid and artevlink.articleid = articles.articleid and articles.public_id = search_publication.publicationid order by venue_name asc"
		 * ); l_tables_used =
		 * "search_event, search_venue, search_publication, articles, artevlink"
		 * ; }
		 */
		else if (m_Table_List1.equals("Venue") && m_Table_List2.equals("") && m_Table_List3.equals("Event")) {
			setClause("upper(search_venue.combined_all) like '%" + m_keyword_textbox1 + "%' and upper(search_event.combined_all) like '%" + m_keyword_textbox3
					+ "%' and search_event.event_venueid = search_venue.venueid order by venue_name asc");
			l_tables_used = "search_event, search_venue";
		 ...
		 * else if(m_Table_List1.equals("Venue") && m_Table_List2.equals("") &&
		 * m_Table_List3.equals("Publication")){
		 * setClause("upper(search_venue.combined_all) like '%" +
		 * m_keyword_textbox1 +
		 * "%' and upper(search_publication.combined_all) like '%" +
		 ...
		 */
			...
			setClause("upper(search_venue.combined_all) like '%" + m_keyword_textbox2 + "%' and upper(search_event.combined_all) like '%" + m_keyword_textbox3
					+ "%' and search_event.event_venueid = search_venue.venueid order by venue_name asc");
			l_tables_used = "search_event, search_venue";
		 ...
		 * else if(m_Table_List1.equals("") && m_Table_List2.equals("Venue") &&
		 * m_Table_List3.equals("Publication")){
		 * setClause("upper(search_venue.combined_all) like '%" +
		 * m_keyword_textbox2 +
		 * "%' and upper(search_publication.combined_all) like '%" +
		 ...
		 */
			...
			setClause("upper(search_venue.combined_all) like '%"
					+ m_keyword_textbox1
					+ "',0)>0 and upper(search_organisation.combined_all) like '%"
					+ m_keyword_textbox2
					+ "%' and upper(search_event.combined_all) like '%"
					+ m_keyword_textbox3
					+ "%' and search_venue.venueid = search_event.event_venueid and search_event.eventid = orgevlink.eventid and orgevlink.organisationid = search_organisation.organisationid order by venue_name asc");
		 ...
		 * "%' and search_venue.venueid = search_event.event_venueid and search_event.eventid = artevlink.eventid and artevlink.articleid = articles.articleid and articles.public_id = search_publication.publicationid order by venue_name asc"
		 * ); l_tables_used =
		 * "search_event, search_venue, search_publication, artevlink, articles"
		 * ; } else if(m_Table_List1.equals("Venue") &&
		 * m_Table_List2.equals("Publication") &&
		 * m_Table_List3.equals("Event")){
		 * setClause("upper(search_venue.combined_all) like '%" +
		 ...
		 * "%' and upper(search_publication.combined_all) like '%" +
		 * m_keyword_textbox3 +
		 * "%' and search_venue.venueid = search_event.event_venueid and search_event.eventid = conevlink.eventid and conevlink.contributorid = search_contributor.contributorid and"
		 * +
		 ...
		 * ); l_tables_used =
		 * "search_event, search_venue, search_publication, search_contributor, conevlink, artevlink, articles"
		 * ; } else if(m_Table_List1.equals("Venue") &&
		 * m_Table_List2.equals("Publication") &&
		 * m_Table_List3.equals("Contributor")){
		 * setClause("upper(search_venue.combined_all) like '%" +
		 ...
		 * "%' and upper(search_publication.combined_all) like '%" +
		 * m_keyword_textbox3 +
		 * "%' and search_venue.venueid = search_event.event_venueid and search_event.eventid = orgevlink.eventid and orgevlink.organisationid = search_organisation.organisationid and"
		 * +
		 * " search_event.eventid = artevlink.eventid and artevlink.articleid = articles.articleid and articles.public_id = search_publication.publicationid order by venue_name asc"
		 * ); l_tables_used =
		 * "search_event, search_venue, search_publication, search_organisation, orgevlink, articles, artevlink"
		 * ; } else if(m_Table_List1.equals("Venue") &&
		 * m_Table_List2.equals("Publication") &&
		 * m_Table_List3.equals("Organisation")){
		 * setClause("upper(search_venue.combined_all) like '%" +
		 ...
		 ********************************************************/
		/*
		 * else if(m_Table_List1.equals("Publication") &&
		 * m_Table_List2.equals("") && m_Table_List3.equals("")){
		 * setClause("contains(search_publication.combined_all, '" +
		 * m_keyword_textbox1 + "',0)>0 order by publication_name asc");
		 * l_tables_used = "search_publication"; } else
		 * if(m_Table_List1.equals("") && m_Table_List2.equals("Publication") &&
		 * m_Table_List3.equals("")){
		 * setClause("contains(search_publication.combined_all, '" +
		 * m_keyword_textbox2 + "',0)>0 order by publication_name asc");
		 * l_tables_used = "search_publication"; } else
		 * if(m_Table_List1.equals("") && m_Table_List2.equals("") &&
		 * m_Table_List3.equals("Publication")){
		 * setClause("contains(search_publication.combined_all, '" +
		 * m_keyword_textbox3 + "',0)>0 order by publication_name asc");
		 * l_tables_used = "search_publication"; } else
		 * if(m_Table_List1.equals("Publication") &&
		 * m_Table_List2.equals("Event") && m_Table_List3.equals("")){
		 * setClause("upper(search_publication.combined_all) like '%" +
		 * m_keyword_textbox1 +
		 * "%' and upper(search_event.combined_all) like '%" +
		 * m_keyword_textbox2 +
		 * "%' and search_event.eventid = artevlink.eventid and artevlink.articleid = articles.articleid and articles.public_id = search_publication.publicationid order by publication_name asc"
		 * ); l_tables_used =
		 * "search_publication, search_event, artevlink, articles"; } else
		 * if(m_Table_List1.equals("Publication") &&
		 * m_Table_List2.equals("Contributor") && m_Table_List3.equals("")){
		 * setClause("upper(search_publication.combined_all) like '%" +
		 * m_keyword_textbox1 +
		 * "%' and upper(search_contributor.combined_all) like '%" +
		 * m_keyword_textbox2 +
		 * "%' and search_contributor.contributorid = conevlink.contributorid and conevlink.eventid = search_event.eventid and search_event.eventid = artevlink.eventid and artevlink.articleid = articles.articleid and articles.public_id = search_publication.publicationid order by publication_name asc"
		 * ); l_tables_used =
		 * "search_publication, search_event, artevlink, articles, conevlink, search_contributor"
		 * ; } else if(m_Table_List1.equals("Publication") &&
		 * m_Table_List2.equals("Organisation") && m_Table_List3.equals("")){
		 * setClause("upper(search_publication.combined_all) like '%" +
		 * m_keyword_textbox1 +
		 * "%' and upper(search_organisation.combined_all) like '%" +
		 * m_keyword_textbox2 +
		 * "%' and search_organisation.organisationid = orgevlink.organisationid and orgevlink.eventid = search_event.eventid and search_event.eventid = artevlink.eventid and artevlink.articleid = articles.articleid and articles.public_id = search_publication.publicationid order by publication_name asc"
		 * ); l_tables_used =
		 * "search_publication, search_event, artevlink, articles, orgevlink, search_organisation"
		 * ; } else if(m_Table_List1.equals("Publication") &&
		 * m_Table_List2.equals("Venue") && m_Table_List3.equals("")){
		 * setClause("upper(search_publication.combined_all) like '%" +
		 * m_keyword_textbox1 +
		 * "%' and upper(search_venue.combined_all) like '%" +
		 * m_keyword_textbox2 +
		 * "%' and search_venue.venueid = search_event.event_venueid and search_event.eventid = artevlink.eventid and artevlink.articleid = articles.articleid and articles.public_id = search_publication.publicationid order by publication_name asc"
		 * ); l_tables_used =
		 * "search_publication, search_event, search_venue, artevlink, articles"
		 * ; } else if(m_Table_List1.equals("Publication") &&
		 * m_Table_List2.equals("") && m_Table_List3.equals("Event")){
		 * setClause("upper(search_publication.combined_all) like '%" +
		 * m_keyword_textbox1 +
		 * "%' and upper(search_event.combined_all) like '%" +
		 * m_keyword_textbox2 +
		 ...
		 * ); l_tables_used =
		 * "search_publication, search_event, artevlink, articles"; } else
		 * if(m_Table_List1.equals("Publication") && m_Table_List2.equals("") &&
		 * m_Table_List3.equals("Contributor")){
		 * setClause("upper(search_publication.combined_all) like '%" +
		 * m_keyword_textbox1 +
		 * "%' and upper(search_contributor.combined_all) like '%" +
		 ...
		 * "search_publication, search_event, artevlink, articles, conevlink, search_contributor"
		 * ; } else if(m_Table_List1.equals("Publication") &&
		 * m_Table_List2.equals("") && m_Table_List3.equals("Organisation")){
		 * setClause("upper(search_publication.combined_all) like '%" +
		 * m_keyword_textbox1 +
		 * "%' and upper(search_organisation.combined_all) like '%" +
		 * m_keyword_textbox3 +
		 ...
		 * "search_publication, search_event, artevlink, articles, orgevlink, search_organisation"
		 * ; } else if(m_Table_List1.equals("Publication") &&
		 * m_Table_List2.equals("") && m_Table_List3.equals("Venue")){
		 * setClause("upper(search_publication.combined_all) like '%" +
		 * m_keyword_textbox1 +
		 * "%' and upper(search_venue.combined_all) like '%" +
		 * m_keyword_textbox3 +
		 ...
		 * "%' and upper(search_contributor.combined_all) like '%" +
		 * m_keyword_textbox3 +
		 * "%' and search_publication.publicationid = articles.public_id and articles.articleid = articles.articleid and articles.articleid = artevlink.articleid and artevlink.eventid = search_event.eventid and"
		 * +
		 ...
		 * ); l_tables_used =
		 * "search_publication, search_event, search_venue, artevlink, articles conevlink, search_contributor"
		 * ; } else if(m_Table_List1.equals("Publication") &&
		 * m_Table_List2.equals("Contributor") &&
		 * m_Table_List3.equals("Event")){
		 * setClause("upper(search_publication.combined_all) like '%" +
		 ...
		 * ); l_tables_used =
		 * "search_publication, search_event, search_venue, artevlink, articles orgevlink, search_organisation"
		 * ; } else if(m_Table_List1.equals("Publication") &&
		 * m_Table_List2.equals("Organisation") &&
		 * m_Table_List3.equals("Event")){
		 * setClause("upper(search_publication.combined_all) like '%" +
		 ...
		 * "search_publication, search_event, search_venue, artevlink, articles orgevlink, search_organisation"
		 * ; } else if(m_Table_List1.equals("Publication") &&
		 * m_Table_List2.equals("Event") && m_Table_List3.equals("Venue")){
		 * setClause("upper(search_publication.combined_all) like '%" +
		 * m_keyword_textbox1 +
		 * "%' and upper(search_event.combined_all) like '%" +
		 * m_keyword_textbox2 +
		 ...
		 * "search_publication, search_event, search_venue, artevlink, articles"
		 * ; } else if(m_Table_List1.equals("Publication") &&
		 * m_Table_List2.equals("Venue") && m_Table_List3.equals("Event")){
		 * setClause("upper(search_publication.combined_all) like '%" +
		 * m_keyword_textbox1 +
		 * "%' and upper(search_venue.combined_all) like '%" +
		 * m_keyword_textbox2 +
		 ...
		 * ); l_tables_used =
		 * "search_publication, search_event, artevlink, articles, conevlink, search_contributor, orgevlink, search_organisation"
		 * ; } else if(m_Table_List1.equals("Publication") &&
		 * m_Table_List2.equals("Organisation") &&
		 * m_Table_List3.equals("Contributor")){
		 * setClause("upper(search_publication.combined_all) like '%" +
		 ...
		 * ); l_tables_used =
		 * "search_publication, search_event, artevlink, articles, conevlink, search_contributor, search_venue"
		 * ; } else if(m_Table_List1.equals("Publication") &&
		 * m_Table_List2.equals("Venue") &&
		 * m_Table_List3.equals("Contributor")){
		 * setClause("upper(search_publication.combined_all) like '%" +
		 ...
		 * ); l_tables_used =
		 * "search_publication, search_event, artevlink, articles, orgevlink, search_organisation, search_venue"
		 * ; } else if(m_Table_List1.equals("Publication") &&
		 * m_Table_List2.equals("Venue") &&
		 * m_Table_List3.equals("Organisation")){
		 * setClause("upper(search_publication.combined_all) like '%" +
	...
	private void setDateClause() {
		...
		} else if (m_Between1_dd.equals("") && m_Between1_mm.equals("") && m_Between1_yyyy.equals("") && m_Between2_dd.equals("") && m_Between2_mm.equals("")
				&& m_Between2_yyyy.equals("")) {
			m_dateClause = " order by first_date desc";
		}
		...
		} else if ((m_Between1_yyyy != null && !m_Between1_yyyy.equals("")) && (m_Between2_yyyy != null && !m_Between2_yyyy.equals(""))) {
