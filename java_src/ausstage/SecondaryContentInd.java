...
public class SecondaryContentInd extends AusstageInputOutput
{
  ...
  */
  ...
  Purpose: Sets the class to a contain the Secondary Content Indicator information for the
           specified Secondary Content Indicator id.

  Parameters:
    p_id : id of the Secondary Content Indicator record
  ...
  */
  public boolean add ()
  {
    try
    {
      ...
      if (validateObjectForDB ())
      {
        ...
        if(!isDuplicate()){
          sqlString = "INSERT INTO SECCONTENTINDICATOR " + 
                      "(SECCONTENTINDICATOR, SECCONTENTINDICATORDESCRIPTION, SECCONTENTINDICATORPREFERREDID) " + 
                      "VALUES (" +
  ...
  private boolean isDuplicate(){
    ...
    try{      
      ...
      Statement stmt = m_db.m_conn.createStatement ();
      String    sqlString;
      sqlString = "select SECCONTENTINDICATOR from SECCONTENTINDICATOR where " +
                  "lower(SECCONTENTINDICATOR)='" + m_db.plSqlSafeString(m_name).toLowerCase() + "'";
                  if(m_id != 0) 
                    sqlString += " and not SECCONTENTINDICATORID=" + m_id;
      l_rs = m_db.runSQL (sqlString, stmt);
    ...
    try
    {
      ...
      if (validateObjectForDB ())
      {
        if(!isDuplicate()){
          ...
          if(m_preferred_id != 0){
            sqlString = "UPDATE SECCONTENTINDICATOR " + 
                        "SET SECCONTENTINDICATOR = '" + m_db.plSqlSafeString(m_name) + "', " +
                        "SECCONTENTINDICATORDESCRIPTION = '" + m_db.plSqlSafeString(m_description) + "', " +
                        "SECCONTENTINDICATORPREFERREDID = " + m_preferred_id + " " +
                        "WHERE SECCONTENTINDICATORID = " + m_id;
            ...
            sqlString = "Insert into SECCONTENTINDICATORPREFERRED " +
            ...
            sqlString  = "UPDATE SECCONTENTINDICATOR " + 
                         "SET SECCONTENTINDICATORPREFERREDID = " + m_preferred_id + " " +
                         "WHERE SECCONTENTINDICATORID = " + m_id;
  ...
  */
  public boolean delete ()
  {
    ...
    try
    {
      ...
      if(!isInUse()){
        sqlString = "SELECT SECCONTENTINDICATORPREFERREDID " +
                    "FROM SECCONTENTINDICATOR WHERE SECCONTENTINDICATORID = " + m_id;
        l_rs = m_db.runSQL (sqlString, stmt);
        ...
        if(l_rs.next()){
          ...
          sqlString = "SELECT count (*) as counter " +
                      "FROM SECCONTENTINDICATOR WHERE " +
                      "SECCONTENTINDICATORPREFERREDID = " +
                      seccontindprefid;
          l_rs = m_db.runSQL (sqlString, stmt);
          
          if(l_rs.next() && l_rs.getInt("counter") == 1){
            // del the preferred term
            sqlString = "DELETE FROM SECCONTENTINDICATORPREFERRED " +
                        "WHERE SECCONTENTINDICATORPREFERREDID = " +
                        seccontindprefid;
  ...
  public boolean isInUse(){
    ...
    try{
      Statement stmt = m_db.m_conn.createStatement ();
      // lets check if a preferred term exist in this secondary content indicator
      sqlString = "select SECCONTENTINDICATORPREFERRED.SECCONTENTINDICATORPREFERREDID from " +
                  "SECCONTENTINDICATORPREFERRED, SECCONTENTINDICATOR " +
                  "where SECCONTENTINDICATOR.SECCONTENTINDICATORID = " + m_id + " " +
                  "and SECCONTENTINDICATOR.SECCONTENTINDICATORPREFERREDID=" +
      ...
      if(l_rs.next()){
        ...
        sqlString = "select count (*) as counter from SECCONTENTINDICATOR " +
                    "where SECCONTENTINDICATORPREFERREDID=" + l_seccontindpreferredid;
        l_rs = m_db.runSQL(sqlString, stmt);
        l_rs.next();
        ...
        if(counter <= 1){
          ...
          sqlString = "select count (*) as counter from SECCONTENTINDICATOREVLINK where " +
                      "SECCONTENTINDICATORPREFERREDID =" + l_seccontindpreferredid;
          l_rs = m_db.runSQL(sqlString, stmt);
          l_rs.next();
          counter = l_rs.getInt("counter");

          if((counter <= 0) != true)        
            ret = true; // can't del sec content id & pref term        
        }
      }  
      stmt.close();
    }catch(Exception e){
  ...
  */
  ...
  Purpose: Returns a record set with all of the Secondary Content Indicator information in it.
