   ...
   */
   public void load(String p_WORKORGLINKID )
   {
     ...
     try
     {
       ...
       sqlString = " SELECT * FROM WORKORGLINK" +
                   " WHERE WORKORGLINKID = " + p_WORKORGLINKID;
   ...
   */
   public boolean update (String p_workId, Vector p_organLinks)
   {
     try
     {
       ...
       sqlString = "DELETE FROM WORKORGLINK where " +
                   "workid=" + p_workId ;
       m_db.runSQL (sqlString, stmt);

       for (int i=0; p_organLinks != null && i < p_organLinks.size(); i++){
         workOrganLink = (WorkOrganLink)p_organLinks.get(i);
         sqlString = "INSERT INTO WORKORGLINK "    +
                     "(workid, WORKORGLINK, orderBy) " +
                     "VALUES (" +
                     p_workId + ", " + workOrganLink.getOrganId() + "," + workOrganLink.getOrderBy() + ")";
         m_db.runSQL (sqlString, stmt);
       }
       l_ret = true;
   ...
   public void setOrganId         (String s) {organId     = s;}
