...
public class EntityType {
	 ...
	 * 
	 * Parameters: p_id : id of the Entity Type record
	 * 
	 * Returns: None
	 */
	public void load(int p_id) {
		...
		try {
			...
			sqlString = "SELECT * FROM entity_type WHERE " + "entity_type_id=" + p_id;
	 ...
	 * id of the new record in the m_entity_type_id member.
	 */
	public boolean add() {
		try {
			Statement stmt = m_db.m_conn.createStatement();
			...
			if (validateObjectForDB()) {
				if (!isDuplicate()) {
					sqlString = "INSERT INTO entity_type (type) VALUES (" + "'" + m_db.plSqlSafeString(m_entity_type_name) + "')";
	...
	private boolean isDuplicate() {
		...
		try {
			...
			Statement stmt = m_db.m_conn.createStatement();
			String sqlString;
			sqlString = "select type from entity_type where " + "lower(type)='" + m_db.plSqlSafeString(m_entity_type_name).toLowerCase() + "'";
			if (m_entity_type_id != 0) sqlString += " and not entity_type_id=" + m_entity_type_id;
			l_rs = m_db.runSQL(sqlString, stmt);
	...
	public boolean update() {
		try {
			...
			if (validateObjectForDB()) {
				if (!isDuplicate()) {
					sqlString = "UPDATE entity_type set type='" + m_db.plSqlSafeString(m_entity_type_name) + "' " + "where entity_type_id=" + m_entity_type_id;
					m_db.runSQL(sqlString, stmt);
					l_ret = true;
				} else {
					l_ret = false;
	 ...
	 * Name: checkInUse ()
	 * 
	 * Purpose: Checks to see if the specified Entity Ttype is in use in the
	 * database.
	 * 
	 * Parameters: p_id : id of the record
	 * 
	 * Returns: True if the entity type is in use, else false
	 */
	public boolean checkInUse(int p_id) {
		...
		try {
			...
			// Venue
			sqlString = "SELECT * FROM entity WHERE " + "type=" + p_id;
			l_rs = m_db.runSQL(sqlString, stmt);
	 ...
	 * Name: getEntityTypes ()
	 * 
	 * Purpose: Returns a record set with all of the entity type information in
	 * it.
	 * 
