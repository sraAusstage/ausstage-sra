...
public class Materials {
	...
	public void load(int p_id) {
		...
		try {
			...
			sqlString = "SELECT * FROM materials WHERE " + "material_id=" + p_id;
	 ...
	 * id of the new record in the m_material_id member.
	 */
	public boolean add() {
		try {
			Statement stmt = m_db.m_conn.createStatement();
			...
			if (validateObjectForDB()) {
				...
				sqlString = "INSERT INTO MATERIALS (MATERIALS) VALUES (" + "'" + m_db.plSqlSafeString(m_materials) + "')";
	...
	public boolean update() {
		try {
			...
			// required fields
			if (validateObjectForDB()) {
				sqlString = "UPDATE MATERIALS set " + "MATERIALS='" + m_db.plSqlSafeString(m_materials) + "' " + "where material_id=" + m_material_id;
				m_db.runSQL(sqlString, stmt);
				l_ret = true;
			}
			stmt.close();
	 ...
	 * Name: checkInUse ()
	 * 
	 * Purpose: Checks to see if the specified Materials is in use in the
	 * database.
	 * 
	 * Parameters: p_id : id of the materials record
	 * 
	 * Returns: True if the materials is in use, else false
	 */
	 ...
	 * 
	 * Purpose: Returns a record set with all of the materials information in
	 * it.
	 * 
