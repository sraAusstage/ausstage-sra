...
public class ContributorFunction extends AusstageInputOutput
{
  ...
  */
  ...
  Purpose: Sets the class to a new event id and populates the object with the new
           contributor function information.
  ...
  */
  ...
  Parameters:
    p_cont_funct            = contributor function
    p_desc                  = contributor function description
    p_contribfunct_preff_id = contributor function preferred term id

  Returns:
     None

  */
  public void reinitialise(String p_cont_funct
                          , String p_desc
                          , int p_contribfunct_pref_id)
  {
    m_name         = p_cont_funct;
  ...
  public boolean add (){
    ...
    try
    {
      ...
      sqlString = "insert into contfunct " + 
                  "(contfunction, contfunctiondescription, contributorfunctpreferredid) " + 
                  "VALUES (" +
  ...
  */
  public boolean update()
  {
    ...
    try{
      ...
      if(m_preferred_id != 0){
        sqlString = "update contfunct " + 
                    "set contfunction='" + m_db.plSqlSafeString(m_name) + "', " +
                    "contfunctiondescription= '" + m_db.plSqlSafeString(m_description) + "', " +
                    "contributorfunctpreferredid= " + m_preferred_id + " " +
        ...
        sqlString = "Insert into contributorfunctpreferred " +
        ...
        sqlString  = "update contfunct " + 
                     "set contributorfunctpreferredid= " + m_preferred_id + " " +
                     "where contfunctionid= " + m_id;
  ...
  public boolean delete(){
    ...
    try{
       ...
       if(!isInUse()){
         ...
         sqlString = "SELECT CONTRIBUTORFUNCTPREFERREDID " +
                       "FROM CONTFUNCT WHERE contfunctionid = " + m_id;
         l_rs = m_db.runSQL (sqlString, stmt);

         if(l_rs.next()){
           String contribfunctprefid = l_rs.getString("CONTRIBUTORFUNCTPREFERREDID");

           sqlString = "SELECT count(*) as counter " +
                         "FROM CONTFUNCT " +
                        "WHERE CONTRIBUTORFUNCTPREFERREDID = " + contribfunctprefid;
           l_rs = m_db.runSQL (sqlString, stmt);

           if(l_rs.next() && l_rs.getInt("counter") == 1){
             // del the preferred term
             sqlString = "DELETE FROM CONTRIBUTORFUNCTPREFERRED " +
                          "WHERE CONTRIBUTORFUNCTPREFERREDID = " + contribfunctprefid;
             m_db.runSQL (sqlString, stmt); 
           }
         }
  ...
  */
  ...
  public boolean isInUse(){
    ...
    try{
      ...
      sqlString = "select CONTRIBUTORFUNCTPREFERRED.CONTRIBUTORFUNCTPREFERREDID from " +
                  "CONTRIBUTORFUNCTPREFERRED, CONTFUNCT " +
                  "where CONTFUNCT.CONTFUNCTIONID = " + m_id + " " +
                  "and CONTFUNCT.CONTRIBUTORFUNCTPREFERREDID=" +
      ...
      if(l_rs.next()){
        ...
        sqlString = "select count(*) as counter from CONTFUNCT " +
                    "where CONTRIBUTORFUNCTPREFERREDID=" + l_contributorfunctpreferredid;
        l_rs = m_db.runSQL(sqlString, stmt);
        l_rs.next();
        counter = l_rs.getInt("counter");
        
        if(counter <= 1){
          // this is the last contfunct that is related to the preferred term
          // now check if preferred term is in use
          sqlString = "select count(*) as counter from CONTFUNCTLINK where " +
                      "CONTRIBUTORFUNCTPREFERREDID=" + l_contributorfunctpreferredid;
          l_rs = m_db.runSQL(sqlString, stmt);
          l_rs.next();
          counter = l_rs.getInt("counter");

          if((counter <= 0) != true)        
            ret = true; // can't contribfunct & pref term    
        }
      }  
      stmt.close();
    }catch(Exception e){
  ...
  public CachedRowSet getNames(){
    ...
    try{
      Statement stmt = m_db.m_conn.createStatement ();
      sqlString = "SELECT cf.ContributorFunctPreferredId, ContFunction, PreferredTerm, contfunctionid " +
                  "FROM ContFunct cf, ContributorFunctPreferred cfp " +
                  "WHERE cf.ContributorFunctPreferredId = cfp.ContributorFunctPreferredId " + 
                  "ORDER BY ContFunction";
      l_rs = m_db.runSQL(sqlString, stmt);
      stmt.close();
    }catch(Exception e){
  ...
  public ResultSet getAssociatedContributors(int p_id, Statement p_stmt){
	    ...
	    try{
	      ...
	      l_sql = "SELECT contributorfunctpreferred.preferredterm,concat_ws(' ', contributor.first_name,contributor.last_name) as name,contributorfunctpreferred.contributorfunctpreferredid,contributor.contributorid  " +
	                          ...
	                          "WHERE contributorfunctpreferred.contributorfunctpreferredid=" + p_id + " Order by name ";
