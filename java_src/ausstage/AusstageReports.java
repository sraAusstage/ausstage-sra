...
public class AusstageReports
{
  ...
  private static final int max_limit_for_sqlInterface_output = 150; // used to set the number rows 
                                                                    // returned to sql interface 
  ...
  private boolean isReachedMaxNumberOfRowsAllowed = false;
	...
	public AusstageReports (ausstage.Database p_db
                          , HttpServletRequest p_request
                          , String p_report_file_name)
	{
    try{
    ...
    boolean ret = true;
      ...
      if(p_sqlString.toLowerCase().startsWith("select")){
        ...
        crs.close();
      }else if(p_sqlString.toLowerCase().startsWith("insert") 
              || p_sqlString.toLowerCase().startsWith("update")
              || p_sqlString.toLowerCase().startsWith("delete")){
    ...
    String sqlString  = "select table_name from information_schema.tables where not table_name like '%$%'";
      ...
      if(crs.next()){
        ...
        do{
          ...
          // add the table record count to the comma delimited buffer object
  ...
  public String getCommaDelimFileSize(){
    if(m_file_size != null && !m_file_size.equals("")){
      if(Integer.parseInt(m_file_size) < 1)
        m_file_size = "1";
    }else{
      m_file_size = "0";
    }
