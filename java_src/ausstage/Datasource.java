...
public class Datasource
{
  ...
  */
  ...
  Parameters:
    p_id : id of the datasource record
  ...
  */
  public void load(int p_id)
  {
    ...
    {      
      ...
      sqlString = "SELECT * FROM DATASOURCE WHERE " +
                  "DATASOURCEID=" + p_id;
  ...
  Returns:
     ...
     new record in the m_datasource_id member.
  ...
  */
  public boolean add()
  {
    ...
    try
    {
      ...
      if (m_datasource_desc.length() >= 300)
        m_datasource_desc = m_datasource_desc.substring (0, 299);

      l_sql = "INSERT INTO datasource (datasource, datadescription) " + 
                   "VALUES ('" + m_db.plSqlSafeString(m_datasource_name) + "', " + 
  ...
  */
  public boolean update ()
  {
    ...
    try
    {
      ...
      l_sql = "UPDATE datasource SET " + 
                "datasource = '" + m_db.plSqlSafeString(m_datasource_name) + "', " + 
                "datadescription = '" + m_db.plSqlSafeString(m_datasource_desc) + "' " + 
  ...
  public boolean addLink (String p_eventid)
  {
    ...
    try
    {
      ...
      l_sql = "INSERT INTO DATASOURCEEVLINK " +
              "(DATASOURCEDESCRIPTION, COLLECTION, EVENTID, DATASOURCEID) " + 
              "values ('" + m_db.plSqlSafeString(m_datasource_desc) + 
              "','" + m_db.plSqlSafeString(m_collection) + 
              "'," + p_eventid + "," + m_datasource_id + ")";
  ...
  Purpose:  Deletes the record. This instance will retain all 
            of the datasource information except the id.
  ...
  */
  ...
  Purpose: Returns a record set with all of the datasource information in it.
  ...
  */
  ...
  Purpose: Returns a record set (depending on the p_sqlString sql parameter)
           with all of the datasource information in it.
  ...
  */
  ...
  Purpose: Checks to see if the specified Datasource is in use in the database.

  Parameters:
    p_id : id of the datasource record

  Returns:
     True if the datasource is in use, else false
  ...
  */
  public boolean checkInUse(int p_id)
  {
    ...
    {      
      ...
      sqlString = "SELECT * FROM datasourceevlink WHERE " +
                  "datasourceid=" + p_id;
      l_rs = m_db.runSQL (sqlString, stmt);
  ...
  public String isCollection(){
    String ret = "No";
    CachedRowSet  l_rs;
    String sqlString = "select collection from DATASOURCEEVLINK " +
                       "where DATASOURCEID=" + m_datasource_id + " and " +
                       "DATASOURCEEVLINKID='" + m_datasoureevlinkid+"'";
    try{
      Statement stmt = m_db.m_conn.createStatement();
      l_rs = m_db.runSQL(sqlString, stmt);
      if(l_rs.next()){
        if(l_rs.getString("collection").equals("yes"))
          ret = "Yes";
      } 
      stmt.close();
    }catch(Exception e){
    ...
    try{
     //load(p_id);
      ...
      //if(m_datasource_name != null && !m_datasource_name.equals("")){
        sqlString = "select DATASOURCE.DATASOURCE, " +
                    "DATASOURCEEVLINK.DATASOURCEEVLINKID, " +
                    "DATASOURCEEVLINK.DATASOURCEID, " +
                    "DATASOURCEEVLINK.DATASOURCEDESCRIPTION, " +
                    "DATASOURCEEVLINK.COLLECTION " +
                    "from DATASOURCEEVLINK, DATASOURCE " +
                    "where EVENTID=" + m_event_id + " and DATASOURCEEVLINKID=" + p_id + " and " +
                    "DATASOURCE.DATASOURCEID=DATASOURCEEVLINK.DATASOURCEID " +
                    "ORDER BY DATASOURCE.DATASOURCE ASC";
        l_rs = m_db.runSQL(sqlString, stmt);
        if(l_rs.next()){
          // make m_datasource_desc & m_collection are to be
          // the description & colection from 
          // DATASOURCEEVLINK.DATASOURCEDESCRIPTION & DATASOURCEEVLINK.COLLECTION
         ...
         // m_datasource_name   = getName(l_rs.getInt("DATASOURCEID"));
          m_datasource_name   = l_rs.getString("DATASOURCE");
          if(m_datasource_name == null || m_datasource_name.equals("")){m_datasource_name = "";}
          if(m_datasource_desc == null || m_datasource_desc.equals("")){m_datasource_desc = "";}
          if(m_collection == null || m_collection.equals("")){m_collection = "";}
          if(m_datasoureevlinkid == null || m_datasoureevlinkid.equals("")){m_datasoureevlinkid = "";}
        }
      //}//else{
  ...
  public void loadLinkedProperties(int p_id, int p_event_id, String p_datasourceevelinkids){
    ...
    String sqlString = "select DATASOURCEEVLINK.DATASOURCEEVLINKID, DATASOURCE.DATASOURCE, " +
                       "DATASOURCEEVLINK.DATASOURCEDESCRIPTION, " +
                       "DATASOURCEEVLINK.COLLECTION " +
                       "from DATASOURCE, DATASOURCEEVLINK " +
                       "where DATASOURCE.DATASOURCEID=" + p_id + " " +
                       "and DATASOURCE.DATASOURCEID=DATASOURCEEVLINK.DATASOURCEID " +
                       "and DATASOURCEEVLINK.EVENTID=" + p_event_id + " and " +
                       "DATASOURCEEVLINK.DATASOURCEEVLINKID not in (" + p_datasourceevelinkids + ") " +
                       "ORDER BY DATASOURCE.DATASOURCE ASC";
  ...
  */
  ...
  public String getName(int p_id)
  {
    CachedRowSet  l_rs;
    String ret_val   = "";
    String sqlString = "select DATASOURCE from DATASOURCE " +
                       "where DATASOURCEID=" + p_id;
    try{
      Statement stmt = m_db.m_conn.createStatement();
      l_rs = m_db.runSQL(sqlString, stmt);
  ...
  Returns:
     Description of the loaded datasource
  ...
  */
  ...
  Parameters: 
     p_new_name   The value to set the name of this datasource to.
