	...
	public ResultSet getAssociatedItems(int p_event_id, Statement p_stmt) {
		...
		try {
			...
			l_sql = "SELECT item.itemid, item.catalogueid,  item.item_description, item.citation, ist.description item_sub_type, organisation.name,  states.state,  lookup_codes.code_lov_id,  lookup_codes.description,  lookup_codes.short_code,  `itemevlink`.`eventid` "
					+ "FROM item LEFT JOIN lookup_codes ist ON (item.item_sub_type = ist.code_lov_id) INNER JOIN itemevlink ON (itemevlink.itemid = item.itemid)  LEFT JOIN organisation ON (item.institutionid = organisation.organisationid)  LEFT JOIN states ON (organisation.state = states.stateid)  LEFT JOIN lookup_codes ON (item.item_type_lov_id = lookup_codes.code_lov_id) "
