...
public class Event {
 ...
 // private boolean m_part_of_a_tour;
  ...
  public CachedRowSet getEventsByVenue(int p_venue_id) {
    ...
    try {
      ...
      sqlString = 
          "select eventid, event_name, events.ddfirst_date, events.mmfirst_date, events.yyyyfirst_date, first_date,date_format(STR_TO_DATE(CONCAT(events.ddfirst_date,' ',events.mmfirst_date,' ',events.yyyyfirst_date), '%d %m %Y'), '%e %M %Y') as evDate  " + 
  ...
  public ResultSet getEventsByItem(int p_event_id, Statement p_stmt) {
    ...
    try {
      ...
      sqlString = 
          "SELECT DISTINCT events.eventid, event_name, events.ddfirst_date, events.mmfirst_date, events.yyyyfirst_date, first_date" + 
          ", venue_name, suburb, states.state " + 
          "FROM events, itemevlink, venue, states " + 
  ...
  public CachedRowSet getEventsByOrg(int p_org_id) {
    ...
    try {
      ...
      sqlString = 
          "select distinct events.eventid, event_name, events.ddfirst_date, events.mmfirst_date, events.yyyyfirst_date, events.first_date, " + 
          "venue_name, suburb, states.state " + 
          "from events, venue, states, orgevlink " + 
          "where events.venueid=venue.venueid " + 
          "and venue.state=states.stateid " + 
          "and events.eventid=orgevlink.eventid " + 
          "and orgevlink.organisationid=" + p_org_id + " " + 
  ...
  public CachedRowSet getEventsByContrib(int p_contrib_id) {
    ...
    try {
      ...
      sqlString = 
          "select distinct events.eventid, events.event_name,  events.ddfirst_date, events.mmfirst_date, events.yyyyfirst_date, events.first_date, " + 
  ...
  public CachedRowSet getEventsById(String p_event_id) {
    ...
    try {
      ...
      sqlString = "select events.*, search_event.resource_flag from events inner join search_event on events.eventid = search_event.eventid where events.eventid=" + p_event_id;
  ...
  */
  ...
  Purpose: Sets the class to a new event id and populates the object with the new
           event information.

  Parameters:
    p_id : id of the event record
  ...
  */
  ...
  public void load(int p_id) {
    ...
    try {
      ...
      if (l_rs.next()) {
        ...
        m_world_premier = 
            Common.convertYesNoToBool(l_rs.getString("world_premier"));
        m_review = 
            Common.convertYesNoToBool(l_rs.getString("review"));
        m_int_perf_history = 
            Common.convertYesNoToBool(l_rs.getString("interesting_perf_history"));
        ...
        m_estimated_dates = 
            Common.convertYesNoToBool(l_rs.getString("estimated_dates"));
        //m_est_first_date           = Common.convertYesNoToBool (l_rs.getString ("estimated_first_dates"));
        //m_est_last_date            = Common.convertYesNoToBool (l_rs.getString ("estimated_last_dates"));
        //m_est_open_date            = Common.convertYesNoToBool (l_rs.getString ("estimated_open_dates"));

        // Encode double quotes
        m_event_name = Common.formFieldEncode(m_event_name, "\"");
        m_description = Common.formFieldEncode(m_description, "\"");
        m_further_information = 
            Common.formFieldEncode(m_further_information, "\"");
        m_umbrella = Common.formFieldEncode(m_umbrella, "\"");

        if (m_venueid.length() != 0)
          m_venue.load(Integer.parseInt(m_venueid));
        ...
        if (m_venueid == null)
          m_venueid = "";
        if (m_primary_genre == null)
          m_primary_genre = "";
        if (m_further_information == null)
          m_further_information = "";
        if (m_description_source == null)
        ...
        if (m_ddopen_date == null)
          m_ddopen_date = "";
        if (m_mmopen_date == null)
          m_mmopen_date = "";
        if (m_yyyyopen_date == null)
          m_yyyyopen_date = "";
      }
      l_rs.close();
      stmt.close();
  ...
  public void loadLinkedProperties(int p_event_id, String p_event_name, 
                                   Date p_first_date, String p_venue_name) {
  ...
  */
  ...
  private void loadDatasources() {
    ...
    try {
      ...
      sqlString = 
          "SELECT DATASOURCEEVLINKID FROM DATASOURCEEVLINK " + "WHERE EVENTID=" + 
          m_eventid;
      l_rs = m_db.runSQL(sqlString, stmt);
  ...
  */
  ...
  private void loadSecondaryGenres() {
    ...
    try {
      ...
      sqlString = 
          "SELECT secgenrepreferredid FROM SECGENRECLASSLINK " + "WHERE EVENTID=" + 
          m_eventid;
      l_rs = m_db.runSQL(sqlString, stmt);
  ...
  */
  ...
  private void loadPrimContentIndicatorEvLinks() {
    PrimContentIndicatorEvLink temp_object = 
      new PrimContentIndicatorEvLink(m_db);

    try {
      m_prim_content_indicator_evlinks = 
          temp_object.getPrimContentIndicatorEvLinksForEvent(Integer.parseInt(m_eventid));
    } catch (Exception e) {
      System.out.println(">>>>>>>> EXCEPTION <<<<<<<<");
  ...
  private void loadSecGenreEvLinks() {
    ...
    try {
      m_sec_genre_evlinks = 
          temp_object.getSecondaryGenreEvLinksForEvent(Integer.parseInt(m_eventid));
    } catch (Exception e) {
      System.out.println(">>>>>>>> EXCEPTION <<<<<<<<");
  ...
  Purpose: Sets the class to a contain the Secondary Content Indicator EvLink information for the
           specified event id.
  ...
  */
  /*private void loadSecContentIndicatorEvLinks()
  {
    CachedRowSet        l_rs;
    String              sqlString;
    SecContentIndicatorEvLink temp_object = new SecContentIndicatorEvLink(m_db);;

    try
    {
      m_sec_content_indicator_evlinks =
          temp_object.getSecContentIndicatorEvLinksForEvent(Integer.parseInt(m_eventid));
    }
    catch (Exception e)
    {
      System.out.println(">>>>>>>> EXCEPTION <<<<<<<<");
      System.out.println ("An Exception occured in loadSecContentIndicatorEvLinks().");
      System.out.println("MESSAGE: " + e.getMessage());
      System.out.println("LOCALIZED MESSAGE: " + e.getLocalizedMessage());
      System.out.println("CLASS.TOSTRING: " + e.toString());
      System.out.println(">>>>>>>>>>>>>-<<<<<<<<<<<<<");
...
*/
  ...
  */
  ...
  private void loadConEvLinks() {
    ...
    try {
      m_con_evlinks = 
          temp_object.getConEvLinksForEvent(Integer.parseInt(m_eventid));
    } catch (Exception e) {
      System.out.println(">>>>>>>> EXCEPTION <<<<<<<<");
  ...
  Purpose: Sets the class to a contain the EventEventLink information for the
           specified event id.
  ...
  */
  private void loadEventEventLinks() {
	  EventEventLink temp_object = new EventEventLink(m_db);
	    ...
	    try {
	      m_event_eventlinks = 
	          temp_object.getEventEventLinksForEvent(Integer.parseInt(m_eventid));
	    } catch (Exception e) {
	      System.out.println(">>>>>>>> EXCEPTION <<<<<<<<");
  ...
  */
  ...
  private void loadOrgEvLinks() {
    ...
    try {
      m_org_evlinks = 
          temp_object.getOrgEvLinksForEvent(Integer.parseInt(m_eventid));
    } catch (Exception e) {
      System.out.println(">>>>>>>> EXCEPTION <<<<<<<<");
  ...
  */
  ...
  Purpose: Sets the class to a contain the ResourceEvLink information for the
           specified event id.
  ...
  */
  ...
  private void loadResourceEvLinks() {
    ...
    try {
      ...
      l_sql = 
          "SELECT itemid " + "FROM itemevlink " + "WHERE eventid=" + m_eventid;
  ...
  Purpose: Sets the class to a contain the PlayOrigin (ie, Country) information for the
           specified event id.
  ...
  */
  ...
  private void loadPlayOrigins() {
    ...
    try {
      ...
      sqlString = 
          "SELECT COUNTRYID FROM PLAYEVLINK " + "WHERE EVENTID=" + m_eventid;
      l_rs = m_db.runSQL(sqlString, stmt);
  ...
  Purpose: Sets the class to a contain the ProductionOrigin (ie, Country) information for the
           specified event id.
  ...
  */
  ...
  private void loadProductionOrigins() {
    ...
    try {
      ...
      sqlString = 
          "SELECT COUNTRYID FROM PRODUCTIONEVLINK " + "WHERE EVENTID=" + 
          m_eventid;
      l_rs = m_db.runSQL(sqlString, stmt);
  ...
  public boolean update() {
    ...
    try {
      ...
      if (validateObjectForDB()) {
        ...
        if (m_event_name.length() > 200)
          m_event_name = m_event_name.substring(0, 199);

        // As the Description is a text area, need to limit characters
        if (m_description.length() > 2000)
          m_description = m_description.substring(0, 1999);

        // As the Further Information is a text area, need to limit characters
        if (m_further_information.length() > 400)
          m_further_information = m_further_information.substring(0, 399);

        if (m_description_source == null || m_description_source.equals(""))
          m_description_source = "0";
        ...
        if (m_entered_date != null)
          l_entered_date = m_db.safeDateFormat(m_entered_date, false);
        if (m_updated_date != null)
          l_updated_date = m_db.safeDateFormat(m_updated_date, false);

        /***************************
        * DEAL WITH FIRST DATE
        ***************************/
        if (m_ddfirst_date.equals("") && m_mmfirst_date.equals("") && 
            m_yyyyfirst_date.equals("")) {
          // no date specified
          l_fisrt_date = "null";
        } else if (m_ddfirst_date.equals("") && !m_mmfirst_date.equals("") && 
                   !m_yyyyfirst_date.equals("")) {
          // month and year were filled in
          l_fisrt_date = 
              m_db.safeDateFormat("01/" + m_mmfirst_date + "/" + m_yyyyfirst_date);
        } else if (m_ddfirst_date.equals("") && m_mmfirst_date.equals("") && 
                   !m_yyyyfirst_date.equals("")) {
        ...
        * DEAL WITH LAST DATE
        ***************************/
        if (m_ddlast_date.equals("") && m_mmlast_date.equals("") && 
            m_yyyylast_date.equals("")) {
          // no date specified
          l_last_date = "null";
        } else if (m_ddlast_date.equals("") && !m_mmlast_date.equals("") && 
                   !m_yyyylast_date.equals("")) {
          // month and year were filled in
          l_last_date = 
              m_db.safeDateFormat(getLastDayOfMonth(m_mmlast_date, m_yyyylast_date) + 
                                  "/" + m_mmlast_date + "/" + m_yyyylast_date);
        } else if (m_ddlast_date.equals("") && m_mmlast_date.equals("") && 
                   !m_yyyylast_date.equals("")) {
          // just year is filled in
        ...
        ***************************/
        if (m_ddopen_date.equals("") && m_mmopen_date.equals("") && 
            m_yyyyopen_date.equals("")) {
          // no date specified
          l_openning_night_date = "null";
        } else if (m_ddopen_date.equals("") && !m_mmopen_date.equals("") && 
                   !m_yyyyopen_date.equals("")) {
          // month and year were filled in
          l_openning_night_date = 
              m_db.safeDateFormat("01/" + m_mmopen_date + "/" + 
                                  m_yyyyopen_date);
        } else if (m_ddopen_date.equals("") && m_mmopen_date.equals("") && 
                   !m_yyyyopen_date.equals("")) {
          // just year is filled in
          l_openning_night_date = 
              m_db.safeDateFormat("01/01/" + m_yyyyopen_date);
        } else {
          // all fields in the this date were filled in
          l_openning_night_date = 
              m_db.safeDateFormat(m_ddopen_date + "/" + m_mmopen_date + "/" + 
                                  m_yyyyopen_date);
        }

        // "', entered_by_user = '" + m_db.plSqlSafeString(m_entered_by_user) +
          //"', interesting_perf_history = '" + Common.convertBoolToYesNo (m_int_perf_history) +
          //", est_first_date = '" + Common.convertBoolToYesNo(m_est_first_date) +                    
          //", est_open_date = '" + Common.convertBoolToYesNo(m_est_last_date) +                    
          //", est_open_date = '" + Common.convertBoolToYesNo(m_est_open_date) +                    
          sqlString = 
            "   UPDATE events SET " + "   event_name = '" + m_db.plSqlSafeString(m_event_name) + 
            ...
            "', status =  " + 
            m_status + " , venueid =  " + m_venueid + " , primary_genre =  " + 
            m_primary_genre + " , further_information = '" + 
            m_db.plSqlSafeString(m_further_information) + 
            "', description_source  =  " + m_description_source + 
            ", ddfirst_date = '" + m_ddfirst_date + "', mmfirst_date = '" + 
            m_mmfirst_date + "', yyyyfirst_date = '" + m_yyyyfirst_date + 
            "', first_date = " + l_fisrt_date + ", ddlast_date = '" + 
            m_ddlast_date + "', mmlast_date = '" + m_mmlast_date + 
            "', yyyylast_date = '" + m_yyyylast_date + "', last_date = " + 
            l_last_date + ", ddopening_night  = '" + m_ddopen_date + 
        ...
        if (!existInEvents(UPDATE)) {
          ...
          // Delete the current links, then for each item id in the vector
          // perform an insert
          ...
          for (int i = 0; i < m_res_evlinks.size(); i++) {
            l_sql = 
                "INSERT INTO itemevlink (EVENTID, ITEMID) " + "VALUES (" + m_eventid + 
                "," + m_res_evlinks.elementAt(i) + ")";
            m_db.runSQLResultSet(l_sql, stmt);
          }
          ...
          m_error_string = 
              "Unable to update " + m_event_name + " because this event is not unique.";
        }
      }
      stmt.close();
      return (l_ret);
  ...
  */
  ...
  public boolean add() {
    try {
      ...
      if (validateObjectForDB()) {
        ...
        // if we are in copy mode lets put a leading 'Copy of' in front of the event name
        if (m_is_in_copy_mode && !m_event_name.startsWith("Copy of"))
          m_event_name = "Copy of " + m_event_name;
        ...
        // As the Description is a text area, need to limit characters
        if (m_description.length() > 600)
          m_description = m_description.substring(0, 599);
        ...
        if (!existInEvents(INSERT)) {
          ...
          ***************************/
          ...
          //", '" + Common.convertBoolToYesNo(m_est_first_date          )  +
            ...
            sqlString = 
              "INSERT INTO events " + "(event_name, umbrella, entered_by_user, entered_date, " + 
              " description,  world_premier, review,  interesting_perf_history," + 
              ...
              "VALUES (" + "   '" + m_db.plSqlSafeString(m_event_name) + 
              ...
              (m_status) + " ,  " + (m_venueid) + " ,  " + (m_primary_genre) + 
              " , '" + m_db.plSqlSafeString(m_further_information) + "',  " + 
              (m_description_source) + ", '" + (m_ddfirst_date) + "', '" + 
              (m_mmfirst_date) + "', '" + (m_yyyyfirst_date) + "', " + 
              l_fisrt_date + ", '" + (m_ddlast_date) + "', '" + 
              (m_mmlast_date) + "', '" + (m_yyyylast_date) + "', " + 
              l_last_date + ", '" + (m_ddopen_date) + "', '" + 
              (m_mmopen_date) + "', '" + (m_yyyyopen_date) + "', " + 
              l_openning_night_date + ", '" + 
              Common.convertBoolToYesNo(m_estimated_dates) + "')";
        ...
        //  modifySecContentIndicatorEvLinks(INSERT);
          ...
          for (int i = 0; i < m_res_evlinks.size(); i++) {
            String l_sql = 
              "INSERT INTO itemevlink (EVENTID, ITEMID) " + "VALUES (" + 
              m_eventid + "," + m_res_evlinks.elementAt(i) + ")";
            m_db.runSQLResultSet(l_sql, stmt);
          }
          ...
          if (m_is_in_copy_mode)
            m_error_string = 
                "Unable to copy " + m_event_name + " because this event is not unique.";
          else
            m_error_string = 
                "Unable to add " + m_event_name + " because this event is not unique.";
        }
      }
      stmt.close();
      return (l_ret);
  ...
  private String getLastDayOfMonth(String p_month, String p_year) {
    ...
    if (p_month.indexOf("0") == 0)
      p_month = p_month.substring(p_month.indexOf("0") + 1);
  ...
  */
  ...
  Returns:
     True if the collection is in use, else false
...
/*
  private void modifySecContentIndicatorEvLinks(int modifyType) {
    SecContentIndicatorEvLink temp_object = 
      new SecContentIndicatorEvLink(m_db);

    try {
      switch (modifyType) {
      case UPDATE:
        temp_object.update(m_eventid, m_sec_content_indicator_evlinks);
        break;
      case DELETE:
        temp_object.deleteSecContentIndicatorEvLinksForEvent(m_eventid);
        break;
      case INSERT:
        temp_object.add(m_eventid, m_sec_content_indicator_evlinks);
        break;
      }
    } catch (Exception e) {
      System.out.println(">>>>>>>> EXCEPTION <<<<<<<<");
      System.out.println("An Exception occured in modifySecContentIndicatorEvLinks().");
      System.out.println("MESSAGE: " + e.getMessage());
      System.out.println("LOCALIZED MESSAGE: " + e.getLocalizedMessage());
      System.out.println("CLASS.TOSTRING: " + e.toString());
      System.out.println(">>>>>>>>>>>>>-<<<<<<<<<<<<<");
...
*/
  ...
  */
  /* private void modifyArticles(int modifyType)
  {
    Articles temp_object;
...
//        temp_object.delete();
      ...
      {
        temp_object = (Articles)m_articles.get(i);
        switch (modifyType)
        {
          case UPDATE:
          case INSERT:
//          temp_object.addLink(m_eventid);
  ...
  */
  ...
  Returns:
     The day/month field with the leading zero.
...
*/
  ...
  public String getEventName(int eventId) {
    ...
    try {
      Statement stmt = m_db.m_conn.createStatement();
      rset = 
          m_db.runSQL("select EVENT_NAME from EVENTS where EVENTID=" + eventId, 
                      stmt);
      if (rset.next())
        event_name = rset.getString("EVENT_NAME");
      stmt.close();
    } catch (Exception e) {
      System.out.println(">>>>>>>> EXCEPTION <<<<<<<<");
  ...
  public String getEventStatus(String status_id) {
    ...
    try {
      if (status_id != null && !status_id.equals("")) {
        Statement stmt = m_db.m_conn.createStatement();
        rset = 
            m_db.runSQL("select STATUS from STATUSMENU where STATUSID=" + status_id, 
                        stmt);
        if (rset.next())
          status_name = rset.getString("STATUS");
        stmt.close();
      }
    } catch (Exception e) {
      System.out.println(">>>>>>>> EXCEPTION <<<<<<<<");
  ...
  public String getContributorFunct(String p_id) {
    ...
    try {
      ...
      sqlString = 
          " SELECT PreferredTerm FROM ConEvLink, ContributorFunctPreferred, events" + 
          " WHERE contributorId = " + p_id + 
          " AND   ConEvLink.eventId = events.eventid " + 
...
*/
  ...
  // 
  void setItemEvLinks(Vector p_event_eventlinks, Statement p_stmt, 
  ...
  */
  ...
  public void setEventAttributes(HttpServletRequest request) {
    ...
    this.m_world_premier = 
        Common.convertYesNoToBool(request.getParameter("f_world_premier"));
    this.m_review= 
    	Common.convertYesNoToBool(request.getParameter("f_review"));
    System.out.println("request.getParameter:" +request.getParameter("f_review"));
    //this.m_int_perf_history    = Common.convertYesNoToBool(request.getParameter("f_int_perf_history"));
    ...
    //this.m_est_first_date      = Common.convertYesNoToBool(request.getParameter("f_est_first_date"));
    //this.m_est_last_date       = Common.convertYesNoToBool(request.getParameter("f_est_last_date"));
    //this.m_est_open_date       = Common.convertYesNoToBool(request.getParameter("f_est_open_date"));
    ...
    {
      m_error_string = 
          "Unable to update the Event. The First Date field is invalid.";
    }
    ...
    try {
      ...
      m_error_string = 
          "Unable to update the Event. The Last Date field is invalid.";
    }
    ...
    try {
      ...
      m_error_string = 
          "Unable to update the Event. The Opening Night field is invalid.";
    }

    this.m_estimated_dates = 
        Common.convertYesNoToBool(request.getParameter("f_estimated_dates"));
  }
  ...
  private boolean existInEvents(int mode) {
    ...
    try {
      ...
      if (mode == 2) {
        //rset = m_db.runSQL("select EVENTID from EVENTS where EVENT_NAME='" + m_db.plSqlSafeString(m_event_name) + "' and EVENTID !=" + m_eventid, stmt);
        l_sqlStr = 
            "select EVENTID from EVENTS where EVENT_NAME='" + m_db.plSqlSafeString(m_event_name) + 
            "' and VENUEID=" + m_venueid;
      } else {
        l_sqlStr = 
            "select EVENTID from EVENTS where EVENT_NAME='" + m_db.plSqlSafeString(m_event_name) + 
            "' and VENUEID=" + m_venueid;

        //rset = m_db.runSQL("select EVENTID from EVENTS where EVENT_NAME='" + m_db.plSqlSafeString(m_event_name) + "'", stmt);
        //rset = m_db.runSQL("select EVENTID from EVENTS where VENUEID=" + m_venueid , stmt);
      }
      //if(rset.next())
      //  isEventNameExistInEvent = true;

      /*
      // check for duplicate event venue
      if(mode == 2){
        l_sqlStr +="";
        //rset = m_db.runSQL("select EVENTID from EVENTS where VENUEID=" + m_venueid + " and EVENTID !=" + m_eventid, stmt);
      }else{
        rset = m_db.runSQL("select EVENTID from EVENTS where VENUEID=" + m_venueid , stmt);
      }
 */
      ...
      if (mode == 2) {
        //update
        if ((m_ddfirst_date != null && !m_ddfirst_date.equals("")) && 
            (m_mmfirst_date != null && !m_mmfirst_date.equals("")) && 
            (m_yyyyfirst_date != null && !m_yyyyfirst_date.equals(""))) {

          l_sqlStr += 
              " and FIRST_DATE=" + m_db.safeDateFormat(m_ddfirst_date + " " + 
                                                       m_db.getMonthName(Integer.parseInt(m_mmfirst_date), 
                                                                         true) + 
                                                       " " + m_yyyyfirst_date);
          //sqlString = "select EVENTID from EVENTS where FIRST_DATE='" + m_ddfirst_date + "-" + m_db.getMonthName(Integer.parseInt(m_mmfirst_date), true) + "-" + m_yyyyfirst_date  + "' and EVENTID !=" + m_eventid;
          ...
          l_sqlStr += 
              " and MMFIRST_DATE='" + m_mmfirst_date + "' and YYYYFIRST_DATE='" + 
              m_yyyyfirst_date + "'";
          /*
          sqlString = "select EVENTID from EVENTS where DDFIRST_DATE= '' and MMFIRST_DATE='" + m_mmfirst_date + "' and YYYYFIRST_DATE='" + m_yyyyfirst_date  + "' and EVENTID !=" + m_eventid;
          rset = m_db.runSQL(sqlString, stmt);
          if(rset.next())
            isFirstDateExistInEvent = true;
          */
          ...
          /*
          sqlString = "select EVENTID from EVENTS where DDFIRST_DATE= '' and MMFIRST_DATE = '' and YYYYFIRST_DATE='" + m_yyyyfirst_date  + "' and EVENTID !=" + m_eventid;
          rset = m_db.runSQL(sqlString, stmt);
          if(rset.next())
            isFirstDateExistInEvent = true;
          */
        ...
        if ((m_ddfirst_date != null && !m_ddfirst_date.equals("")) && 
          ...
          //sqlString = "select EVENTID from EVENTS where FIRST_DATE='" + m_ddfirst_date + "-" + m_db.getMonthName(Integer.parseInt(m_mmfirst_date), true) + "-" + m_yyyyfirst_date  + "'";
          //rset = m_db.runSQL(sqlString, stmt);
          //if(rset.next())
          //  isFirstDateExistInEvent = true;
          ...
          //sqlString = "select EVENTID from EVENTS where DDFIRST_DATE= '' and MMFIRST_DATE='" + m_mmfirst_date + "' and YYYYFIRST_DATE='" + m_yyyyfirst_date  + "'";
          //rset = m_db.runSQL(sqlString, stmt);    
          //if(rset.next())
          //  isFirstDateExistInEvent = true;
        } else if (m_yyyyfirst_date != null && !m_yyyyfirst_date.equals("")) {
          ...
          //  sqlString = "select EVENTID from EVENTS where DDFIRST_DATE= '' and MMFIRST_DATE = '' and YYYYFIRST_DATE='" + m_yyyyfirst_date  + "'";
          //  rset = m_db.runSQL(sqlString, stmt);    
          //  if(rset.next())
          //    isFirstDateExistInEvent = true;
        }
    ...
    //if(isEventNameExistInEvent && isVenueExistInEvent && isFirstDateExistInEvent)
  ...
  public String getEventInfoForItemDisplay(int p_event_id, Statement p_stmt) {
    ...
    try {
      /*
      sqlString = "SELECT DISTINCT event_name || ', '|| first_date || ', ' || venue.venue_name || ', ' || states.STATE || ', ' || venue.SUBURB as display_info " +
    ...
    */
      // If no day then omit '/'
        ...
        sqlString = 
          "SELECT DISTINCT concat(concat_ws(', ', events.event_name , venue.venue_name, venue.suburb, " + 
          "states.state," + 
          "concat_ws(' ', events.ddfirst_date, " + 
          "monthname(concat('2000-',events.mmfirst_date, '-01')), " + 
          "events.yyyyfirst_date))) as display_info " + "FROM events, venue, states " + 
          "WHERE eventid=" + p_event_id + " " + 
          "AND events.venueid = venue.venueid " + 
  ...
  public String getEventInfoForCIDisplay(int p_event_id, Statement p_stmt) {
	    ...
	    try {
	          ...
	          "FROM events Inner JOIN contentindicator ON (events.content_indicator = contentindicator.contentindicatorid) " + 
	          "WHERE eventid=" + p_event_id + " " ;

	     l_rs = m_db.runSQLResultSet(sqlString, p_stmt);

	      if (l_rs.next())
	        retStr = l_rs.getString("contentindicator");
  ...
  Parameters:
    p_event_id - The id of the event.

  Returns:
     A String with item information if the event is found to be accociated.
  */
  ...
  public ResultSet getAssociatedItems(int p_event_id, Statement p_stmt) {
    ...
    try {
      ...
      l_sql = 
          "SELECT item.itemid, item.catalogueid,  item.item_description, item.citation,  organisation.name,  states.state,  lookup_codes.code_lov_id,  lookup_codes.description,  lookup_codes.short_code,  `itemevlink`.`eventid` " +
  ...
  public ResultSet getAssociatedWorks(int p_event_id, Statement p_stmt) {
	    ...
	    try {
	      ...
	      l_sql = 
	          "SELECT events.eventid, work.work_title, work.workid " +
  	  ...
  	  private void loadLinkedEvents() {
  	    ...
  	    try {
  	      ...
  	      l_sql = 
  	          ...
  	          "FROM events " + 
  	          "LEFT JOIN eventeventlink on events.eventid = eventeventlink.eventid " + 
  	          "INNER join venue on events.venueid = venue.venueid   " + 
  	          "WHERE events.eventid=" +  m_eventid + " " +
