...
public class FormProcessManager {
			...
			if (rset.next()) {
				...
				if (is_non_group_subscription_exist) {
					if (AppConstants.DATABASE_TYPE == 1) {
						CallableStatement callable = db.m_conn.prepareCall("begin UPDATE_NAME_DATA_VALUES ('" + db.plSqlSafeString(name_val_buff) + "', '"
								+ db.plSqlSafeString(data_val_buff) + "'," + subs_id + "); end;");
						callable.execute();
						callable.close();
					} else {
						sqlString = "update subscribers SET name_value='" + db.plSqlSafeString(name_val_buff) + "', data_value='" + db.plSqlSafeString(data_val_buff)
	...
	public void setEmailBody(String display_name, String type_name, String html_name, String default_val, String default_val_from_DB, String default_display_val,
			String is_multiselect, String default_val_array[], boolean is_part_of_group, String group_id, boolean is_subscription_form) {
		...
		if (type_name.equals("textbox")) {
		...
		} else if (type_name.equals("textarea")) {
			if (!default_val.equals("")) {
		...
		} else if (type_name.equals("checkbox")) {
			if (is_part_of_group && default_val_array != null) {
				...
				htmlEmail += "<br></td></tr>";
			} else if (!default_val.equals("")) {
				textEmail += "\n" + display_name + " : " + default_val;
				htmlEmail += "\n<tr><td>" + display_name + " :</td><td>&nbsp;&nbsp;</td><td>" + default_val + "</td></tr>";
			} else if (is_part_of_group) {
				label = fem.getLabel(group_id, db);
				textEmail += "\n" + label + " : NONE.";
				htmlEmail += "\n<tr><td>" + label + " : NONE.</td><td>&nbsp;&nbsp;</td><td>";
			}
		} else if (type_name.equals("radiobutton")) {
			if (!default_val.equals("")) {
				label = fem.getLabel(group_id, db);
				if (label.equals("")) {
					textEmail += " LABEL_NOT_IN_GROUP ";
		...
		} else if (type_name.equals("pulldown")) {
			if (!default_val.equals("")) {
				textEmail += "\n" + display_name + " : " + default_val;
				htmlEmail += "\n<tr><td>" + display_name + ":</td><td>&nbsp;&nbsp;</td><td>" + default_val + "</td></tr>";
			}
		} else if (is_multiselect != null && is_multiselect.toUpperCase().equals("TRUE")) {
			if (default_val_array.length > 0) {
				...
				label0: for (int i = 0; i < default_val_array.length; i++) {
					int x = 0;
					do {
		...
		} else if (is_multiselect != null && !is_multiselect.equals("") && is_multiselect.toUpperCase().equals("FALSE") && !default_val.equals("")) {
			textEmail += "\n" + display_name + " : " + default_val;
			htmlEmail += "\n<tr><td>" + display_name + ":</td><td>&nbsp;&nbsp;</td><td>" + default_val + "</td></tr>";
		}
	}
	...
	public String formProcess(HttpServletRequest p_request, String p_url, String p_page_id) {
		...
		try {
			...
			if (rset.next()) {
				...
				for (int n = 0; n < feVector.size(); n++) {
					...
					if (is_multiselect != null && is_multiselect.toUpperCase().equals("TRUE")) {
						...
						default_val = "";
					} else if (!type_name.equals("label")) {
					...
					if (type_name.equals("radiobutton")) {
						...
						if (is_subscription_form) {
							...
							if (email_confirmation_required) {
								setEmailBody("", type_name, html_name, default_val, default_val_from_DB, default_display_val, is_multiselect, default_val_array, is_part_of_group,
										group_id, is_subscription_form);
							}
						} else {
							setEmailBody("", type_name, html_name, default_val, default_val_from_DB, default_display_val, is_multiselect, default_val_array, is_part_of_group,
									group_id, is_subscription_form);
					...
					if (type_name.equals("checkbox")) {
						...
						if (is_subscription_form) {
							...
							if (email_confirmation_required) {
								setEmailBody(display_name, type_name, html_name, default_val, default_val_from_DB, default_display_val, is_multiselect, default_val_array,
										is_part_of_group, group_id, is_subscription_form);
							}
						} else {
							setEmailBody(display_name, type_name, html_name, default_val, default_val_from_DB, default_display_val, is_multiselect, default_val_array,
									is_part_of_group, group_id, is_subscription_form);
			...
			if (!is_subscription_form) {
				if (!textEmailMessage.equals("") && textEmailMessage.indexOf("LABEL_NOT_IN_GROUP") == -1) {
					...
					if (rset.next()) {
						...
						try {
							...
							strConfirmationMsg = "<p" + contentoutput.applyStyle(4) + " align='" + formTableAlignment + "'>You have successfully sent an email to <b>"
									+ recipientNameList + "</b>" + singular_or_plural_recipient + " should receive your form information shortly.</p>";
						} catch (Exception e) {
							System.out.println("Trying to send an email - We have an exception!");
							System.out.println("MESSAGE: " + e.getMessage());
							System.out.println("LOCALIZED MESSAGE: " + e.getLocalizedMessage());
							System.out.println("CLASS.TOSTRING: " + e.toString());
							strConfirmationMsg = "<p " + contentoutput.applyStyle(4) + " align='" + formTableAlignment + "'>An error occurred while sending an email to <b>"
									+ recipientEmailAddress + "</b>." + "<br>Email sending was aborted.</p>";
						}
					} else {
						strConfirmationMsg = "<p" + contentoutput.applyStyle(4) + " align='" + formTableAlignment + "'>Email Sending Form Process did not execute,"
								+ " because there are no recipient found for this process.</p>";
					}
				} else {
					strConfirmationMsg = "<p" + contentoutput.applyStyle(4) + " align='" + formTableAlignment
							+ "'>Sending Email form processing did not execute due to following errors "
							+ "that may have occurred.<br><br>&nbsp;&nbsp;&nbsp;The form may have been incomple" + "te.<br>"
							+ "&nbsp;&nbsp;&nbsp;The form author may have created the form with errors.<br><br>"
							+ "Please go back and complete the form and if problem persists,<br>" + "please notify the owner of this website.</p>";
				}
			} else {
				...
				if (email_confirmation_required) {
					strConfirmationMsg = strConfirmationMsg + "<br>&nbsp;&nbsp;&nbsp; You should receive an email shortly confirming your subsc" + "ription details.</p>";
				} else {
			...
			System.out.println("LOCALIZED MESSAGE: " + e.getLocalizedMessage());
			System.out.println("CLASS.TOSTRING: " + e.toString());
			strConfirmationMsg = "<p " + contentoutput.applyStyle(4) + " align='" + formTableAlignment + "'>Form Process did not execute due to following errors "
					+ "that may have occurred.<br><br>&nbsp;&nbsp;&nbsp;The form may have been incomple" + "te.<br>"
					+ "&nbsp;&nbsp;&nbsp;The form author may have created the form with errors.<br><br>" + "Please go back and complete the form and if problem persists,<br>"
					+ "please notify the owner of this website.</p>";
		}
