...
public class CollInfoAccess {
	 ...
	 * Name: load ()
	 * 
	 * Purpose: Sets the class to a contain the CollInfoAccess information for
	 * the specified collection_info_access_id.
	 * 
	 * Parameters: p_id : id of the collection_info_access record
	 * 
	 * Returns: None
	 */
	public void load(int p_id) {
		...
		try {
			...
			sqlString = "SELECT * FROM collection_info_access WHERE " + "collection_info_access_id=" + p_id;
			...
			if (l_rs.next()) {
				...
				if (m_coll_access == null) m_coll_access = "";
			}
			l_rs.close();
			stmt.close();
		} catch (Exception e) {
	 ...
	 * id of the new record in the m_collection_info_access_id member.
	 */
	public boolean add() {
		try {
			Statement stmt = m_db.m_conn.createStatement();
	 ...
	 * Name: checkInUse ()
	 * 
	 * Purpose: Checks to see if the specified Collection Information Access
	 * record is in use in the database.
	 * 
	 * Parameters: p_id : id of the collection_info_access record
	 * 
	 * Returns: True if the collection_info_access record is in use, else false
	 */
	public boolean checkInUse(int p_id) {
		...
		try {
			...
			sqlString = "SELECT * FROM collection_information WHERE " + "collection_access=" + p_id;
			l_rs = m_db.runSQL(sqlString, stmt);
	 ...
	 * 
	 * Purpose: Returns a record set with all of the collection_info_access
	 * information in it.
	 * 
	...
	public void setCollAccess(String s) {
		m_coll_access = s;
	}

	public int getCollInfoAccessId() {
		return m_collection_info_access_id;
	}

	public String getCollAccess() {
		return m_coll_access;
	}
