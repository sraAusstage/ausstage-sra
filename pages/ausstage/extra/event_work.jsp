...
<%
  ...
  if (filter_id == null)
  {
    list_db_sql = "SELECT workid, work_title " +
                    "FROM work  " +
                   "ORDER BY work_title";
  }
  else
  {
    ...
    if (selected_db_sql.length() == 0)
      list_db_sql += " WHERE work.workid NOT IN (-1) " ;
    else
      list_db_sql += " WHERE work.workid NOT IN  (" + selected_db_sql + ") " ;   

    // Add the filters to the SQL
    if (!filter_id.equals (""))
      list_db_sql += "and workid=" + filter_id + " ";
    if (!filter_work_title.equals (""))
      list_db_sql += "and LOWER(work_title) like '%" + db_ausstage.plSqlSafeString(filter_work_title.toLowerCase()) + "%' ";
