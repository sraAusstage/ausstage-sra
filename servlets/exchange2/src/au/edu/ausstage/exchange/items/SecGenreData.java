 ...
 */
public class SecGenreData extends BaseData{
	 ...
	 *
	 */
	public SecGenreData(DbManager database, String[] ids, String outputType, String recordLimit, String sortOrder) {
	...
	public String getEventData() {
		...
		if(ids.length == 1) {
			...
			sql = "SELECT e.eventid, e.event_name, e.yyyyfirst_date, e.mmfirst_date, e.ddfirst_date, "
				...
				+ "inner join country c on v.countryid = c.countryid "
				+ "INNER JOIN states s ON v.state = s.stateid "
				+ "WHERE sl.secgenrepreferredid IN (";
	...
	public String getResourceData() {
		...
		if(ids.length == 1) {
			...
			sql = "SELECT i.itemid, i.citation, IFNULL(i.title, 'Untitled') "
				+ "FROM item i, itemsecgenrelink isl "
				+ "WHERE isl.itemid = i.itemid "
				+ "AND isl.secgenrepreferredid IN (";
