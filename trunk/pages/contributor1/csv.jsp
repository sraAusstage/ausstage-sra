<%@ page pageEncoding="UTF-8"%><%@ page contentType="text/csv; charset=UTF-8"%><%@ page import = "java.util.Vector, java.text.SimpleDateFormat"%><%@ page import = "java.sql.*, sun.jdbc.rowset.*, java.util.Calendar"%><%@ page import = "ausstage.State"%><%@ page import = "admin.Common"%><%@ page import = "ausstage.Event, ausstage.DescriptionSource"%><%@ page import = "ausstage.Datasource, ausstage.Venue, ausstage.ItemItemLink"%><%@ page import = "ausstage.PrimaryGenre, ausstage.SecGenreEvLink"%><%@ page import = "ausstage.Country, ausstage.PrimContentIndicatorEvLink"%><%@ page import = "ausstage.OrgEvLink, ausstage.Organisation, ausstage.Organisation"%><%@ page import = "ausstage.ConEvLink, ausstage.Contributor"%><%@ page import = "ausstage.Item, ausstage.LookupCode, ausstage.ContentIndicator"%><%@ page import = "ausstage.ItemContribLink, ausstage.ItemOrganLink"%><%@ page import = "ausstage.SecondaryGenre, ausstage.Work"%><%@ page import = "ausstage.WorkContribLink, ausstage.WorkEvLink,ausstage.WorkOrganLink" %><%@ taglib prefix="cms" uri="http://www.opencms.org/taglib/cms" %><%@ page session="true" import="org.opencms.main.*, org.opencms.jsp.*,org.opencms.file.*, java.lang.String, java.util.*" %><%
...
admin.AppConstants ausstage_search_appconstants_for_drill = new admin.AppConstants(request);%><%@ page import = "ausstage.AusstageCommon"%><%!
...
out.println("Event Name,Venue Name,Suburb,State,First Date");
  do{      
    ...
    sqlString = 
             ...
             "SELECT DISTINCT events.eventid,events.event_name,events.ddfirst_date,events.mmfirst_date, " + 
     ...
     if(l_rs.next()){
	  ...
	  out.println("Function,Event Name,Venue Name,Suburb,State,First Date");
 	   ...
 	   out.print(l_rs.getString("preferredterm")+","+l_rs.getString("event_name"));
     ...
     eo_rs = m_db.runSQL(sqlString2, stmt2);
      ...
      if(eo_rs.next()){
      	  out.println("\nEvents By Organisation");
          out.println("Organisation Name,Event Name,Venue Name,Suburb,State,First Date");

     	 do{
           out.print(eo_rs.getString("name")+","+eo_rs.getString("event_name"));
...
//Works
    ...
    //Contributor ID
    out.println("\nContributor Identifier," + contributor.getId());
