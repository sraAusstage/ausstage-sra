			...
			if (rset.next()) {
				...
				if (crset1.next()) {
					do {
						...
						sqlString = "update subscription_groups_rel set subs_id=" + new_subs_id + " where group_id=" + unApprovedGroupId + " and subs_id="
								+ crset1.getString("subs_id");
						...
						if (clobRset.next()) {
							...
							if (AppConstants.DATABASE_TYPE == 1) {
								CallableStatement callable = m_db.m_conn.prepareCall("begin UPDATE_NAME_DATA_VALUES ('" + m_db.plSqlSafeString(name_value) + "', '"
										+ m_db.plSqlSafeString(data_value) + "'," + new_subs_id + "); end;");
								callable.execute();
								callable.close();
							} else {
								sqlString = "update subscribers set name_value='" + m_db.plSqlSafeString(name_value) + "', data_value='" + m_db.plSqlSafeString(data_value)
										+ "' where subs_id=" + new_subs_id;
								m_db.runSQL(sqlString, stmt4);
							}
						}
					} while (crset1.next());
