...
public class PageTemplate
{
    ...
    public void writePageLocation(JspWriter p_out, String p_pageTitles[], String p_pageURLs[])
    {
        ...
        {
            System.out.println("An exception occurred when trying to access the JspWriter object : writePageLoca" +
"tion()"
);
        }
    }
            ...
            if(rset.next())
            {
                ...
                if(currentPageLevel.equals("3"))
                {
                    ...
                    if(rset.next())
                    {
                        ...
                        {
                            System.out.println("An error occurred when trying to access the database object : determineParentPag" +
"es()"
);
                        }
                    } else
...
"es()"
        ...
        {
            System.out.println("An exception occurred when trying to access the database object : determineParen" +
"tPages()"
);
        }
        db.closeDatabase();
    ...
    public void writeItemImage(JspWriter p_out, String p_image)
    {
        ...
        {
            System.out.println("An exception occurred when trying to access the JspWriter object : writeItemImag" +
"e()"
);
        }
    }
    ...
    public void writeItemTextAndImage(JspWriter p_out, String p_image, String p_image_width, String p_image_height, String p_text)
    {
        try
        {
            ...
            if(p_image != null && !p_image.equals(""))
            {
                ...
                {
                    p_out.println("<embed " + image_height + " " + image_width + " src='" + ITEM_IMAGE_LOCATION + "/" + p_image + "' quality='high' bgcolor='#FFFFFF' TYPE='application/x-shockwave-flash' PLUGINSP" +
"AGE='http://www.macromedia.com/shockwave/download/index.cgi?P1_Prod_Version=Shoc" +
"kwaveFlash'>"
);
                } else
                {
        ...
        {
            System.out.println("An exception occurred when trying to access the JspWriter object : writeItemText" +
"AndImage()"
);
        }
    }
    ...
    public void writeItemRelatedLinks(JspWriter p_out, String p_item_id)
    {
        try
        {
            ...
            Statement stmt = Database.m_conn.createStatement();
            db.connectDatabase();
            String sqlString = "select related_links.page_id as pageId, pages.title as pageTitle, pages.url as u" +
"rl from related_links, pages where related_links.pagi_id="
 + p_item_id + " and related_links.page_id=pages.page_id order by rell_id asc";
            ResultSet rset = db.runSQL(sqlString, stmt);
            String out_text;
            for(out_text = ""; rset.next(); out_text = out_text + ("<br><a class='altbluenet' id='altbluenet' href='" + ROOT_DIRECTORY + rset.getString("url") + "'>" + rset.getString("pageTitle") + "</a><br>")) { }
            rset.close();
            if(!out_text.equals(""))
            {
        ...
        {
            System.out.println("An exception occurred when trying to access the JspWriter object or database obj" +
"ect : writeItemRelatedLinks()"
);
        }
        db.closeDatabase();
    }
    ...
    public void writeItemEmailLinks(JspWriter p_out, String p_item_id)
    {
        try
        {
            ...
            ResultSet rset = db.runSQL(sqlString, stmt);
            String out_text;
            for(out_text = ""; rset.next(); out_text = out_text + ("<br><a class='altbluenet' id='altbluenet' href='mailto:" + rset.getString("address") + "'>" + rset.getString("description") + "</a><br>")) { }
            rset.close();
            if(!out_text.equals(""))
            {
                p_out.println(preAuxItemStruct());
    ...
    public void writeItemURLLinks(JspWriter p_out, String p_item_id)
    {
        try
        {
            ...
            for(out_text = ""; rset.next(); out_text = out_text + ("<br>" + rset.getString("description") + "<br>"))
            {
                ...
                out_text = out_text + ("<br><a name='" + rset.getString("title") + "' class='altbluenet' target='_blank' id='altbluenet' href='" + address + "'>" + rset.getString("title") + "</a>");
            }
...
"FROM periodicals WHERE type_lov_id='file' AND resource_sip=files.file_id) AS is_" +
"peri FROM files, file_types WHERE pagi_id="
 + p_item_id + " " + "AND files.filt_id=file_types.filt_id " + "ORDER BY files.title ASC";
            ResultSet rset = db.runSQL(sqlString, stmt);
            String out_text = "";
            while(rset.next()) 
            {
                if(rset.getString("is_peri").equals("0"))
                {
                    ...
                    {
                        out_text = out_text + "<br><table width='100%' border='0' cellspacing='0' cellpadding='0'><tr><td valig" +
"n='top' width='70'>"
;
                        out_text = out_text + ("<a name='" + rset.getString("title") + "' class='altbluenet' id='altbluenet' href=\"Javascript:viewXML('" + rset.getString("content") + "')\">");
                        out_text = out_text + ("<img alt='" + rset.getString("content") + " (" + rset.getString("file_size") + " bytes) " + rset.getString("extension") + "' border='0' src='" + IMAGE_DIRECTORY + "icons/" + rset.getString("icon") + "'>");
                        out_text = out_text + "</a></td><td valign='top' class='smallbody' width='225'>";
                        out_text = out_text + ("<a name='" + rset.getString("title") + "' class='altbluenet' id='altbluenet' href=\"Javascript:viewXML('" + rset.getString("content") + "')\">");
                        out_text = out_text + (rset.getString("title") + "</a><br>" + rset.getString("description") + " (" + Long.toString(Long.parseLong(rset.getString("file_size")) / (long)1024) + " kb)<br>");
...
"n='top' width='70'>"
;
                        out_text = out_text + ("<a name='" + rset.getString("title") + "' class='altbluenet' id='altbluenet' target='_blank' href='" + FILE_LOCATION + "/" + rset.getString("content") + "'>");
                        out_text = out_text + ("<img alt='" + rset.getString("content") + " (" + rset.getString("file_size") + " bytes) " + rset.getString("extension") + "' border='0' src='" + IMAGE_DIRECTORY + "icons/" + rset.getString("icon") + "'>");
                        out_text = out_text + "</a></td><td valign='top' class='smallbody' width='225'>";
                        out_text = out_text + ("<a name='" + rset.getString("title") + "' class='altbluenet' id='altbluenet' target='_blank' href='" + FILE_LOCATION + "/" + rset.getString("content") + "'>");
                        out_text = out_text + (rset.getString("title") + "</a><br>" + rset.getString("description") + " (" + Long.toString(Long.parseLong(rset.getString("file_size")) / (long)1024) + " kb)<br>");
...
;
                    ...
                    out_text = out_text + ("<a name='" + rset.getString("title") + "' class='altbluenet' id='altbluenet' href=\"Javascript:viewXML('" + rset.getString("content") + "')\">");
                    out_text = out_text + (rset.getString("title") + "</a><br>" + rset.getString("description") + " (" + Long.toString(Long.parseLong(rset.getString("file_size")) / (long)1024) + " kb)<br>");
                    out_text = out_text + ("</td><td><a href='" + ROOT_DIRECTORY + URL_SUBSCRIBE + "?id=" + rset.getString("file_id") + "&type=file'><img border='0' alt='Register your interest in \"" + rset.getString("title") + "\"' src='" + IMAGE_DIRECTORY + "icons/attention.gif'></a>&nbsp;&nbsp;</td></tr></table>");
                } else
                {
                    out_text = out_text + "<br><table width='100%' border='0' cellspacing='0' cellpadding='0'><tr><td valig" +
"n='top' width='70'>"
...
"ect : writeItemFiles()"
    ...
    public void writePageContentFooter(JspWriter p_out)
    {
        try
        {
            ...
            p_out.println("<table border='0' cellpadding='0' cellspacing='0' width='100%'><tr><td width='10" +
"0%'><img border='0' src='"
 + IMAGE_DIRECTORY + "inside/int_darkblue_spacer.gif' width='100%' height='1'></td>" + "</tr>" + "</table>");
        }
        catch(Exception e)
        {
            System.out.println("An exception occurred when trying to access the JspWriter object : writePageCont" +
"entFooter()"
);
        }
    }
...
">"
    ...
    public void writeSideMenu(JspWriter p_out, String p_page_id, String p_lvlOnePageId, String p_lvlTwoPageId, String p_lvlThreePageId)
    {
        try
        {
            ...
            String sqlString1 = "SELECT * FROM pages where page_level=2 and (is_approved is null or is_approved='" +
"t') and parent_id="
 + p_lvlOnePageId + " order by title, page_id asc";
            db.connectDatabase();
            ...
            {
                String sqlString2 = "SELECT * FROM pages where page_level=3 and (is_approved is null or is_approved='" +
"t') and parent_id="
 + rset2ndLevel.getString("page_id") + " order by title, page_id asc";
                ...
                {
                    p_out.println("&nbsp;<a class='sidemenu' name='anc" + divCounter + "' id='anc" + divCounter + "' href='Javascript:if (!NS4){toogleSideMenu (div" + divCounter + ", anc" + divCounter + ")}'>-</a>");
                    selectedDiv = true;
                } else
                {
                    p_out.println("&nbsp;<a class='sidemenu' name='anc" + divCounter + "' id='anc" + divCounter + "' href='Javascript:if (!NS4){toogleSideMenu (div" + divCounter + ", anc" + divCounter + ")}'>+</a>");
                ...
                for(; more3rdLevelRecs && rset3rdLevel.getString("parent_id").equals(rset2ndLevel.getString("page_id")); more3rdLevelRecs = rset3rdLevel.next())
                {
                    ...
                    {
                        p_out.println("<tr><td width='15'>&nbsp;</td><td><a class='sidemenuselectedsmall' href='" + ROOT_DIRECTORY + rset3rdLevel.getString("url") + "'>" + rset3rdLevel.getString("title") + "</a></td></tr>");
                    } else
                    {
                        p_out.println("<tr><td width='15'>&nbsp;</td><td><a class='sidemenusmall' href='" + ROOT_DIRECTORY + rset3rdLevel.getString("url") + "'>" + rset3rdLevel.getString("title") + "</a></td></tr>");
                    }
                }
...
"nterest in this <br>&nbsp;&nbsp;page, <a class='altbluenet' class='altbluenet' h" +
"ref='"
 + ROOT_DIRECTORY + URL_SUBSCRIBE + "?id=" + p_page_id + "&type=page'>click here ></a></td>");
...
"m_metadatas, resources, element where resources.resource_type='page' and resourc" +
"es.object_id="
 + p_page_id + " " + "and resources.reso_id=item_metadatas.reso_id " + "and item_metadatas.element_id=element.element_id";
            ResultSet rset;
            for(rset = db.runSQL(sqlString, stmt); rset.next(); p_out.println("<meta http-equiv=\"" + rset.getString("standard_id") + "." + rset.getString("name") + "\" content=\"" + rset.getString("content") + "\">")) { }
            rset.close();
            stmt.close();
        }
        catch(Exception e)
...
">"
            ...
            p_out.println("    \t\t<td width='544' align='left'>");
            for(; rset.next() && sectionCounter > 0; p_out.println("<a class='nav' href='" + ROOT_DIRECTORY + rset.getString("url") + "'> " + rset.getString("section_title") + "</a>&nbsp;&nbsp;"))
            {
                sectionCounter--;
            }
...
"'>"
            ...
            p_out.println("  </tr>");
            p_out.println("  <tr>");
            p_out.println("    <td width='190' bgcolor='#C2D6E9'><img border='0' src='" + IMAGE_DIRECTORY + "spacer.gif' alt='navigation design element' WIDTH='190' HEIGHT='1'></td>");
            p_out.println("  </tr>");
            p_out.println("  <tr>");
            p_out.println("    <td>");
            p_out.println("    </td>");
...
"'>"
            ...
            p_out.println("  <table border='0' cellpadding='0' cellspacing='0' width='100%' bgcolor='#FFFFF" +
"F'>"
);
            p_out.println("  <tr>");
...
"sclaimer</a>&nbsp;&nbsp;<a href='/public/content/show_input.asp?xcid=10'>subscri" +
"be</a></td>"
);
