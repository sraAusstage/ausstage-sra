...
//delete me
  ...
  */
  ...
  Parameters:
    p_id : id of the Contributor record
...
*******/
        ...
        sqlString = "select CONTRIBUTORFUNCTPREFERREDID from " +
                    "CONTFUNCTLINK where CONTRIBUTORID=" + p_id;
      ...
      if (m_m_name == null)     {m_m_name = "";}
      if (m_l_name == null)     {m_l_name = "";}
      if (m_suffix == null)     {m_suffix = "";}
	  if (m_display_name == null){m_display_name = "";}
	  if (m_place_of_birth == null){m_place_of_birth = "";}
      if (m_place_of_death == null){m_place_of_death = "";}
      if (m_other_names == null){m_other_names = "";}
      if (m_gender_id == null)  {m_gender_id = "";}
      if (m_dob_d == null)      {m_dob_d = "";}
      if (m_dob_m == null)      {m_dob_m = "";}
      if (m_dob_y == null)      {m_dob_y = "";}
      if (m_nationality == null){m_nationality = "";}
      if (m_address == null)    {m_address = "";}
      if (m_town == null)       {m_town = "";}
      if (m_state == null)      {m_state = "";}
      if (m_post_code == null)  {m_post_code = "";}
      if (m_phone == null)      {m_phone = "";}
      if (m_email == null)      {m_email = "";}
      if (m_notes == null)      {m_notes = "";}
      if (m_nla == null)        {m_nla = "";}
      if (m_dod_d == null)      {m_dod_d = "";}
      if (m_dod_m == null)      {m_dod_m = "";}
      if (m_dod_y == null)      {m_dod_y = "";}
      if (m_country_id == null) {m_country_id = "";}
      if (m_weblink == null)    {m_weblink = "";}    
      if(m_contfunct_ids == null){m_contfunct_ids = "";}
 ...
 // public String getUpdatedByUser(){return m_updated_by_user;}
  ...
  public Vector getContributors(){return m_contrib_contriblinks;}
   ...
   */
   private void loadOrganisationLinks()
   {
     ...
     try
     {
       ...
       sqlString = "SELECT organisationid, description " +
                     "FROM CONORGLINK " +
   ...
   private void loadLinkedContributors()
   {
     ...
     try
     {
       ...
       l_sql = "SELECT DISTINCT childid " +
                ...
                "WHERE contribcontriblink.contributorid =" + m_id;
 ...
 /*
  ...
  Returns:
     ...
     new record in the Contributor member.
  ...
  */
  public boolean add ()
  {
    try
    {
      ...
      if(m_notes.length() >= 600)
        m_notes = m_notes.substring (0, 599);

      // Check to make sure that it can be added
      if (validateObjectForDB ())
      {
    	  System.out.println("In add 1");
        sqlString = "INSERT INTO Contributor " + 
                    "(PREFIX, FIRST_NAME, MIDDLE_NAME, LAST_NAME, SUFFIX, DISPLAY_NAME, OTHER_NAMES, " +
                    "GENDER, DDDATE_OF_BIRTH, MMDATE_OF_BIRTH, " +
                    "YYYYDATE_OF_BIRTH, PLACE_OF_BIRTH, PLACE_OF_DEATH, NATIONALITY, ADDRESS, " +
                    "SUBURB, STATE, POSTCODE, " +
                    "EMAIL, NOTES, NLA, DDDATE_OF_DEATH, MMDATE_OF_DEATH, " + 
                    "YYYYDATE_OF_DEATH, COUNTRYID, entered_by_user, entered_date) " + 
                    "VALUES (" +
                    ...
                    + Integer.parseInt(m_state) + ","  + m_db.plSqlSafeString(m_post_code.equals("")?"null":m_post_code) + ","  +
        ...
        if(!m_contfunct_ids.equals("")){
          if(m_contfunct_ids.indexOf(",") != -1){
            ...
            while(contribfunct_ids.hasMoreTokens()){
              sqlString = "Insert into contfunctlink(CONTRIBUTORID, CONTRIBUTORFUNCTPREFERREDID) values " +
                          "(" + m_id + "," + contribfunct_ids.nextToken() + ")";
              m_db.runSQL (sqlString, stmt); 
            }
          }
          else {
  ...
  */
  public boolean update ()
  {
    ...
    try{
      ...
      // trim the notes first to db specified max chars (-1) before updating
      if(m_notes.length() >= 600)
        m_notes = m_notes.substring (0, 599);
        
        if(m_country_id == null || m_country_id.equals("") || m_country_id.equals("0"))  { m_country_id = "null"; }
        if(m_state == null      || m_state.equals("")      || m_state.equals("0"))       { m_state      = "null"; }
        if(m_gender_id == null  || m_gender_id.equals("")  || m_gender_id.equals("0"))   { m_gender_id  = "null"; }
        if(m_post_code == null  || m_post_code.equals("")  || m_post_code.equals("0"))   { m_post_code  = "null"; }
        if(m_place_of_birth == null  || m_place_of_birth.equals("")  || m_place_of_birth.equals("0"))   { m_place_of_birth  = "null"; }
        if(m_place_of_death == null  || m_place_of_death.equals("")  || m_place_of_death.equals("0"))   { m_place_of_death  = "null"; }
  ...
  */
  ...
  public CachedRowSet getNames(){
    ...
    try{
      Statement stmt = m_db.m_conn.createStatement ();
      sqlString = "select CONTRIBUTORID, FIRST_NAME, LAST_NAME from CONTRIBUTOR " +
                  "order by LAST_NAME";
      l_rs = m_db.runSQL(sqlString, stmt);
      stmt.close();
    }catch(Exception e){
  ...
  public CachedRowSet getContFunctPreff(int contFunctPreffId){
    ...
    try{
      ...
      sqlString = "SELECT * FROM ContributorFunctPreferred " +
                  "where ContributorFunctPreferredid=" + contFunctPreffId;
      l_rs = m_db.runSQL(sqlString, stmt);
      stmt.close();
    }catch(Exception e){
  ...
  public String getContFunctPreffTermByContributor(String p_id){
    ...
    try{
      Statement stmt = m_db.m_conn.createStatement ();
      sqlString = "SELECT PreferredTerm FROM ContFunctLink, ContributorFunctPreferred where " +
                  "ContFunctLink.contributorid=" + p_id + " " + 
                  "and ContFunctLink.ContributorFunctPreferredid=" +
                  "ContributorFunctPreferred.ContributorFunctPreferredid";
  ...
  */
  ...
  Purpose: Checks to see if a Contributor already exist (by first and last names)

  Parameters:
     None
     
  Returns:
     True if the contributor already exists, else false
  ...
  */
  public boolean doesContributorAlreadyExist ()
  {
    ...
    {      
      ...
      // Check to see if a Contributor record exists (same last and first names)
  ...
  public void setContributorContributorLinks(Vector p_contrib_contriblinks) { m_contrib_contriblinks = p_contrib_contriblinks;}
  ...
  public String getContributorInfoForContributorDisplay(int p_contrib_id, Statement p_stmt) {
	    ...
	    try {
	      ...
	      sqlString = 
	          "SELECT contributor.contributorid, concat_ws(' ', last_name, first_name) AS name " + 
	          "FROM contributor "+
	          "LEFT OUTER JOIN contribcontriblink ON (contributor.contributorid = contribcontriblink.CONTRIBUTORID) "+
	          "WHERE contributor.contributorid=" + p_contrib_id;

	      l_rs = m_db.runSQLResultSet(sqlString, p_stmt);

	      if (l_rs.next())
	        retStr = l_rs.getString("name");
   ...
   public ResultSet getContributorsByItem(int p_contributor_id, Statement p_stmt){
    ...
    try{
      ...
      sqlString = "SELECT DISTINCT contributor.contributorid, display_name as name, preferredterm " +
                    "FROM contributor, itemconlink, contributorfunctpreferred, contfunctlink " +
                   "WHERE itemconlink.contributorid=" + p_contributor_id + " " + 
                     "AND itemconlink.contributorid = contributor.contributorid " +
                     "AND contributor.contributorid = contfunctlink.contributorid " +
...
//////////////////////////////////////////////////////////
  ...
  */
  public void setContributorAttributes (HttpServletRequest request)
  {
    ...
    // allow posting from contributor to organisations and retain the contributor functions
  ...
  */
  ...
  public ResultSet getAssociatedItems(int p_contributor_id, Statement p_stmt){
    ...
    try{
      ...
      l_sql = "SELECT contributor.last_name,contributor.first_name, contributor.contributorid,item.itemid,item.citation " +
                          " FROM item " +
  ...
  */
  ...
  public ResultSet getAssociatedWorks(int p_contributor_id, Statement p_stmt){
    ...
    try{
      ...
      l_sql = "SELECT DISTINCT w.workid, WORK_TITLE " +
                         "FROM work w, workconlink wcl " +
                        "WHERE wcl.contributorid=" + p_contributor_id + " " + 
