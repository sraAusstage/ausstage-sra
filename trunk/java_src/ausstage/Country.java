...
public class Country {
	...
	public boolean update() {
		try {
			...
			// required fields
			if (validateObjectForDB()) {
				sqlString = "UPDATE country set countryname='" + m_db.plSqlSafeString(m_country_name) + "' " + "where countryid=" + m_country_id;
				m_db.runSQL(sqlString, stmt);
				l_ret = true;
			}
			stmt.close();
	...
	public boolean checkInUse(int p_id) {
		...
		try {
			...
			// Venue
			sqlString = "SELECT * FROM venue WHERE " + "countryid=" + p_id;
			l_rs = m_db.runSQL(sqlString, stmt);
			...
			// Organisation
			sqlString = "SELECT * FROM organisation WHERE " + "countryid=" + p_id;
			l_rs = m_db.runSQL(sqlString, stmt);
			...
			// Contributor
			sqlString = "SELECT * FROM contributor WHERE " + "countryid=" + p_id;
			l_rs = m_db.runSQL(sqlString, stmt);
