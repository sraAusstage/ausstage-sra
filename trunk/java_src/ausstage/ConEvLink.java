...
public class ConEvLink
{
  ...
  */
  ...
  Parameters:
    eventId
    String array of ConEvLink records.
  ...
  */
  ...
  Purpose: Modifies this object in the database by deleting all ConEvLinks for this
           Event and inserting new ones from an array of ConEvLink records to link
           to this Event.
  ...
  */
  public boolean update (String eventId, Vector conEvLinks)
  {
    try
    {
      ...
      sqlString = "DELETE FROM ConEvLink where " +
                  "eventId=" + eventId;
      m_db.runSQL (sqlString, stmt);

      for (int i=0; conEvLinks != null && i < conEvLinks.size(); i++){
        conEvLink = (ConEvLink)conEvLinks.get(i);
        sqlString = "INSERT INTO ConEvLink "    +
                    "(eventId, contributorId, function, notes) " +
                    "VALUES (" +
                    eventId + ", " + conEvLink.getContributorId() + ", " + conEvLink.getFunction() + ", '" +
                    conEvLink.getNotes() + "')";
        m_db.runSQL (sqlString, stmt);
  ...
  */
  ...
  Purpose: Returns a record set with all of the ConEvLinks information in it.
  ...
  */
  public CachedRowSet getConEvLinks(int eventId)
  {
    ...
    try
    {
      ...
      sqlString = "SELECT ConEvLink.*, ContributorFunctPreferred.* " +
      		" FROM ConEvLink, ContributorFunctPreferred, contributor " +
      		" WHERE ConEvLink.eventId  = " + eventId +
      		" AND ConEvLink.function = ContributorFunctPreferredId" +
      		" AND ConEvLink.CONTRIBUTORID = contributor.CONTRIBUTORID"+
  ...
  Returns:
     A Vector of all ConEvLinks for this Event.
  */
...
"Left JOIN contributorfunctpreferred ON (conevlink.`function` = contributorfunctpreferred.contributorfunctpreferredid) where contributor.contributorId = " + this.getContributorId();
  ...
  public void setContributorBean (Contributor c)          {contributorBean  = c;}
  public void setContFunctBean   (ContributorFunction cf) {contFunctBean    = cf;}
 ...
 // public String              getFunctionDescId  () {if(functionDescid == null) functionDescid =""; return (functionDescid);}
  public String              getNotes           () {return (notes);}
  public Contributor         getContributorBean () {return (contributorBean);}
  public ContributorFunction getContFunctBean   () {return (contFunctBean);}
  public String              getError           () {return (m_error_string);}

  public boolean sameContributorAndFunction (ConEvLink conEvLink)
  {
    if (this.toLongString().equals(conEvLink.toLongString()))
      return true;
    return false;
  }
