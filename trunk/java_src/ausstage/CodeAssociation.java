...
public class CodeAssociation {
  ...
  */
  ...
  // containing the code_lov_ids and the other containing the description for the code_lov_id
  public static ArrayList getAssociatedLookupCodes(final String code1LovId, final String code2Type,final ausstage.Database p_db){
    ...
    try {
      ...
      sqlString = "SELECT CODE_2_LOV_ID, lookup_codes.DESCRIPTION " + 
                  "FROM CODE_ASSOCIATIONS, lookup_codes " +
                  "WHERE code_1_lov_id= " + code1LovId;
      // only use code 2 type if it is supplied
      if (!code2Type.equals("")){
        sqlString += "  and code_2_type='" + code2Type + "'";
      }
      sqlString += "  and CODE_ASSOCIATIONS.CODE_2_LOV_ID=lookup_codes.CODE_LOV_ID " +
  ...
  // creates populates fields for the code association object from an input code_1_type
  public void load(final String code1LovId){
    ...
    try {
      ...
      // query will return unique values
      sqlString = "SELECT CODE_1_TYPE, CODE_1_LOV_ID,  " + 
                  "CODE_2_TYPE, CODE_2_LOV_ID " +
                  "FROM CODE_ASSOCIATIONS " +
                  "WHERE code_1_lov_id= " + code1LovId;
      ...
      if (l_rs.next()) {
        ...
        // while there are records to process, loadd the code 2 lov ids into the string array
        while (l_rs.next()){
          //arlCode2LovIds.add(new Integer(l_rs.getInt("CODE_2_LOV_ID")));
          m_code_2_lov_ids[recordCount] = ("" + l_rs.getInt("CODE_2_LOV_ID"));
           recordCount++;
        }
      } else {
  ...
  Returns:
    True if the add was successful, else false. 

  */
  //public boolean add (final String code1Type, final String code1LovId, final String code2Type, final ArrayList codeLovIds)
  public boolean add ()
  {
    boolean   ret = false;
    try {
      ...
      sqlInsertString = "INSERT INTO CODE_ASSOCIATIONS " + 
                        "(CODE_1_TYPE, CODE_1_LOV_ID, CODE_2_TYPE, CODE_2_LOV_ID) ";
      ...
      for (int i = 0; i < m_code_2_lov_ids.length; i ++){
        ...
        sqlString = "VALUES (" +
                    "'" + m_db.plSqlSafeString(m_code_1_type) + "'" + ", "  +
                    m_code_1_lov_id + ", " +
                    "'" + m_db.plSqlSafeString(m_code_2_type) + "'" + ", "  +
                    codeLovId.intValue() + ") ";
  ...
  */
  public void delete () {
    ...
    try
    {
      ...
      sqlString = "DELETE from code_associations " + 
                  "WHERE code_1_type= '" + m_db.plSqlSafeString(m_code_1_type) + "'" +
                  "  AND code_1_lov_id = " + m_code_1_lov_id;
