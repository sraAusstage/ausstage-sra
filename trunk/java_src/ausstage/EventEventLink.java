...
public class EventEventLink
{
  ...
  */
  ...
  Purpose: Sets the class to a contain the EventEventLink information for the
           specified EventEventLink id.

  Parameters:
    p_event_id  : id of the main Event record
  ...
  */
  public void load(String p_eventeventlink_id)
  {
    ...
    try
    {
      ...
      sqlString = " SELECT * FROM EventEventLink, lookup_codes " +
                  " WHERE eventeventlinkId = " + p_eventeventlink_id +
                  "   AND EventEventLink.function_lov_id=lookup_codes.code_lov_id";
  ...
  Parameters:
    eventId
    String array of child Event ids.
  ...
  */
  ...
  Purpose: Modifies this object in the database by deleting all EventEventLinks for this
           event and inserting new ones from an array of p_childLinks.
  ...
  */
  ...
  {
	  //System.out.println("In update:" + p_childLinks + ", Event Id:"+ p_eventId);
    try
    {
      Statement stmt = m_db.m_conn.createStatement ();
      String       sqlString;
      ...
      sqlString = "DELETE FROM EventEventLink where " +
                  "eventId=" + p_eventId;
      m_db.runSQL (sqlString, stmt);
      if (p_childLinks != null) {
    	  //System.out.println("Event:" + p_childLinks + ", Event Id:"+ p_eventId);
	      for (int i=0; i < p_childLinks.size(); i++){
	    	  try{
	        sqlString = "INSERT INTO EventEventLink "    +
	                    "(eventId, childId, function_lov_id) " +
	                    "VALUES (" +
	                    p_eventId + ", " + ((EventEventLink)p_childLinks.get(i)).getChildId() + ", null)";	        
	      }catch(Exception e){
	    	  sqlString = "INSERT INTO EventEventLink "    +
              "(eventId, childId, function_lov_id) " +
              "VALUES (" +
              p_eventId + ", " + p_childLinks.get(i) + ", null)";
  ...
  */
  ...
  Purpose: Returns a record set with all of the EventEventLinks information in it.
