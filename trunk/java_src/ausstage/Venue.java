...
public class Venue
{
  ...
  */
  ...
  public Vector getVenues(){return m_venue_venuelinks;}  
  ...
  Returns:
     ...
     new record in the m_venue_id member.
    ...
    try
    {
      ...
      if (validateObjectForDB ())
      {
        ...
        sqlString += ") VALUES (" +
                    ...
                    "'" + m_db.plSqlSafeString(m_street) + "','" + m_db.plSqlSafeString(m_suburb) + "','" + m_db.plSqlSafeString(m_postcode) + "'," +
                    Integer.parseInt(m_state_id) + "," + strCapacity + "," +
                    "'" + m_db.plSqlSafeString(m_contact) + "','" + m_db.plSqlSafeString(m_phone) + "'," +
                    "'" + m_db.plSqlSafeString(m_fax) + "','" + m_db.plSqlSafeString(m_email) + "'," +
                    "'" + m_db.plSqlSafeString(m_web_links) + "','" + m_db.plSqlSafeString(m_notes) + "'," + m_country + "," +
        ...
        if (!m_longitude.equals (""))
          sqlString += ",'" + m_longitude + "'";
        if (!m_latitude.equals (""))
          sqlString += ",'" + m_latitude + "'";
        if (!m_radius.equals (""))
          sqlString += ",'" + m_radius + "'";
        if (!m_elevation.equals (""))
          sqlString += ",'" + m_elevation + "'";
  ...
  public boolean update ()
  {
    try
    {
      ...
      if (validateObjectForDB ())
      {
        ...
        sqlString = "UPDATE VENUE set VENUE_NAME='" + m_db.plSqlSafeString(m_venue_name) + "', " +
                    "STREET='" + m_db.plSqlSafeString(m_street) + "', " +
                    "SUBURB='" + m_db.plSqlSafeString(m_suburb) + "', STATE=" + Integer.parseInt(m_state_id) + ", " +
                    "POSTCODE='" + m_db.plSqlSafeString(m_postcode) + "'," + 
                    " updated_BY_USER = '" + m_db.plSqlSafeString(m_updated_by_user) + "'" +
                    ", updated_DATE =  now()  " +
                    ",CAPACITY=" + strCapacity + " , CONTACT='" + m_db.plSqlSafeString(m_contact) + "', " +
                    "PHONE='" + m_db.plSqlSafeString(m_phone) + "', FAX='" + m_db.plSqlSafeString(m_fax) + "', EMAIL='" + m_db.plSqlSafeString(m_email) + "', " +
                    "WEB_LINKS='" + m_db.plSqlSafeString(m_web_links) + "', NOTES='" + m_db.plSqlSafeString(m_notes) + "', COUNTRYID=" + m_country + ", "+
    ...
    catch (Exception e)
    {
     m_error_string = e.getMessage();
      //m_error_string = "Unable to update the venue. The data may be invalid.";
      return (false);
    }
  }
  ...
  */
  ...
  Returns:
     True if the venue is in use, else false
  ...
  */
  public boolean checkInUse(int p_id)
  {
    ...
    {      
      ...
      sqlString = "SELECT * FROM events WHERE " +
                  "venueid=" + p_id;
      l_rs = m_db.runSQL (sqlString, stmt);
  ...
  */
  private boolean validateObjectForDB ()
  {
    ...
    {      
      ...
      if(! admin.Common.checkIfNumber(m_postcode)){
          m_error_string = "Postcode must be a number.";
          l_ret = false;
      }*/
      //System.out.println("2");
      // Default
      if (m_capacity.equals (""))
        m_capacity = "null";

      // Check to see if a Venue record exists (same name and state) 
      sqlString = "select venueid from venue where venue_name='" + m_db.plSqlSafeString(m_venue_name) + "' and state=" + m_state_id +
                  " and suburb='" + m_db.plSqlSafeString(m_suburb) + "' and venueid!=" + m_venue_id;
      l_rs = m_db.runSQL (sqlString, stmt);
  ...
  public String getVenueInfoForVenueDisplay(int p_venue_id, Statement p_stmt) {
	    ...
	    try {
	      ...
	      sqlString = 
	    	    "select venueid, venue_name, street , suburb ,states.state as state,country.countryname, "+
  ...
  public String getVenueInfoForItemDisplay(int p_venue_id, Statement p_stmt){
    ...
    try{
      ...
      sqlString = "SELECT DISTINCT concat_ws(', ', venue.venue_name, venue.street, venue.suburb, states.state) as display_info " +
                    "FROM venue, states " +
                   "WHERE venue.venueid=" + p_venue_id + " " + 
   ...
   public ResultSet getVenuesByItem(int p_venue_id, Statement p_stmt){
    ...
    try{
      ...
      sqlString = "SELECT DISTINCT venue.venueid, venue_name " +
                    "FROM venue, itemvenuelink " +
                   "WHERE itemvenuelink.venueid=" + p_venue_id + " " + 
 ...
 private void loadLinkedVenues()
 {
   ...
   try
   {
     ...
     l_sql = "SELECT DISTINCT childid " +
               "FROM venuevenuelink " +
              "WHERE venuevenuelink.venueid =" + m_venue_id;
  ...
  Parameters:
    p_venue_id - The id of the venue.

  Returns:
     A ResultSet with item information if the venue is found to be accociated.
  */
  ...
  public ResultSet getAssociatedItems(int p_venue_id, Statement p_stmt){
    ...
    try{
      ...
      l_sql = "SELECT DISTINCT item.CITATION,item.ITEMID,venue.VENUE_NAME,venue.venueid  " +
                          " FROM item " +
                          " INNER JOIN itemvenuelink ON (item.ITEMID = itemvenuelink.ITEMID) " +
                          " INNER JOIN venue ON (itemvenuelink.VENUEID = venue.VENUEID)"+
                          " WHERE itemvenuelink.venueid=" + p_venue_id + " " + 
                          " AND itemvenuelink.itemid = item.itemid ";
