...
public class Organisation {
	 ...
	 * 
	 * Parameters: p_id : id of the organisation record
	 * 
	 * Returns: None
	 */
	public void load(int p_id) {
		...
		try {
			...
			sqlString = "SELECT * FROM organisation " + "WHERE organisationId=" + p_id;
			...
			if (l_rs.next()) {
				...
				if (m_place_of_origin == null) {
					m_place_of_origin = "";
				}
				if (m_place_of_demise == null) {
					m_place_of_demise = "";
				}
				if (m_nla == null) {
					m_nla = "";
				...
				if (m_state_id == null) {
					...
					sqlString = "SELECT state FROM states " + "WHERE stateId=" + m_state_id;
	 ...
	 * id of the new record in the m_organisation_id member.
	 */
	public boolean add() {
		try {
			Statement stmt = m_db.m_conn.createStatement();
			...
			if (validateObjectForDB()) {
				...
				sqlString = "INSERT INTO organisation (name, other_names1, other_names2, " + "other_names3, address, suburb, state, contact, postcode, phone1, phone2, phone3, "
						...
						+ "ddfirst_date, mmfirst_date, yyyyfirst_date, ddlast_date, mmlast_date, yyyylast_date" + ", PLACE_OF_ORIGIN, PLACE_OF_DEMISE, nla) VALUES (" + "'"
						+ m_db.plSqlSafeString(m_organisation_name)
						+ "','"
						...
						+ "',"
						+ "'"
						+ m_db.plSqlSafeString(m_phone2)
						+ "','"
						+ m_db.plSqlSafeString(m_phone3)
						+ "',"
						+ "'"
						+ m_db.plSqlSafeString(m_fax)
						+ "','"
						+ m_db.plSqlSafeString(m_email)
						+ "',"
						+ "'"
						+ m_db.plSqlSafeString(m_web_links)
						+ "','"
	...
	public boolean update() {
		try {
			...
			if (validateObjectForDB()) {
				// As the notes is a text area, need to limit characters
				if (m_notes.length() >= 300) m_notes = m_notes.substring(0, 299);
				if (m_country == null || m_country.equals("") || m_country.equals("0")) {
					m_country = "null";
				}
				if (m_organisation_type == null || m_organisation_type.equals("")) {
					m_organisation_type = "1";
				}
				if (m_place_of_origin == null || m_place_of_origin.equals("") || m_place_of_origin.equals("0")) {
					m_place_of_origin = "null";
				}
	 ...
	 * Name: checkInUse ()
	 * 
	 * Purpose: Checks to see if the specified Organisation is in use in the
	 * database.
	 * 
	 * Parameters: p_id : id of the organisation record
	 * 
	 * Returns: True if the organisation is in use, else false
	 */
	public boolean checkInUse(int p_id) {
		...
		try {
			...
			sqlString = "SELECT orgevlinkid FROM orgevlink WHERE " + "organisationid=" + p_id;
			l_rs = m_db.runSQL(sqlString, stmt);

			if (l_rs.next()) ret = true;
			l_rs.close();

			sqlString = "SELECT workorglinkid FROM workorglink WHERE " + "organisationid=" + p_id;
			l_rs = m_db.runSQL(sqlString, stmt);
			if (l_rs.next()) ret = true;
			l_rs.close();

			sqlString = "SELECT conorglinkid FROM conorglink WHERE " + "organisationid=" + p_id;
			l_rs = m_db.runSQL(sqlString, stmt);
			if (l_rs.next()) ret = true;
			l_rs.close();

			sqlString = "SELECT itemorglinkid FROM itemorglink WHERE " + "organisationid=" + p_id;
			l_rs = m_db.runSQL(sqlString, stmt);
			if (l_rs.next()) ret = true;
			l_rs.close();
	...
	public ResultSet getOrgTypes(Statement p_stmt) {
		...
		try {
			...
			l_sql = "SELECT organisation_type.organisation_type_id, organisation_type.type " + "FROM organisation_type ";
	...
	public Vector generateDisplayInfo(Vector p_ids, String p_id_type, Statement p_stmt) {
		...
		for (int i = 0; i < p_ids.size(); i++) {
			if (p_id_type.equals("organisation")) {
				...
				int childOrganisationId = Integer.parseInt(((OrganisationOrganisationLink) m_org_orglinks.elementAt(i)).getChildId());
				l_info_to_add = childOrganisation.getOrganisationInfoForOrganisationDisplay(childOrganisationId, p_stmt);
	...
	public void setOrganisationAttributes(HttpServletRequest request) {
		...
		if (m_error_string == null) m_error_string = "";
	...
	// Utility methods
	// //////////////////////
	// public String getOrganisationInfoForItemDisplay(int p_item_id, Statement
	// p_stmt) {
	public String getOrganisationInfoForItemDisplay(int p_org_id, Statement p_stmt) {
		...
		try {
			...
			// sqlString =
			// "select citation display_info " + "FROM item i " +
			// " WHERE i.itemid = " +
			// p_item_id;
			...
			sqlString = "SELECT organisation.organisationid, name, if(min(events.yyyyfirst_date) = max(events.yyyylast_date),min(events.yyyyfirst_date),concat(min(events.yyyyfirst_date),' - ', max(events.yyyylast_date))) dates, "
					...
					+ "LEFT JOIN events ON (orgevlink.eventid = events.eventid)  " + "WHERE organisation.organisationid=" + p_org_id;
	...
	public String getOrganisationInfoForWorkDisplay(int p_work_id, Statement p_stmt) {
		...
		try {
			...
			sqlString = "SELECT work.workid, work.work_title,organisation.organisationid,organisation.name " + "FROM work "
					+ "INNER JOIN workorglink ON (workorglink.workid = work.workid) " + "INNER JOIN organisation ON (organisation.organisationid = workorglink.organisationid) "
					+ "WHERE work.workid=" + p_work_id;

			l_rs = m_db.runSQLResultSet(sqlString, p_stmt);

			if (l_rs.next()) retStr = l_rs.getString("organisation.name");
	 ...
	 * 
	 * Purpose: Returns a record set with all of the educational organisations
	 * in it.
	 * 
	 * Parameters: p_stmt : Database statement p_org_type : organisation_type
	 * 
	...
	public ResultSet getAssociatedWorks(int p_org_id, Statement p_stmt) {
		...
		try {
			...
			l_sql = "SELECT DISTINCT w.workid, WORK_TITLE, ORDER_BY " + "FROM work w, workorglink wcl " + "WHERE wcl.ORGANISATIONID=" + p_org_id + " "
					+ "AND wcl.workid = w.workid " + "ORDER BY WORK_TITLE";
