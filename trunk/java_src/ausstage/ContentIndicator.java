...
public class ContentIndicator
{
  ...
  */
  ...
  Purpose: Sets the class to a contain the ContentIndicator information for the
           specified Content Indicator ID.
  ...
  public boolean add ()
  {
    ...
    try
    {
      ...
      if (validateObjectForDB ())
      {
       ...
       sqlString = "INSERT INTO CONTENTINDICATOR " + 
                    "(CONTENTINDICATOR, CONTENTINDICATORDESCRIPTION, SECGENREPREFERREDID) " + 
                    "VALUES (" +
    ...
    try
    {
      ...
      if (validateObjectForDB ())
      {
        ...
        sqlString = "UPDATE ContentIndicator " + 
                    ...
                    "CONTENTINDICATORSDESCRIPTION = '" + m_db.plSqlSafeString(m_description) + "' " + 
...
*/
  ...
  public String getContentIndInfoForItemDisplay(int p_contentindicator_id, Statement p_stmt){
    ...
    try{
      ...
      sqlString = "SELECT CONTENTINDICATOR display_info " + 
                  "  FROM CONTENTINDICATOR " + 
                  " WHERE CONTENTINDICATORID= " + p_contentindicator_id;
  ...
  public ResultSet getAssociatedEvents(int p_id, Statement p_stmt){
	    ...
	    try{
	      ...
	      l_sql = "SELECT DISTINCT events.eventid, events.event_name, venue_name, venue.suburb, if(states.state='O/S', country.countryname, states.state) state,  " +
	                          ...
	                          "WHERE events.venueid = venue.venueid and events.content_indicator=" + p_id + " order by datesort DESC";
  	...
  	public ResultSet getAssociatedItems(int p_id, Statement p_stmt){
	    ...
	    try{
	      ...
	      l_sql = "SELECT item.itemid,item.CITATION as title,contentindicator.contentindicator,item.catalogueid  " +
	      					  "FROM item "+
