...
public class Work
{
    ...
    public boolean addWork()
    {
        ...
        try
        {
            ...
            if(!l_rs.next())
            {
                l_sql = "INSERT INTO work (WORK_TITLE, ALTER_WORK_TITLE, entered_by_user, entered_date) V" +
"ALUES ("
;
                l_sql = (new StringBuilder(String.valueOf(l_sql))).append("'").append(m_db.plSqlSafeString(m_work_title)).append("', ").append("'").append(m_db.plSqlSafeString(m_alter_work_title)).append("', ").append("'").append(m_db.plSqlSafeString(m_entered_by_user)).append("', NOW())").toString();
                ...
                if(!l_work_id.equals(""))
                {
                    ...
                    for(int i = 0; i < m_work_conlinks.size(); i++)
                    {
                        ...
                        l_sql = (new StringBuilder("INSERT INTO workconlink (WORKID, CONTRIBUTORID, ORDER_BY) VALUES (")).append(l_work_id).append(",").append(workContribLink.getContribId()).append(",").append(workContribLink.getOrderBy()).append(")").toString();
                        m_db.runSQLResultSet(l_sql, stmt);
                    }
                    ...
                    for(int i = 0; i < m_work_orglinks.size(); i++)
                    {
                        ...
                        l_sql = (new StringBuilder("INSERT INTO workorglink (WORKID, ORGANISATIONID, ORDER_BY) VALUES (")).append(l_work_id).append(",").append(workOrganLink.getOrganId()).append(",").append(workOrganLink.getOrderBy()).append(")").toString();
                        m_db.runSQLResultSet(l_sql, stmt);
                    }
    ...
    public boolean updateWork()
    {
        ...
        try
        {
            ...
            if(!l_rs.next())
            {
                ...
                {
                    WorkContribLink workContribLink = (WorkContribLink)m_work_conlinks.elementAt(i);
                    l_sql = (new StringBuilder("INSERT INTO workconlink (WORKID, CONTRIBUTORID, ORDER_BY) VALUES (")).append(m_workid).append(",").append(workContribLink.getContribId()).append(",").append(workContribLink.getOrderBy()).append(")").toString();
                    m_db.runSQLResultSet(l_sql, stmt);
                }
                ...
                {
                    WorkOrganLink workOrganLink = (WorkOrganLink)m_work_orglinks.elementAt(i);
                    l_sql = (new StringBuilder("INSERT INTO workorglink (WORKID, ORGANISATIONID, ORDER_BY) VALUES (")).append(m_workid).append(",").append(workOrganLink.getOrganId()).append(",").append(workOrganLink.getOrderBy()).append(")").toString();
                    m_db.runSQLResultSet(l_sql, stmt);
                }
    ...
    public boolean isInUse()
    {
        ...
        {
            Statement stmt = m_db.m_conn.createStatement();
            sqlString = (new StringBuilder("select sum(c.counter) counter from ( select count(*) as counter from itemworklin" +
"k where workid="
)).append(m_workid).toString();
            sqlString = (new StringBuilder(String.valueOf(sqlString))).append(" UNION ALL select count(*) as counter from eventworklink where workid=").append(m_workid).append(") c").toString();
...
" workorglink.organisationid = organisation.organisationid AND work.workid="
)).append(m_workid).append(" ").append("ORDER BY WORKORGLINK.ORDER_BY ").toString();
            l_rs = m_db.runSQLResultSet(l_sql, stmt);
...
" ON (contributor.contributorid = conevlink.contributorid) WHERE  work.workid="
)).append(p_id).append(" ").toString();
...
"ink`.`organisationid`= `organisation`.`organisationid`) WHERE  work.workid="
)).append(p_id).append(" ").toString();
            l_rs = m_db.runSQLResultSet(l_sql, p_stmt);
        }
        catch(Exception e)
...
"eid = venue.venueid)  INNER JOIN states ON (venue.state = states.stateid) WHERE " +
"work.workid="
)).append(p_id).append(" order by events.FIRST_DATE DESC").toString();
            l_rs = m_db.runSQLResultSet(l_sql, p_stmt);
        }
        catch(Exception e)
        {
    ...
    public ResultSet getAssociatedItems(int p_id, Statement p_stmt)
    {
        ...
        try
        {
            l_sql = (new StringBuilder("SELECT DISTINCT item.ITEMID,item.citation,work.WORKID FROM item INNER JOIN itemw" +
"orklink ON (item.ITEMID = itemworklink.ITEMID)  INNER JOIN work ON (itemworklink" +
".WORKID = work.WORKID) WHERE  work.workid="
)).append(p_id).append(" order by item.citation").toString();
            l_rs = m_db.runSQLResultSet(l_sql, p_stmt);
        }
        catch(Exception e)
        {
...
"k, work WHERE work.workid = workconlink.workid AND workconlink.contributorid = c" +
"ontributor.contributorid AND work.workid="
)).append(m_workid).append(" ").append("ORDER BY workconlink.ORDER_BY ").toString();
            l_rs = m_db.runSQLResultSet(l_sql, stmt);
...
"conlink ON work.workid = workconlink.workid LEFT JOIN contributor ON workconlink" +
".contributorid = contributor.contributorid WHERE work.workid =  "
