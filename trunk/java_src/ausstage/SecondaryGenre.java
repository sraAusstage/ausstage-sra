...
public class SecondaryGenre extends AusstageInputOutput {
	...
	public boolean add() {
		...
		try {
			...
			if (validateObjectForDB()) {
				...
				if (m_preferred_id == 0) {
					sqlString = "Insert into SECGENREPREFERRED " + "(PREFERREDTERM) values " + "('" + m_db.plSqlSafeString(m_new_pref_name) + "')";
					m_db.runSQL(sqlString, stmt);

					setPrefId(Integer.parseInt(m_db.getInsertedIndexValue(stmt, "SECGENREPREFERREDID_SEQ")));
				}

				sqlString = "INSERT INTO SECGENRECLASS " + "(GENRECLASS, GENRECLASSDESCRIPTION, SECGENREPREFERREDID) " + "VALUES (" + "'" + m_db.plSqlSafeString(m_name) + "', "
	...
	public boolean update() {
		...
		try {
			...
			if (validateObjectForDB()) {
				...
				sqlString = "UPDATE SECGENRECLASS " + "SET GENRECLASS = '" + m_db.plSqlSafeString(m_name) + "', " + "GENRECLASSDESCRIPTION = '"
						+ m_db.plSqlSafeString(m_description) + "', " + "SECGENREPREFERREDID = " + m_preferred_id + " " + "WHERE GENRECLASSID = " + m_id;
				m_db.runSQL(sqlString, stmt);
			}
			stmt.close();
		} catch (Exception e) {
	...
	public boolean delete() {
		...
		try {
			...
			if (!isInUse()) {
				...
				// Get the perferred ID so that we can delete the peferred term
				// id this
				// Secondary Genre is the last one for this preferred ID.
				sqlString = "SELECT SECGENREPREFERREDID " + "FROM SECGENRECLASS WHERE GENRECLASSID = " + m_id;
				l_rs = m_db.runSQL(sqlString, stmt);

				if (l_rs.next()) {
					secgenprefid = l_rs.getString("SECGENREPREFERREDID");

					sqlString = "SELECT count(*) as counter FROM SECGENRECLASS " + "WHERE SECGENREPREFERREDID = " + secgenprefid;
				...
				if (delPreferred) {
					...
					sqlString = "DELETE FROM SECGENREPREFERRED " + "WHERE SECGENREPREFERREDID = " + secgenprefid;
					m_db.runSQL(sqlString, stmt);
				}
			}
			stmt.close();
	...
	public boolean isInUse() {
		...
		try {
			...
			sqlString = "select SECGENREPREFERRED.SECGENREPREFERREDID from " + "SECGENREPREFERRED, SECGENRECLASS " + "where SECGENRECLASS.GENRECLASSID = " + m_id + " "
			...
			if (l_rs.next()) {
				...
				sqlString = "select count(*) as counter from SECGENRECLASS " + "where SECGENREPREFERREDID=" + l_secgenrepreferredid;
				l_rs = m_db.runSQL(sqlString, stmt);
				l_rs.next();
				counter = l_rs.getInt("counter");

				if (counter <= 1) {
					// this is the last genre that is related to the preferred
					// term
					// now check if preferred term is in use
					sqlString = "select count(*) as counter from SECGENRECLASSLINK where " + "SECGENREPREFERREDID=" + l_secgenrepreferredid;
					l_rs = m_db.runSQL(sqlString, stmt);
					l_rs.next();
					counter = l_rs.getInt("counter");
					if ((counter > 0)) ret = true; // can't delete genre & pref
													// term
				}
			}
			stmt.close();
	...
	public void loadLinkedProperties(int p_id) {
		...
		String sqlString = "select SECGENREPREFERREDID," + "PREFERREDTERM " + "from SECGENREPREFERRED " + "where SECGENREPREFERREDID=" + p_id;
	...
	public String getGenreInfoForItemDisplay(int p_secgenreprefered_id, Statement p_stmt) {
		...
		try {
			...
			sqlString = "SELECT concat_ws(', ',SECGENRECLASS.GENRECLASS, " + "SECGENREPREFERRED.PREFERREDTERM) output " + "FROM SECGENRECLASS, SECGENREPREFERRED "
					+ "WHERE SECGENRECLASS.SECGENREPREFERREDID = SECGENREPREFERRED.SECGENREPREFERREDID " + "AND SECGENRECLASS.SECGENREPREFERREDID= " + p_secgenreprefered_id;
