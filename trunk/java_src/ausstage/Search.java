...
import java.util.GregorianCalendar;
  ...
  private void buildSqlSearchString() {
    ...
    if (m_yyyyfirst_date != null && !m_yyyyfirst_date.equals("")) {
      ...
      if (!m_ddfirst_date.equals("")) {
        m_sql_string.append(" and first_date=STR_TO_DATE('" + 
                            m_yyyycopyright_date + "-" + m_mmcopyright_date + 
                            "-" + m_ddcopyright_date + "','%Y-%m-%d') ");

      } else if (!m_mmcopyright_date.equals("") && 
        ...
        if (m_mmcopyright_date.equals("02")) {
          ...
          if (cal.isLeapYear(Integer.parseInt(m_yyyyfirst_date))) {
            m_sql_string.append(" and (first_date between STR_TO_DATE('" + 
                                m_yyyyfirst_date + 
                                "-02-01','%Y-%m-%d') and ");
            m_sql_string.append("STR_TO_DATE('" + m_yyyyfirst_date + 
                                "-02-29','%Y-%m-%d')) ");
          } else {
            m_sql_string.append(" and (first_date between STR_TO_DATE('" + 
                                m_yyyyfirst_date + 
          ...
          String num_of_days = 
            Integer.toString(getNumberOfDays(m_mmfirst_date, Integer.parseInt(m_yyyyfirst_date)));

          m_sql_string.append(" and (first_date between STR_TO_DATE('" + 
                              m_yyyyfirst_date + "-" + m_mmfirst_date + 
                              "-01','%Y-%m-%d') and ");
          m_sql_string.append("STR_TO_DATE('" + m_yyyyfirst_date + "-" + 
                              m_mmfirst_date + "-" + num_of_days + 
                              "','%Y-%m-%d')) ");
        }
      } else { // only YEAR is specified
    	  
        m_sql_string.append(" and (first_date between STR_TO_DATE('" + 
                            m_yyyyfirst_date + "-01-01','%Y-%m-%d') and ");
        m_sql_string.append("STR_TO_DATE('" + m_yyyyfirst_date + 
                            "-12-31','%Y-%m-%d')) ");
      }
    } else if ((m_yyyybetween_from_date != null && 
                !m_yyyybetween_from_date.equals("")) && 
               (m_yyyybetween_to_date != null && 
                !m_yyyybetween_to_date.equals(""))) {
    ...
    // normal query does not limit on the resource flag, so only apply filter if hiding resources
  ...
  private void buildBetweenDateWhereClause(String l_date_field) {
    ...
    if (!m_ddbetween_from_date.equals("")) {
      m_sql_string.append("  (" + l_date_field + " between STR_TO_DATE('" + 
                          m_yyyybetween_from_date + "-" + 
                          m_mmbetween_from_date + "-" + m_ddbetween_from_date + 
                          "','%Y-%m-%d') and ");
    } else if (!m_mmbetween_from_date.equals("") && 
               !m_yyyybetween_from_date.equals("")) {
      m_sql_string.append("  (" + l_date_field + " between STR_TO_DATE('" + 
                          m_yyyybetween_from_date + "-" + 
                          m_mmbetween_from_date + "-01','%Y-%m-%d') and ");
    } else { 
    ...
    if (!m_ddbetween_to_date.equals("")) {
      m_sql_string.append("STR_TO_DATE('" + m_yyyybetween_to_date + "-" + 
                          m_mmbetween_to_date + "-" + m_ddbetween_to_date + 
                          "','%Y-%m-%d')) ");
    } else if (!m_mmbetween_to_date.equals("") && 
               !m_yyyybetween_to_date.equals("")) {
      ...
      if (m_mmbetween_to_date.equals("02")) {
        ...
        // lets get the last date of the given month & year
        String num_of_days = 
          Integer.toString(getNumberOfDays(m_mmbetween_to_date, 
                                           Integer.parseInt(m_yyyybetween_to_date)));
        m_sql_string.append("STR_TO_DATE('" + m_yyyybetween_to_date + "-" + 
                            m_mmbetween_to_date + "-" + num_of_days + 
                            "','%Y-%m-%d')) ");
      }
  ...
  private void buildDateWhereClause(String l_date_field) {
    ...
    if (m_yyyyfirst_date != null && !m_yyyyfirst_date.equals("")) {
      ...
      // assume that if here then dd mm and yyyy are populated and make a valid date
      // if the dd, mm and yyyy are not populated then, look at between date fields
      //
      if (!m_ddfirst_date.equals("")) {
        m_sql_string.append("  " + l_date_field + " =STR_TO_DATE('" + 
                            m_yyyyfirst_date + "-" + m_mmfirst_date + "-" + 
                            m_ddfirst_date + "','%Y-%m-%d') ");
        ...
        if (m_mmfirst_date.equals("02")) {
          ...
          GregorianCalendar cal = new GregorianCalendar();
          if (cal.isLeapYear(Integer.parseInt(m_yyyyfirst_date))) {
            m_sql_string.append("  (" + l_date_field + 
                                " between STR_TO_DATE('" + m_yyyyfirst_date + 
                                "-02-01','%Y-%m-%d') and ");
            m_sql_string.append("STR_TO_DATE('" + m_yyyyfirst_date + 
                                "-02-29','%Y-%m-%d')) ");
          } else {
  ...
  private void buildResourceSqlSearchString() {
    ...
    if (m_key_word != null && !m_key_word.trim().equals("")) {
      if (m_key_word.indexOf(" ") != -1) { // multiple keywords
        ...
        ///////////////////////////////////////////////////////////
        if (m_search_within_result_str.equals(""))
          m_search_within_result_str = 
              getFormattedMultipleKeyWords(m_key_word, "combined_all");
        else
          m_sql_string.append(" and " + m_search_within_result_str);

      } else { // single keyword
        m_sql_string.append("MATCH (combined_all) AGAINST ('" + 
                            m_db.plSqlSafeString(m_key_word.toLowerCase()) + "') ");
    ...
    if (m_yyyybetween_to_date != null && !m_yyyybetween_to_date.equals("") &&
        m_yyyybetween_from_date != null && !m_yyyybetween_from_date.equals("") ){
      m_sql_string.append(" and ( ");
    ...
    f_firstdate_yyyy
    */
    if (m_collectingInstitution != null && 
        !m_collectingInstitution.equals("")) {
      m_sql_string.append(" and INSTITUTIONID in (" + m_collectingInstitution + 
                          ")");
    }

    if (m_work != null && !m_work.equals("")) {
      m_sql_string.append(" and itemid in (SELECT itemid " + 
                          "                  FROM ITEMWORKLINK IW, WORK W " + 
                          "                 WHERE w.workid = iw.workid" + 
                          "                   and upper(work_title) LIKE '%" + 
                          m_work.toUpperCase() + "%' )");
    }
    if (m_event != null && !m_event.equals("")) {
      m_sql_string.append(" and itemid in (SELECT itemid " + 
                          "                  FROM ITEMEVLINK IE, EVENTS E " + 
                          "                 WHERE e.eventid = ie.eventid" + 
                          "                   and upper(event_name) LIKE '%" + 
                          m_event.toUpperCase() + "%' )");
    }

    if (m_contributor != null && !m_contributor.equals("")) {
      m_sql_string.append(" and itemid in (SELECT itemid " + 
                          "                  FROM ITEMCONLINK IC, CONTRIBUTOR C " + 
                          "                 WHERE c.CONTRIBUTORID = ic.CONTRIBUTORID" + 
                          "                   AND CREATOR_FLAG ='N' " + 
                          "                   and upper(concat_ws(' ', FIRST_NAME, LAST_NAME)) LIKE '%" + 
                          m_contributor.toUpperCase() + "%' )");
    }

    if (m_venue != null && !m_venue.equals("")) {

      m_sql_string.append(" and itemid in (SELECT itemid " + 
                          "                  FROM ITEMVENUELINK IV, VENUE V " + 
                          "                 WHERE v.VENUEID = iv.VENUEID" + 
                          "                   and upper(VENUE_NAME) LIKE '%" + 
                          m_venue.toUpperCase() + "%' )");
    }
    ...
    if (m_organisation != null && !m_organisation.equals("")) {
      m_sql_string.append(" and itemid in (SELECT itemid " + 
                          "                  FROM ITEMORGLINK IO, ORGANISATION O " + 
                          "                 WHERE o.ORGANISATIONID = io.ORGANISATIONID" + 
                          "                   AND (CREATOR_FLAG ='N' OR CREATOR_FLAG is null)" + 
                          "                   and upper(NAME) LIKE '%" + 
                          m_organisation.toUpperCase() + "%' )");
    }
    ...
    if (m_source != null && !m_source.equals("")) {
      m_sql_string.append(" and sourceid in (select itemid from item where upper(title) LIKE '%" + 
                          m_source.toUpperCase() + "%' )");
    }
    ...
    if (m_secondary_genre != null) {
      ...
      m_sql_string.append(" and itemid in (SELECT itemid " + 
                          "                  FROM ITEMSECGENRELINK IW" + 
                          "                 WHERE SECGENREPREFERREDID  IN (" + 
                          secGenre + " ))");
    }
...
//	System.out.println("l_key_word: " + l_key_word);
	...
	if (field.equals("combined_all")) {
	  ...
	  // For "and" and "or" loop through the words and add the switch between
  ...
  public CachedRowSet getResources() {
    ...
    String m_sql_items = "search_resource.itemid, search_resource.item_sub_type_lov_id,search_resource.item_sub_type," +
    		"  search_resource.copyright_date,  search_resource.issued_date, search_resource.accessioned_date,  search_resource.citation_date," +
                             " search_resource.source_citation,search_resource.citation,search_resource.title"+
                             ", if(ITEM_URL is null,' ','ONLINE') resource_flag";
  ...
  public CachedRowSet getVenues() {
    ...
    m_sql_string.append("select distinct " + m_sql_items + " from search_venue " +
    		"LEFT JOIN events ON (search_venue.venueid = events.venueid) "+
  ...
  public CachedRowSet getContributors() {
    ...
    m_sql_string.append("select distinct " + m_sql_items + " from search_contributor LEFT JOIN conevlink ON (search_contributor.contributorid = conevlink.contributorid) "+
		 ...
		 "Left JOIN contfunctlink ON (search_contributor.contributorid = contfunctlink.contributorid) "+
  ...
  public CachedRowSet getOrganisations() {
    ...
    m_sql_string.append("select distinct " + m_sql_items + "  from search_organisation  LEFT JOIN orgevlink ON (orgevlink.organisationid = search_organisation.organisationid) "+
  ...
  public CachedRowSet getWorks() {
    ...
    m_sql_string.append("select distinct " + m_sql_items + "  from search_work " +
    		"LEFT  JOIN eventworklink ON (search_work.workid = eventworklink.workid) "+
"LEFT  JOIN events ON (eventworklink.eventid = events.eventid) "+
"LEFT  JOIN workconlink ON (search_work.workid = workconlink.workid) "+
...
"Left join itemworklink On (search_work.workid = itemworklink.workid) where ");
  ...
  public void setFirstDate(String p_yyyyfirst_date, String p_mmfirst_date, 
                           String p_ddfirst_date) {
  ...
  public void setBetweenFromDate(String p_yyyybetween_from_date, 
                                 String p_mmbetween_from_date, 
                                 String p_ddbetween_from_date) {
  ...
  public void setBetweenToDate(String p_yyyybetween_to_date, 
                               String p_mmbetween_to_date, 
                               String p_ddbetween_to_date) {
  ...
  public void setCopyrightDate(String p_yyyycopyright_date, 
                               String p_mmcopyright_date, 
                               String p_ddcopyright_date) {
  ...
  public void setBetweenCopyrightFromDate(String p_yyyycopybetween_from_date, 
                                          String p_mmcopybetween_from_date, 
                                          String p_ddcopybetween_from_date) {
  ...
  public void setBetweenCopyrightToDate(String p_yyyycopybetween_to_date, 
                                        String p_mmcopybetween_to_date, 
                                        String p_ddcopybetween_to_date) {
  ...
  public void setIssuedDate(String p_yyyyissued_date, String p_mmissued_date, 
                            String p_ddissued_date) {
  ...
  public void setBetweenIssuedFromDate(String p_yyyyissuedbetween_from_date, 
                                       String p_mmissuedbetween_from_date, 
                                       String p_ddissuedbetween_from_date) {
  ...
  public void setBetweenIssuedToDate(String p_yyyyissuedbetween_to_date, 
                                     String p_mmissuedbetween_to_date, 
                                     String p_ddissuedbetween_to_date) {
  ...
  public void setAccessionedDate(String p_yyyyaccessioned_date, 
                                 String p_mmaccessioned_date, 
                                 String p_ddaccessioned_date) {
  ...
  public void setBetweenAccessionedFromDate(String p_yyyyaccessionedbetween_from_date, 
                                            String p_mmaccessionedbetween_from_date, 
                                            String p_ddaccessionedbetween_from_date) {
  ...
  public void setBetweenAccessionedToDate(String p_yyyyaccessionedbetween_to_date, 
                                          String p_mmaccessionedbetween_to_date, 
                                          String p_ddaccessionedbetween_to_date) {
