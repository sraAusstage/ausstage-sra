...
public class PrimaryGenre extends AusstageInputOutput {
	 ...
	 * Name: load ()
	 * 
	 * Purpose: Sets the class to a contain the Primary Genre information for
	 * the specified Primary Genre id.
	 * 
	 * Parameters: p_id : id of the Primary Genre record
	 * 
	 * Returns: None
	 */
	 ...
	 * id of the new record in the m_primary_genre_id member.
	 */
	public boolean add() {
		boolean ret = true;
		try {
			...
			if (validateObjectForDB()) {
				...
				sqlString = "INSERT INTO PRIGENRECLASS (GENRECLASS, GENRECLASSDESCRIPTION) " + "VALUES (" + "'" + m_db.plSqlSafeString(m_name) + "', " + "'"
	...
	public boolean update() {
		...
		try {
			...
			if (validateObjectForDB()) {
				...
				sqlString = "UPDATE PRIGENRECLASS " + "SET GENRECLASS = '" + m_db.plSqlSafeString(m_name) + "', " + "GENRECLASSDESCRIPTION = '"
						+ m_db.plSqlSafeString(m_description) + "' " + "WHERE GENRECLASSID = " + m_id;
				m_db.runSQL(sqlString, stmt);
			}
			stmt.close();
		} catch (Exception e) {
