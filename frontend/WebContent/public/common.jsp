...
public void displayUpdateForm(String                p_id,
  ...
  try {	  
	//BW fixes for bugs introduced with the exhibition and full text articles
	...
	//  p_out.println(" });");
	 ...
	 // p_out.println("  var tag					=	document.getElementById(\"f_tag\");");
	  ...
	  p_out.println("    $('#UpdateFormDiv').modal('hide');");
    ...
    //BW fixes for bugs introduced with the exhibition and full text articles -- full text articles uses a newer form of dialogues that seems to have broken the feedback dialog
    ...
    p_out.println("  <tr align='left' ><td align='left' nowrap ><a href='csv.jsp?id=" + p_request.getParameter("id") + "' align='left' >Export</a> | <a style='cursor:pointer' data-toggle='modal' data-target='#UpdateFormDiv'>Feedback</a> | ");   
    ...
    p_out.println("<DIV id='UpdateFormDiv' class='modal fade' role='dialog'>");
    p_out.println("	<div class='modal-dialog' >");
    p_out.println("		<div class='modal-content'>");
    p_out.println("			<div class='modal-header'>");
    p_out.println(" 	     			<button type='button' class='close' data-dismiss='modal'>&times;</button>");
    p_out.println("	     			<h4 class='modal-title'>Provide feedback on "+p_object_name+"</h4>");
    p_out.println("	     		</div>");
    p_out.println("			<div class='modal-body'>"); 
   ...
   // p_out.println("   </table><br>");
    ...
    //bit of a shitty fix to shift the feedback modal to the BODY. This needs to be done because otherwise it is the child of a ABSOLUTELY positioned element, which means it's z-index works independently of the modal backdrop.
    // https://weblog.west-wind.com/posts/2016/Sep/14/Bootstrap-Modal-Dialog-showing-under-Modal-Background
    p_out.println("<script>");
    p_out.println("	$('#UpdateFormDiv').appendTo('body');");
    p_out.println("</script>");
