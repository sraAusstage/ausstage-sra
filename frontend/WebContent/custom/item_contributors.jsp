...
<%
  ...
  buttons_names.addElement ("Select");
    if (session.getAttribute("permissions").toString().contains("Resource Editor") || session.getAttribute("permissions").toString().contains("Contributor Editor") || session.getAttribute("permissions").toString().contains("Administrators")) {
 ...
 selected_db_sql       = "";
  ...
  if (filter_id == null)
  {
    list_db_sql = "SELECT contributor.`contributorid`,"+
		"if(min(events.yyyyfirst_date) = max(events.yyyylast_date),min(events.yyyyfirst_date),concat(min(events.yyyyfirst_date),' - ', max(events.yyyyfirst_date))) dateRange, "+
...
"Where 1=1 ";	
   ...
   // list_db_sql += "Group by contributor.contributorid order by " + request.getParameter ("f_order_by");
      if(request.getParameter ("f_order_by").equals("first_name")){
    	list_db_sql += "Group by contributor.contributorid order by " + request.getParameter ("f_order_by") + ", last_name, dateRange";
    }
    if(request.getParameter ("f_order_by").equals("last_name")){
    	list_db_sql += "Group by contributor.contributorid order by " + request.getParameter ("f_order_by") + ", first_name, dateRange";
    }
  }
