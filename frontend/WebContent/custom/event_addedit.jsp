...
pageFormater.writeHelper(out, "Event Status", "helpers_no14.gif");

pageFormater.writeTwoColTableHeader(out, "Event Status");
  LookupCode eventStatus = new LookupCode(db_ausstage);
  CachedRowSet rsetEvent = eventStatus.getLookupCodes("EVENT_STATUS");
  out.println("<select name='f_event_status_id' size='1' class='line250'>");
    out.print("<option value=''>--Select Status--</option>");

  // Display all of the Event Statuses
  while (rsetEvent.next())
  {
    out.print("<option value='" + rsetEvent.getString ("code_lov_id") + "'");

    if (rsetEvent.getString("code_lov_id").equals(eventObj.getEventStatusId()))
      out.print(" selected");
    out.print(">" + rsetEvent.getString ("Description") + "</option>");
  }
    out.print("</select");
  rsetEvent.close ();
pageFormater.writeTwoColTableFooter(out);
...
if (groupNames.contains("Administrators") || groupNames.contains("Reviewers")) {
  //Reviewed Flag
  pageFormater.writeHelper(out, "Reviewed", "helpers_no15.gif");
  ...
  pageFormater.writeHelper(out, "Data Entry Information", "helpers_no16.gif");
} else {
