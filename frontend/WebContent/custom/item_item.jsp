...
<%
  ...
  if (!session.getAttribute("permissions").toString().contains("Resource Editor") && !session.getAttribute("permissions").toString().contains("Administrators")) {
	...
	// Wrapping `itemid` column in a function in order to get around issue where the query throws 
	// Invalid Column Name
	...
	list_db_sql = "SELECT  concat(itemid) as childid,    citation  " + "FROM ITEM item left join organisation orga on item.institutionid = orga.organisationid " + "Where 1=1 ";
