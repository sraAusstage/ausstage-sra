...
if (session.getAttribute("userName")!= null) {
  ...
  if(contrib_id != null){
    ...
    out.println("     <td class='record-value bold' style='padding-right:290px'>" + contributor.getPrefix() + " " + contributor.getName() + " " + contributor.getMiddleName() + " " + contributor.getLastName() + " " + contributor.getSuffix());
 ...
 // Contributors
	...
	sqlString = "SELECT DISTINCT events.eventid,events.event_name,events.ddfirst_date,events.mmfirst_date, "+
	     "events.yyyyfirst_date,concat(events.first_date, '') as first_date,venue.venue_name,venue.suburb,states.state,contributorfunctpreferred.preferredterm, evcount.num "+
	...
	sqlString2 = 
	    "SELECT DISTINCT events.eventid,events.event_name,events.ddfirst_date,events.mmfirst_date,events.yyyyfirst_date, "+
	  	"concat(events.first_date, '') as first_date,venue.venue_name,venue.suburb,states.state,organisation.name,organisation.organisationid,evcount.num "+
	  	",concat_ws(', ', venue.venue_name, venue.suburb, if(states.state='O/S', country.countryname, states.state)) venue " +
...
<%
 ...
 //Exhibitions
 if (!(contributor.getExhibitions()).isEmpty()) {
							
%>
	<tr>
		<th class='record-label b-105'>Exhibitions</th>				
			<td class='record-value' colspan='2'>
				<ul>
				<%Vector<Exhibition> exhibitions = contributor.getExhibitions();			
