 ...
 */
 ...
 /* Viewer Controller class. controls interaction elements, and viewer objects  */
	...
	var TO_MANY_SELECTED = 'Only one contributor can be selected for contributor network';
...
ViewerControlClass.prototype.addId = function(id, data){
	...
	if (this.selectedContributors.id.length == 1){
...
console.log('displatnetwork');
		...
		$('#viewerMsg').empty().append(buildLoadingMsgBox(DATA_RETRIEVAL_MSG, "text-align:center")).show();
		...
		case 'CONTRIBUTOR':
			...
			$.jsonp({
				...
				error:function(req, textStatus, errorThrown){
						$('#viewerMsg').empty().append(buildErrorMsgBox(VIEWER_ERROR_MSG)).show();
