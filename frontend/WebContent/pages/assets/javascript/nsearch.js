...
function SearchClass(){
	...
	this.BASE_URL = '/networks/networks.jsp';
...
SearchClass.prototype.displayContributorResults = function(data){
	...
	if(i == this.QUERY_LIMIT) {
        $("#contributor_heading").empty().append("Contributors ("+this.QUERY_LIMIT+"+)");
    } else {
...
//        if(data[i].venue.suburb != null || data[i].venue.state != null) {
  //      	html = html.substr(0, html.length - 2);
    ...
    if(i == this.QUERY_LIMIT) {
       $("#event_heading").empty().append("Events ("+this.QUERY_LIMIT+"+)");
    } else {
