...
<%
	...
	String orgSqlString =	"SELECT search_organisation.organisationid, organisation.name, "+
				...
				(where_clause != null ? " AND "+ where_clause : "") +
				" group by organisation.organisationid Order by " + orgSortCol + " " + orgSortOrd ; 
  	
  	out.println("<!--"+orgSqlString + "-->");	 	
	int start= orgSqlString.indexOf("and MATCH (combined_all) AGAINST");
	int end = orgSqlString.indexOf("group by");
	//This step is because in certain cases there are 2 in boolean mode checks
	int isRepreated =orgSqlString.indexOf("IN BOOLEAN MODE",orgSqlString.indexOf("IN BOOLEAN MODE")+1);
	if(isRepreated <2){
	orgSqlString = orgSqlString.substring(0, start)+ orgSqlString.substring(end,orgSqlString.length());	
	}
	...
	//VENUE select to retrieve data associated with venues from the selected country 	
  	String	venSqlString =	"SELECT search_venue.venueid venueid,venue.venue_name name, "+ 
				...
				"WHERE venue.countryid = "+id+
			        " AND (" + where_clause + " OR venue_country like '" + m_db.plSqlSafeString(keyword.toLowerCase()) + "%')" +
			 	" group by venue.venueid Order by " + venSortCol + " " + venSortOrd ; 
  	
  	out.println("<!--"+venSqlString + "-->");	
...
<form name="form_searchSort_report" method="POST" action="." style="margin-bottom:5em">	
...
<table class="sub-browse-table ">
	...
	%>
	        <tr class="<%=evenOdd[evenOddValue]%>">
        		...
        		<td width="25%"><%=(org_rs.getString("num").equals("0"))?"":org_rs.getString("num")%></td>  
        	</tr>
	<%         
	}
	%>
...
%>
	        <tr class="<%=evenOdd[evenOddValue]%>">
	        	...
	        	<td width="25%"><%=(ven_rs.getString("num").equals("0"))?"":ven_rs.getString("num")%></td>  
	        </tr>
	<%         
		}
	%>
	...
	<thead>
    		<tr>
     		 ...
     		 <th width="25%" align="left" class="browse-th-work"><a href="#" onClick="reSortDataWor('creator')">Creators</a></th>
      		 ...
      		 <th width="25%" align="left" class="browse-th-work"><a href="#" onClick="reSortDataWor('num')">Events</a></th>
    		</tr>
    	</thead>
<%
