...
<script>
	//ugly fix by Brad - some weirdness happening with page scrolling. this stops an extra scroll bar appearing on the body - I assume it has something to do with including extra jsps, that aren't immediately styled? 
...
<%
  ...
  String         validSql = "ASC DESC event_name venue_name first_date total";
  String         orderBy                         = request.getParameter("f_order_by");
  String         sortOrd                         = request.getParameter("order");
  if (orderBy == null || !validSql.contains(orderBy)) orderBy = "name";
  ...
  if(crset != null && crset.next()){
    ...
    if(page_num == null){
     ...
     out.print("<div class=\"search\"><div class=\"search-bar b-90\">"
    		 ...
    		 +"</span></div>");
      out.println("     <div class='scroll-table'> <table class=\"search-table\">");
...
%>    
    ...
    <tr>
      ...
      <th width="15%" align="right" style="text-align:right;padding-right: 16px;"><a href="#" onClick="reSortNumbers('first_date')">First Date</a></th>
      <th width="15%" align="right" style="text-align:right;padding-right: 16px;"><a href="#" onClick="reSortNumbers('total')">Resources</a></th>
...
// DISPLAY SEARCH RESULT(S)
    do{
      ...
      if(do_print){
        ...
        if(crset.getString("state").equals("O/S")){ venueAddress += crset.getString("countryname");}
      	...
      	%>
        <tr>
        	<td width="35%" class="<%=bgcolour%>" valign="top" ><a href='/pages/event/<%=crset.getString("eventid")%>' onmouseover="this.style.cursor='hand';"><%=crset.getString("event_name")%></a></td>
        	<td width="35%" class="<%=bgcolour%>" valign="top" ><%=crset.getString("venue_name")+", "+venueAddress%></td>         
       		<td width="15%" class="<%=bgcolour%>" valign="top" align='right' ><%=formatDate(crset.getString("DDFIRST_DATE"), crset.getString("MMFIRST_DATE"),crset.getString("YYYYFIRST_DATE"))%></td>
        	<td width="15%" class="<%=bgcolour%>" valign="top" align='right' ><%if(!crset.getString("total").equals("0"))out.println(crset.getString("total"));%></td>   
        </tr>
        <%
    ...
    if(Integer.parseInt(recset_count) > Integer.parseInt(resultsPerPage)){
      ...
      out.println("     </table></div></div>");}//end first if
  else{
