...
<%@ page import = "ausstage.ItemArticle"%>
...
<%
  ...
  if (resultsPerPage == null || !"20 50 75 100".contains(resultsPerPage)) resultsPerPage = "100";
	...
	out.println("<thead>");
    	out.println("	<tr>");
    	out.println("		<th width=\"5%\"><a href=\"#\" onClick=\"reSortData ('item_sub_type')\">Type</a></th>");
      	out.println("		<th width=\"95%\"><a href=\"#\" onClick=\"reSortData ('customsort')\">Citation</a></th> "); 
      	out.println("	</tr>	");	
    	out.println("</thead>");
      ...
      do{
        ...
        if(do_print){
         ...
         %>
          <tr>
	  	<td width="5%" <%=bgcolour%> valign="top" ><%=crsetResult.getString("item_sub_type")%></td> 
          	<td width="95%" <%=bgcolour%> valign="top" >
          	         	       					<!----------------------->											
	  <%
		ItemArticle article = new ItemArticle(db_ausstage_for_collection_result);
		article.load(crsetResult.getString("itemid"));
	  ...
	  %>
			...
			<span class='glyphicon glyphicon-eye-open clickable' data-toggle="modal" data-target='#article<%=crsetResult.getString("itemid")%>'></span>
				<div id='article<%=crsetResult.getString("itemid")%>' class="modal fade" role="dialog">
					<div class="modal-dialog wysiwyg" >
					<!-- Modal content-->
					   <div class="modal-content">
						<div class="modal-header">
					          <button type="button" class="close" data-dismiss="modal">&times;</button>
					         <h4 class="modal-title"><%=crsetResult.getString("citation")%></h4>
					       </div>		      
	...
	%>
          	...
          	<a href="/pages/resource/<%=crsetResult.getString("itemid")%> " >
          	<%= crsetResult.getString("citation")%></a></td>
