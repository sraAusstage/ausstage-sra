...
<thead>
    <tr>
      <th style="width:25%"><a href="#" onClick="reSortData('name')">Name  (<%=SearchCount.formatSearchCountWithCommas(l_rs.getString(1))%>)</a></th>
      <th style="width:40%"><a href="#" onClick="reSortData('function')">Functions</a></th>
      <th style="width:15%" align="left"><a href="#" onClick="reSortNumbers('year')">Event Dates</a></th>
      <th style="width:10%; text-align:right" align="right"><a href="#" onClick="reSortNumbers('num')">Events</a></th>
      <th style="width:10%; text-align:right" align="right"><a href="#" onClick="reSortNumbers('total')">Resources</a></th>
    </tr>
</thead>
    <%
    sqlString = 	"SELECT contributor.`contributorid`,contributor.last_name,contributor.first_name, "+
			...
			" WHERE TRIM(leading ' ' from lcase(`contributor`.last_name)) " + switchLike + " group by `contributor`.contributorid " +
  			//" ORDER BY  " + (sortCol.equals("name")? ((letter.equals("0"))?"convert(":"")+ "last_name " + ((letter.equals("0"))?", decimal)":"")+ sortOrd + ", "+((letter.equals("0"))?"convert(":"")+"first_name"+((letter.equals("0"))?", decimal)":"function"):sortCol) 
			" ORDER BY  " + (sortCol.equals("name")? ((letter.equals("0"))?"convert(":"")+ "last_name " + ((letter.equals("0"))?", decimal)":"")+ sortOrd + ", "+((letter.equals("0"))?"convert(":"")+"first_name"+((letter.equals("0"))?", decimal)":""):(sortCol.equals("function")? "`function`":sortCol))
    ...
    <tr class="<%=evenOdd[evenOddValue]%>">
      <td width="25%"><a href="/pages/contributor/<%=l_rs.getString("contributorid")%>"><%=l_rs.getString("first_name")==null?"":l_rs.getString("first_name")%> <%=l_rs.getString("last_name")==null?"":l_rs.getString("last_name")%></a></td>
      <td width="40%" align="left"><%if (l_rs.getString("function") != null) out.write(l_rs.getString("function")); %></td>
      <td width="15%" align="left"><%   	     
        if (l_rs.getString("daterange") != null) 
      	{
          out.write(l_rs.getString("daterange"));
      	}%>
      </td>
     <td width="10%" align="right"><%if(l_rs.getString("num").equals("0")){
   	  out.write("");}else
   	  out.write(l_rs.getString("num")); %>
      </td>  
      <td width="10%" align="right"><%if(l_rs.getString("total").equals("0")){
   	  out.write("");}else
   	  out.write(l_rs.getString("total")); %>
      </td>  	 
    </tr>
