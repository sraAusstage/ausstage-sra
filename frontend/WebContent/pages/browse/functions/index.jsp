...
<%
  ...
  if (letter.length() > 1) letter = letter.substring(0,1);
  letter = letter.toLowerCase();
  String validSql = "total preferredterm DESC ASC";
  String sortCol = request.getParameter("col");
  if (sortCol == null || !validSql.contains(sortCol)) sortCol = "preferredterm";
...
<table class="browse-table">
  ...
  <%-- These are hidden inputs that will be populated by the reSortData() JavaScript function. --%>
    ...
    <thead>
    <tr>
      <th width="50%"><b><a href="#" onClick="reSortData('preferredterm')">Name  (<%=SearchCount.formatSearchCountWithCommas(l_rs.getString(1))%>)</a></b> </th>
      
      <th width="50%" align="right" style="text-align:right"><b><a href="#" onClick="reSortNumbers('total')">Contributors</a></b></th>
    </tr>
    </thead>
    <%
...
//			"Order by " + sortCol + " " + sortOrd + ", preferredterm limit " + ((pageno)*resultsPerPage) + ","+(resultsPerPage+1);  
    ...
    <tr class="<%=evenOdd[evenOddValue]%>">
      <td width="50%"><a href="/pages/function/<%=l_rs.getString(3)%>"><%=l_rs.getString(1)%></a></td>
       
      <td width="50%" align="right"><%=l_rs.getString(2)%></td>  	 
    </tr>
