...
<%@ page import = "ausstage.OrgEvLink"%>
...
<div style="margin: 0px;">
	<!--<div class='b-121 exhibition-element-header show-hide' >-->
	<div class='b-121 browse-bar show-hide' >
	<%
	...
	String org_id               = request.getParameter("organisationid");
	Event event = null;
	Country country;
	OrgEvLink orgEvLink;
	Organisation organisation = new Organisation(db_ausstage);
	country = new Country(db_ausstage);
	...
	if(org_orglinks.size() > 0){
		assocOrganisation = new Organisation(db_ausstage);
	}
	%>
	<script type="text/javascript">
	function displayOrgRow(name, id){
		document.getElementById("org_venue"+id).style.display = 'none';
		document.getElementById("org_venue"+id+"btn").style.backgroundColor = '#aaaaaa';
		document.getElementById("org_organisation"+id).style.display = 'none';
		document.getElementById("org_organisation"+id+"btn").style.backgroundColor = '#aaaaaa';
		document.getElementById("org_contributor"+id).style.display = 'none';
		document.getElementById("org_contributor"+id+"btn").style.backgroundColor = '#aaaaaa';
		document.getElementById("org_events"+id).style.display = 'none';
		document.getElementById("org_events"+id+"btn").style.backgroundColor = '#aaaaaa';
	...
	//Name
	%>					
<!--	<a style="color: white;" target='_blank' href='/pages/organisation/<%= org_id%>'>-->
		<img src="/resources/images/icon-organisation.png" class="browse-icon"><span class='browse-heading large' ><%=organisation.getName()%></span>
<!--	</a>-->
	...
	<div style="width: 100%;" class="toggle-div">					
		<div class='record' style="margin: 0px; padding-bottom: 0em;">
			<table class='record-table'  style="margin-top: 0px;">
				...
				%>
				<tr>
					<th class='record-label organisation-light'>Other Names</th>
				...
				//Address
						%>
						<tr>
							<th class='record-label organisation-light'>Address</th>
						...
						%>
						<tr>
							<th class='record-label organisation-light'>First Date</th>
								<td class='record-value' colspan='2'>
						...
						%>
						<tr>
							<th class='record-label organisation-light'>Place of Origin</th>
						...
						%>
						<tr>
							<th class='record-label organisation-light'>Last Date</th>
								<td class='record-value' colspan='2'>
						...
						%>
						<tr>
							<th class='record-label organisation-light'>Place of Demise</th>
	...
	%>
				<tr>
					<th class='record-label organisation-light '>Website</th>				
					<td class='record-value' colspan='2'>
						<a href='<%=(organisation.getWebLinks().indexOf("http://") < 0)?"http://":""%><%=organisation.getWebLinks()%>'><%=organisation.getWebLinks()%></a>
					</td>
				</tr>
	<%
	}
	//Functions
						...
						%>
							<tr>
								<th class='record-label organisation-light'>Functions</th>
						...
						%>
							<tr >
								<th class='record-label organisation-light'>NLA</th>
						...
						%>
							<tr>
								<th class='record-label organisation-light'>Notes</th>
						...
						if (assocOrganisation != null && org_orglinks.size() > 0) {
							%>
							<tr>
								<th class='record-label organisation-light'>Related Organisations</th>
								...
								<td class='record-value' colspan='2'>
									...
									for (OrganisationOrganisationLink orgOrgLink : (Vector <OrganisationOrganisationLink>) org_orglinks) {
										...
										RelationLookup lookup = new RelationLookup(db_ausstage);
										...
										<li>
												...
												//EDIT BW 02/17. Date range for associated organisations is first_year - last_year of organisation
												String firstYear = assocOrganisation.getYyyyfirstDate(); // get first year
												String lastYear = assocOrganisation.getYyyylastDate();	 // last year
												// then combine, only inserting the divider if both are there.
												String returnDate = firstYear + ((firstYear.length() < 4 || lastYear.length() < 4) ? "" : " - ") + lastYear;
...
//												}
												//only print the dates if theres anythign worth printing. 
	...
	//get associated objects
					...
					<tr>
						<th class='record-label organisation-light'>Events</td>
						
						<td class='record-value' id="tabs" colspan=2>
							<ul class='record-tabs label'>
								<li><a href="#" onclick="displayOrgRow('org_events<%=organisation.getId()%>','<%=organisation.getId()%>')" id='org_events<%=organisation.getId()%>btn'>Date</a></li>
								<li><a href="#" onclick="displayOrgRow('org_contributor<%=organisation.getId()%>', '<%=organisation.getId()%>')" id='org_contributor<%=organisation.getId()%>btn'>Contributor</a></li>   
								<li><a href="#" onclick="displayOrgRow('org_organisation<%=organisation.getId()%>', '<%=organisation.getId()%>')" id='org_organisation<%=organisation.getId()%>btn'>Organisation</a></li> 	
								<li><a href="#" onclick="displayOrgRow('org_venue<%=organisation.getId()%>', '<%=organisation.getId()%>')" id='org_venue<%=organisation.getId()%>btn'>Venue</a></li>
							</ul>
						</td>
					</tr>
				    <%
				    ...
				    %>
				    <tr id='org_events<%=organisation.getId()%>' class='events-tab'>
					<%
					
				    // Events Tab	
					if (crsetEvt.size() > 0) {
					%>
							<th class='record-label organisation-light'></th>
				    ...
				    <tr id='org_organisation<%=organisation.getId()%>' class='events-tab'>
				    <%
					//Organisation Tab				 
					String prevOrg = "";
					if (crsetOrg.size() > 0) {
						%>
						<th class='record-label organisation-light'></td>
					...
					<tr id='org_venue<%=organisation.getId()%>' class='events-tab'>
					<%
					//Venue Tab				 
					String prevVen = "";
					if (crsetVen.size() > 0) {
					...
					<tr id='org_contributor<%=organisation.getId()%>' class='events-tab'>
					<%
				    //Contributor Tab			
					String prevCont = "";
					if (crsetCon.size() > 0) {
	...
	<%
					...
					%>
					<tr >
						<th class='record-label organisation-light'>
					...
					%>
						<tr>
					 		<th class='record-label organisation-light'>Resources</th>
							...
							<td class='record-value' colspan='2'>
								...
								%>
									<li> 
								      ...
								      <span class='glyphicon glyphicon-eye-open clickable' data-toggle="modal" data-target='#article<%=rset.getString("itemid")%><%=org_id%>'></span>
	
								      <div id='article<%=rset.getString("itemid")%><%=org_id%>' class="modal fade" role="dialog">
					...
					%>
					<tr>
						<th class='record-label organisation-light'>Organisation Identifier</th>
			...
			<script>
				if (!document.getElementById("org_venue<%=organisation.getId()%>").innerHTML.match("[A-Za-z]")) document.getElementById("org_venue<%=organisation.getId()%>btn").style.display = "none";
				if (!document.getElementById("org_organisation<%=organisation.getId()%>").innerHTML.match("[A-Za-z]")) document.getElementById("org_organisation<%=organisation.getId()%>btn").style.display = "none";
				if (!document.getElementById("org_contributor<%=organisation.getId()%>").innerHTML.match("[A-Za-z]")) document.getElementById("org_contributor<%=organisation.getId()%>btn").style.display = "none";
				if (!document.getElementById("org_events<%=organisation.getId()%>").innerHTML.match("[A-Za-z]")) document.getElementById("org_events<%=organisation.getId()%>btn").style.display = "none";
				displayOrgRow("org_events<%=organisation.getId()%>", "<%=organisation.getId()%>");
			</script>				
		</div>
	</div>
</div>
